#!/usr/bin/env python3
"""Validate the frozen GDT323 register-conditioning design."""
import csv
import hashlib
import json
...
OUT = R / "gdt323_design_validation.json"
...
def main():
    ...
    check("registers", sorted({row["register"] for row in panel}) == sorted(design["registers"]))
    check("models", design["models"] == ["CELL", "GLOBAL_TWO_RULE", "CURRIER_TWO_RULE", "REGISTER_TWO_RULE"])
    check("counts", design["parameter_counts"] == {"GLOBAL_TWO_RULE": 2, "CURRIER_TWO_RULE": 4, "REGISTER_TWO_RULE": 10})
    check("effects", design["effects"] == ["s_X_line_first", "q_X_prev_dy"])
    ...
    validation = {"schema": "GDT323_DESIGN_VALIDATION_V1", "status": "PASS", "checks_passed": len(checks), "checks": checks, "design_sha256": sha(DESIGN), "f84_rows": 0}
