#!/usr/bin/env python3
"""Run the unchanged GDT170 surface instrument on GDT171 observations only."""
...
SOURCE = R / "gdt171_observation_corpus.json.gz"
...
OPERATIONS = R / "gdt171_blind_operations.tsv"
DIAGNOSTICS = R / "gdt171_blind_diagnostics.tsv"
...
def main() -> None:
    ...
    assert design["status"] == "FROZEN_UNCHANGED_GDT170_INSTRUMENT_BEFORE_V2_BLIND_PARSE"
    assert design["parent_runner_sha256"] == sha(PARENT_RUNNER)
    with gzip.open(SOURCE, "rt", encoding="utf8") as handle: payload = json.load(handle)
    assert payload["schema"] == "GDT171_STRICT_OBSERVATION_CORPUS_V1"; rows = payload["rows"]
    ...
    for world, values in sorted(by_world.items()):
        ...
        selected_by[world] = (left, right); selected = {("LEFT", x) for x in left} | {("RIGHT", x) for x in right}
        for item in stats:
        ...
        for mode in MODES:
            ...
            rec = record_metrics(parsed); comp = compatibility(set(counts), left, right, world + mode); short, same, external = short_and_substitution(parsed)
    ...
    # GDT167 analogue: partitioned register spaces, without glyph identity.
    ...
    write_gzip(PARSES, {"schema": "GDT171_BLIND_PARSES_V1", "rows": parse_rows})
    ...
    result = {"schema": "GDT171_BLIND_INSTRUMENT_RESULT_V1", "status": "GDT171_BLIND_OUTPUTS_FROZEN_BEFORE_ORACLE_EVALUATION",
              ...
              "summary": summary, "inputs": {SOURCE.name: sha(SOURCE), SOURCE_FREEZE.name: sha(SOURCE_FREEZE), DESIGN.name: sha(DESIGN), PARENT_RUNNER.name: sha(PARENT_RUNNER)},
              ...
              "blind_firewall": {"read_files": [SOURCE.name, SOURCE_FREEZE.name, DESIGN.name, PARENT_RUNNER.name, METHOD.name],
                                   "forbidden_inputs_opened": False, "oracle_fields_used": False, "voynich_inputs": 0, "f84r_access": False},
