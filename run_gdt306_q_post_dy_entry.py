...
def main():
 ...
 report+=['','## Interpretation','',f"The frozen primary and sensitivities {'all point toward a q-conditioned post-DY transition.' if all(gates.values()) else 'do not jointly support a stable q-conditioned post-DY transition.'} The primary effect is evaluated independently of same-group q parsing by reading only the immediately preceding physical group. Physical line start is kept separate.",'',"This is a prospective, exact-surface-disjoint replication/localization of the post-DY `qo...` wrapper ecology already reported by GDT024 and GDT061. It strengthens generalization beyond their frequent forms; it does not create a new compiler state or reopen their host-transition negatives.",'','## Gates','']+[f"- `{k}`: **{'PASS' if v else 'FAIL'}**" for k,v in gates.items()]+['','## Claim ceiling','',design['claim_ceiling']+' No f84 row was opened, parsed, retained, joined, or scored.'];REPORT.write_text('\n'.join(report)+'\n');outputs=[EVENTS,SCORES,NULL,COUNTER,REPORT];inputs=[SOURCE,PANEL,CAP,R/'gdt306_design_validation.json',R/'gdt305_result.json',R/'gdt304_result.json'];res={'schema':'GDT306_Q_POST_DY_ENTRY_RESULT_V1','status':status,'prior_overlap':'PROSPECTIVE_SURFACE_DISJOINT_REPLICATION_OF_GDT024_GDT061_POST_DY_Q_WRAPPER_ECOLOGY','summary':{'events':len(rows),'primary_cells':39,'primary_delta':primary['q_minus_none_delta'],'primary_exact_p':primary['exact_one_sided_p'],'line_start_delta':line['q_minus_none_delta']},'gates':gates,'semantic_assignments':0,'claim_ceiling':design['claim_ceiling'],'f84':{'input_files':0,'opened':False,'parsed':False,'retained':False,'joined':False,'scored':False},'inputs':{p.name:sha(p) for p in inputs},'documents':{METHOD.name:sha(METHOD),REPORT.name:sha(REPORT)},'implementation':{Path(__file__).name:sha(Path(__file__))},'outputs':{p.name:sha(p) for p in outputs}};res['content_sha256']=can(res);RESULT.write_text(json.dumps(res,indent=2,sort_keys=True)+'\n');print(json.dumps({'status':status,'gates':gates,'primary':primary,'line':line},sort_keys=True))
