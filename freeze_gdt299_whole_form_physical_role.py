...
def main():
 ...
 for panel in sorted({x['control_id'] for x in rows}):
  ...
  for x in eligible:strata[(x['section'],x['currier'],x['hand'],x['group_count'],x['page_host'])].append(x)
