...
up=['experiments/yolo/gdt823_qol_source_anaphor_trial/WORKING_THEORY.md', 'experiments/yolo/gdt824_qolchedy_fixed_composition/WORKING_THEORY.md', 'experiments/yolo/gdt824_qolchedy_fixed_composition/METHOD.md', 'experiments/yolo/gdt829_repeated_passage_reflow_capacity/src/run.py', 'experiments/semantic_assumptions/results/source_separator_transcription.tsv', 'experiments/yolo/gdt631_prefixed_cth_quality_parts/artifacts/PAGE_ALLOWLIST.tsv']
...
m.update(title='Complete literal qolchedy joined split context inventory',question='Do literal joined qolchedy and definite-space split qol chedy share exact outer contexts across the admitted panel?'     ,claim_ceiling='Descriptive exact written forms only; no confirmatory significance, morpheme, number or meaning.',status=status,dependencies=['GDT823','GDT824','GDT829'],inputs=[{'path':n,'sha256':sha(ROOT/n),'role':'frozen_source_or_extractor'} for n in up],outputs=[{'path':p.relative_to(ROOT).as_posix(),'sha256':sha(p),'role':'primary_report' if p.name=='REPORT.md' else 'source_or_artifact'} for p in outputs],validation={'status':'PASS' if result.exists() else 'NOT_RUN','artifact':rel+'/artifacts/VALIDATION.json' if result.exists() else None})
