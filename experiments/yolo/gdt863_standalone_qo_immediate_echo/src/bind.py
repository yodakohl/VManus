...
m.update(title='Standalone qo strict immediate echo',question='Does every eligible raw standalone qo immediately precede a qo-initial complete raw follower under strict three-reading concordance?',claim_ceiling='Literal immediate-echo contract only; no general false-start, semantic, physical boundary or pen-motion claim',status=status,dependencies=['GDT851'],inputs=[{'path':n,'sha256':sha(ROOT/n),'role':'frozen_source_or_extractor'} for n in up],outputs=[{'path':p.relative_to(ROOT).as_posix(),'sha256':sha(p),'role':'primary_report' if p.name=='REPORT.md' else 'source_or_artifact'} for p in outputs],validation={'status':validation_status,'artifact':rel+'/artifacts/VALIDATION.json' if validation_file.exists() else None})
