#!/usr/bin/env python3
"""Build GDT665: close the 148-form V41 frontier as concrete V42 recipe cards."""
...
BASE_REL = Path("experiments/yolo/gdt665_one_hundred_forty_eight_residual_family_completion")
ART = ROOT / BASE_REL / "artifacts"
G664 = Path("experiments/yolo/gdt664_one_hundred_forty_residual_family_completion")
_spec = importlib.util.spec_from_file_location("gdt664_builder_for_gdt665", ROOT / G664 / "src/run.py")
if _spec is None or _spec.loader is None:
    raise RuntimeError("cannot load GDT664 builder")
g664 = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(g664)
TOKENS_REL, CROSS_REL = g664.TOKENS_REL, g664.CROSS_REL
STATUS = "PASS_994_TARGET_POSITIONS__V42_CONCRETE_RECIPE_REGISTER"
...
def read_card_specs(path: Path) -> tuple[dict[str, str], ...]:
    ...
    if not rows or set(rows[0]) != expected:
        raise RuntimeError("bad GDT665 CARD_SPECS.tsv columns")
    surfaces = [row["surface"] for row in rows]
    if len(rows) != 148 or len(set(surfaces)) != 148:
        raise RuntimeError("GDT665 CARD_SPECS.tsv must contain 148 unique surfaces")
    return tuple(rows)
...
CONTEXT_SCOPED_SURFACES = frozenset({"keechy", "dom", "shok", "otolaiin", "kesey"})
...
LOW_SURFACES = frozenset({
    "leeesain", "teedu", "oykchor", "soiry", "sfm", "dom", "keem", "chytaroiin",
    "keishy", "olom", "qeey", "deeedy", "checphey", "oreeeg", "kesey",
})
STRONG_SURFACES = frozenset({
    "oshey", "opchy", "chry", "cheodaiin", "lcheedy", "chearaiin", "chdaldy",
    "loraiir", "lkey", "lcheor", "pcheedy", "okaram", "arain", "yty", "chckhan",
    "shed", "ycheey", "chealy", "qokeeor", "ykeey", "lsheedy", "lkchedy", "rain",
    "shotchy", "dor", "chtol", "chos", "tshoiin", "okaly", "chcthedy", "olchdaiin",
    "qotcheeaiin", "okchor", "yteey", "taraiin", "chkalykedy", "chdain", "yshol",
    "dalor", "dalshedy", "ytam", "chotchy", "ody", "aram", "qoain", "qockhy",
    "darchd", "ykair", "kchol", "psheey", "sheodaiin", "qokaiir", "qolkary",
    "okoldy", "qolsheol", "cham", "otolaiin", "doraiin", "chdor", "yched",
    "lorain", "cthdy", "qckhedy", "shedaldy", "yteedy", "okol", "chsey", "olcheey",
    "olkshey", "sheyteedy", "yshedy", "chedyor", "solchey", "okainy", "soin",
    "dshedy", "olkal", "oiiin", "qokechedy", "qolkeey", "chsdy", "chkeedy",
    "okeeol", "qolkeedy", "sheolkain", "lols", "tdam", "ykaiin", "dshey",
    "qopcheey", "keodar", "kalchdy", "ypchdair", "qokololal", "ytoldy",
})
...
OUTPUT_NAMES = (
    ...
    "FAMILY_COMPOSITION_ATLAS.tsv", "FRONTIER_148_COMPLETIONS.tsv",
    "TARGET_LINE_TRANSLATIONS.tsv", "ROUND_COVERAGE_COUNTS.tsv",
    "NEWLY_COMPLETED_LINES.tsv", "NEWLY_EXPOSED_ONE_HOLE_LINES.tsv",
    "V42_WORKING_TOKEN_GLOSSARY.tsv", "WORKING_DICTIONARY_V42.tsv",
    "ALL_LINE_CONCRETE_COVERAGE_V42.tsv", "COMPLETE_PASSAGES_V42.tsv",
    "ONE_UNKNOWN_PASSAGES_V42.tsv", "INHERITED_OL_RENDER_REVISIONS.tsv",
    "INHERITED_SOL_RENDER_REVISIONS.tsv", "STEM_MODEL_V42.tsv", "MANUAL_PASSAGE_AUDIT.tsv",
)
...
# These are practical render revisions only.  V41's structural tags remain
# byte-visible in its inherited glossary/dictionary; V42 translates the 19
...
def read_ol_revision_specs(path: Path) -> tuple[dict[str, str], ...]:
    ...
    if not rows or set(rows[0]) != expected:
        raise RuntimeError("bad GDT665 INHERITED_OL_REVISION_SPECS.tsv columns")
    surfaces = [row["surface"] for row in rows]
    if len(rows) != 19 or len(set(surfaces)) != 19 or sum(int(row["positions"]) for row in rows) != 256:
        raise RuntimeError("GDT665 OL revision source must contain 19 unique forms at 256 positions")
...
def read_sol_revision_specs(path: Path) -> tuple[dict[str, str], ...]:
    ...
    expected = {"surface", "positions", "practical_meaning_de", "composition", "retained_rival_de"}
    if not rows or set(rows[0]) != expected:
        raise RuntimeError("bad GDT665 INHERITED_SOL_REVISION_SPECS.tsv columns")
    surfaces = [row["surface"] for row in rows]
    if len(rows) != 2 or len(set(surfaces)) != 2 or sum(int(row["positions"]) for row in rows) != 4:
        raise RuntimeError("GDT665 SOL revision source must contain 2 unique forms at 4 positions")
    return tuple(rows)
...
# all token gloss markers at this locus.  V42 preserves that sentence but must
...
def read_stem_model_specs(path: Path) -> tuple[dict[str, str], ...]:
    ...
    if not rows or set(rows[0]) != expected or len(rows) != 47:
        raise RuntimeError("GDT665 STEM_MODEL_SPECS.tsv must contain the fixed 47-row schema")
    return tuple(rows)
...
def read_manual_passage_specs(path: Path) -> tuple[dict[str, str], ...]:
    ...
    if not rows or set(rows[0]) != expected or len(rows) != 30:
        raise RuntimeError("GDT665 MANUAL_PASSAGE_SPECS.tsv must contain the fixed 30-row schema")
    if [int(row["rank"]) for row in rows] != list(range(1, 31)):
        raise RuntimeError("GDT665 manual passage ranks must be 1..30")
    if len({row["locus"] for row in rows}) != 30:
        raise RuntimeError("GDT665 manual passage loci must be unique")
    return tuple(rows)
...
def line_translation(
    ...
    """Render V42 with reader joins over the complete V41 renderer."""
    working_glosses = list(glosses)
    # GDT664's renderer only applies its positional action cards to the map it
    # receives as the current layer.  Re-promote the archived G664 rows here;
    ...
    inherited = {
        ...
        if not str(row.get("occurrence_id", "")).startswith("G664-")
    }
    current = {
        key: row for key, row in inherited_target_by_token.items()
        if str(row.get("occurrence_id", "")).startswith("G664-")
    }
    ...
    rendered = g664.line_translation(
...
def build(output_dir: Path) -> dict[str, object]:
    ...
    tokens, token_stats = g664.g663.g662.g661.g659.guarded_query(
        TOKENS_REL, pages, "page,locus,token_index,eva,kind,section,language,hand"
    )
    cross, cross_stats = g664.g663.g662.g661.g659.guarded_query(
        CROSS_REL, pages,
    ...
    base_glossary_rows = read_tsv(base_art / "V41_WORKING_TOKEN_GLOSSARY.tsv")
    ...
    if dimensions != (1266, 874, 4128, 613, 332, 156):
        raise RuntimeError(f"V41 base dimensions drift: {dimensions!r}")
    frontier_order = tuple(dict.fromkeys(row["unknown_surface"] for row in frontier))
    if frontier_order != TARGET_ORDER or len(TARGET_SURFACES) != 148:
        raise RuntimeError("the 148-form frontier or fixed order drifted")
    ...
    if any(surface in base_glossary for surface in TARGET_SURFACES):
        raise RuntimeError("a GDT665 target unexpectedly already has a V41 glossary row")
    ...
    y_occurrences = read_tsv(
        ROOT / g664.g663.g662.g661.g660.G659 / "artifacts/Y_OCCURRENCE_CENSUS.tsv"
    )
    ...
    inherited_target_by_token: dict[tuple[str, int], dict[str, object]] = {}
    inherited_audits = (
        ROOT / g664.g663.g662.g661.G660 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        ROOT / g664.g663.g662.G661 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        ROOT / g664.g663.G662 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        ROOT / g664.G663 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
    ...
    if observed_counts != EXPECTED_SURFACE_COUNTS or sum(observed_counts.values()) != 994:
        raise RuntimeError(f"target surface count drift: {observed_counts!r}")
    exact, normalized = g664.g663.g662.g661.g660.stable_maps(tokens, cross_by_locus)
    ...
    for locus in sorted(by_line):
        ...
        for index, token in enumerate(line):
            ...
            item: dict[str, object] = {
                "occurrence_id": f"G665-T{len(occurrence_rows) + 1:04d}",
    ...
    if len(occurrence_rows) != 994 or len(target_by_token) != 994:
    ...
    if contextual_surface_counts != Counter({"keechy": 3, "dom": 5, "shok": 3, "otolaiin": 1, "kesey": 1}):
    ...
    for base_row in base_coverage:
        ...
        if not (len(line) == len(glosses) == len(sources) == len(states)):
            raise RuntimeError(f"V41 token columns misalign: {locus}")
        ...
        for index, token in enumerate(line):
            ...
            if glosses[index] != f"[{surface}:?]" or sources[index] != "OPEN" or states[index] != "UNKNOWN_SURFACE":
                raise RuntimeError(f"V41 target not open at {locus}.{index + 1}: {surface}")
            glosses[index] = str(occurrence["working_gloss_de"])
            if surface in CONTEXT_SCOPED_SURFACES:
                sources[index] = f"GDT665:{occurrence['rendering_class']}:{occurrence['reader_merge_surface']}"
                states[index] = "KNOWN_CONTEXT_LICENSED" if int(occurrence["reader_exact"]) else "READER_BOUNDARY_UNSTABLE"
            else:
                sources[index] = f"GDT665:EXACT_WHOLE:{surface}"
        ...
        if int(base_row["unknown_tokens"]) - len(target_ordinals) != len(unknown_pairs):
            raise RuntimeError(f"V41->V42 arithmetic drift: {locus}")
    ...
    for row in coverage_rows:
        ...
        if old and old["unknown_surface"] == surface and int(old["unknown_ordinal"]) == ordinal:
            ...
            proposal, basis, strength = f"[{surface}:?]", "NEWLY_EXPOSED_BY_GDT665_NO_NEW_CARD", "OPEN"
        strict = int(
            int(row["ambiguous_tokens"]) == 0 and int(row["reader_unstable_tokens"]) == 0
            and int(row["all_present_exact"]) == 1
        )
    ...
    for spec_row in EXACT_WHOLE_SPECS:
        ...
        glossary_rows.append({
            ...
            "source": "GDT665:CONTEXT_SHORT_DEFAULT" if surface in CONTEXT_SCOPED_SURFACES else "GDT665:EXACT_WHOLE",
        ...
        dictionary_rows.append({
            "entry": f"{surface}@GDT665_DEFAULT", "kind": card_type(surface),
            ...
            "status": "NEW_V42_PROVISIONAL_CONCRETE_RECIPE_DEFAULT",
        })
    glossary_rows.sort(key=lambda row: str(row["surface"]))
    if len(glossary_rows) != 1022:
        raise RuntimeError("V42 glossary dimension drift")
    ...
    for klass, surface, merge_surface, render in context_keys:
        ...
        context_cards.append({
            "card_id": f"G665-C{len(context_cards) + 1:03d}", "rendering_class": klass,
    ...
    for occurrence in occurrence_rows:
        ...
        audit_rows.append({
            **occurrence,
            "v41_gloss_de": split_pipe(base_row["token_glosses_de"])[ordinal - 1],
            "v42_gloss_de": split_pipe(final_row["token_glosses_de"])[ordinal - 1],
            "v41_scope_state": split_pipe(base_row["scope_states"])[ordinal - 1],
            "v42_scope_state": split_pipe(final_row["scope_states"])[ordinal - 1],
            "v42_working_translation_de": line_translation(
    ...
    if any(str(row["v41_gloss_de"]) != f"[{row['surface']}:?]" for row in audit_rows):
        raise RuntimeError("not every target occurrence was open in V41")
    if any(GENERIC_FILLER.search(str(row["v42_working_translation_de"])) for row in audit_rows):
        raise RuntimeError("generic work filler leaked into GDT665")
    if any("Eigenschafts-/Zustands-/Materialträger" in str(row["v42_working_translation_de"]) for row in audit_rows):
        raise RuntimeError("structural OL meta-gloss leaked into practical V42 prose")
    ...
    for index, spec_row in enumerate(EXACT_WHOLE_SPECS, 1):
        ...
        decision_rows.append({
            "decision_id": f"G665-D{index:03d}", "surface": surface, "family": spec_row["family"],
            ...
            "strength": card_strength(surface), "status": "ACCEPT_V42_REPLACEABLE",
        })
    ...
    for locus in sorted(affected_loci):
        ...
        target_line_rows.append({
            ...
            "zl3b_line": final_row["zl3b_line"], "v41_token_glosses_de": base_row["token_glosses_de"],
            "v42_token_glosses_de": final_row["token_glosses_de"],
            "v42_working_translation_de": line_translation(
                locus, by_line[locus], split_pipe(final_row["token_glosses_de"]),
                y_occurrence_by_token, inherited_target_by_token, target_by_token,
            ),
            "v41_unknown_tokens": base_row["unknown_tokens"], "v42_unknown_tokens": final_row["unknown_tokens"],
            "v42_complete": int(int(final_row["unknown_tokens"]) == 0),
        })
    ...
    for row in frontier:
        ...
        frontier_rows.append({
            ...
            "zl3b_line": row["zl3b_line"], "v41_translation_de": row["proposed_complete_translation_de"],
            "v42_translation_de": line_translation(
                locus, by_line[locus], split_pipe(final_row["token_glosses_de"]),
                y_occurrence_by_token, inherited_target_by_token, target_by_token,
            ),
            "status": "COMPLETE_WITH_PROVISIONAL_CONCRETE_DEFAULT",
        })
    if len(frontier_rows) != 156 or any(f"[{row['surface']}:?]" in str(row["v42_translation_de"]) for row in frontier_rows):
        raise RuntimeError("a GDT665 frontier slot remained open in V42")
    ...
    for spec_row in MANUAL_PASSAGE_SPECS:
        ...
        if int(final_row["unknown_tokens"]) != 0:
            raise RuntimeError(f"manual passage is not V42-complete: {locus}")
        ...
        manual_passage_rows.append({
            **spec_row,
            "automatic_v42_translation_de": automatic,
            "v42_unknown_tokens": final_row["unknown_tokens"],
            "comparison_scope": "manual workshop prose versus deterministic V42 renderer; neither is confirmed plaintext",
        })
    ...
    expected_base = {
        "physical_lines": 4128, "known_token_positions": 21558, "unknown_token_positions": 10781,
        "complete_multi_token_lines": 613, "strict_complete_lines": 185,
        "one_unknown_lines": 332, "strict_one_unknown_lines": 72, "working_glossary_surfaces": 874,
    }
    if base_metrics != expected_base:
        raise RuntimeError(f"V41 base metrics drift: {base_metrics!r}")
    round_rows = [
        {"version": "V41", "added_cards": "BASE", "dictionary_entries": len(base_dictionary), **base_metrics},
        {"version": "V42", "added_cards": f"148_DEFAULTS+{len(context_cards)}_RENDERINGS+19_OL_REVISIONS+2_SOL_REVISIONS",
    ...
    write_tsv(output_dir / "FRONTIER_148_COMPLETIONS.tsv", frontier_rows, list(frontier_rows[0]))
    ...
    write_tsv(output_dir / "V42_WORKING_TOKEN_GLOSSARY.tsv", glossary_rows, list(base_glossary_rows[0]))
    write_tsv(output_dir / "WORKING_DICTIONARY_V42.tsv", dictionary_rows, list(base_dictionary[0]))
    write_tsv(output_dir / "ALL_LINE_CONCRETE_COVERAGE_V42.tsv", coverage_rows, coverage_fields)
    write_tsv(output_dir / "COMPLETE_PASSAGES_V42.tsv", complete_rows, complete_fields)
    write_tsv(output_dir / "ONE_UNKNOWN_PASSAGES_V42.tsv", one_rows, one_fields)
    write_tsv(
    ...
    write_tsv(
        ...
        (
            ...
            "automatic_v42_translation_de", "v42_unknown_tokens", "comparison_scope",
        ),
    )

    input_paths = (
        G664 / "REPORT.md", G664 / "artifacts/RESULT.json", G664 / "artifacts/PAGE_ALLOWLIST.tsv",
        G664 / "artifacts/V41_WORKING_TOKEN_GLOSSARY.tsv", G664 / "artifacts/WORKING_DICTIONARY_V41.tsv",
        G664 / "artifacts/ALL_LINE_CONCRETE_COVERAGE_V41.tsv", G664 / "artifacts/COMPLETE_PASSAGES_V41.tsv",
        G664 / "artifacts/ONE_UNKNOWN_PASSAGES_V41.tsv", G664 / "artifacts/NEWLY_EXPOSED_ONE_HOLE_LINES.tsv",
        G664 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        g664.G663 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        g664.g663.G662 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        g664.g663.g662.G661 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        g664.g663.g662.g661.G660 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        g664.g663.g662.g661.g660.G659 / "artifacts/Y_OCCURRENCE_CENSUS.tsv",
        ...
        BASE_REL / "src/STEM_MODEL_CANDIDATE.tsv",
    ...
    result_core: dict[str, object] = {
        "schema": "GDT665_ONE_HUNDRED_FORTY_EIGHT_RESIDUAL_FAMILY_COMPLETION_RESULT_V1",
        "experiment_id": "GDT665", "status": STATUS,
        "guard": {
        ...
        "manual_passages": {
            ...
            "v42_complete_rows": sum(int(row["v42_unknown_tokens"]) == 0 for row in manual_passage_rows),
        ...
        "working_dictionary": {
            "v41_entries": len(base_dictionary), "v42_entries": len(dictionary_rows),
            "added_default_entries": len(EXACT_WHOLE_SPECS), "added_rendering_entries": len(context_cards),
            "v41_glossary_surfaces": len(base_glossary_rows), "v42_glossary_surfaces": len(glossary_rows),
        },
        ...
        "claim_boundary": (
            "Exploratory replaceable concrete defaults for 148 V41 residual surfaces at 994 inherited positions. "
            "Productive compounds, learned actions and learned wholes coexist. Five boundary-sensitive forms use a local "
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.artifact_dir.resolve() == ART.resolve():
        with tempfile.TemporaryDirectory(prefix="gdt665_replay_") as directory:
    ...
    print(
        f"GDT665 built: targets={result['targets']['positions']} surfaces=148 "
