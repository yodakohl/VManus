...
def main():
    ...
    assert len(rows)==len(jobs)==118 and [x['id'] for x in rows]==[x['id'] for x in jobs]
    ...
    source=read(R/s['source_contexts']);inherited=read(A/'INHERITED.json')
    assert [p['id'] for p in panel[1:]]==[p['id'] for p in source]
    assert len(source)==120 and len(inherited)==2
    assert set(j['paragraph_ids'][1] for j in jobs).isdisjoint(x['paragraph'] for x in inherited)
    assert {j['paragraph_ids'][1] for j in jobs}|{x['paragraph'] for x in inherited}=={p['id'] for p in source}
    prior=read(R/s['source_previous_rows']);checks=read(R/s['source_previous_validation'])['checks']
    for x in inherited:
        r=next(q for q in prior if q['id']==x['row']);c=next(q for q in checks if q['id']==x['row'])
        assert r['family']=='FUNCTIONAL' and r['status']=='UNSAT' and c['independent']=='unsat' and r['paragraph_ids'][1]==x['paragraph']
    for p in panel[1:]:
