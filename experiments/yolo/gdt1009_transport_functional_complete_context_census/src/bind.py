...
if args.lock:
    ...
    science=['DECISION.md','METHOD.md','PREREGISTRATION.md','requirements.txt','src/SPEC.json','src/common.py','src/solver.py','src/independent.py','src/prepare.py','src/preflight.py','src/run.py','src/validate.py','artifacts/PANEL.json','artifacts/PREDICTIONS.json','artifacts/KNOWN_COUNTERCASE.json','artifacts/INHERITED.json','artifacts/PREFLIGHT.json']
...
m=read(E/'experiment.json');m.update(question='Can the alias-permitting FUNCTIONAL family extend any complete original reading into the118remaining GDT1007 contexts?',claim_ceiling='Complete shared grammar capacity and at most two semantic witness samples; no word confirmation, source identity or significance.',dependencies=['GDT993','GDT994','GDT1002','GDT1003','GDT1006','GDT1007','GDT1008'],status=args.status,inputs=[dict(path=x,role='fixed_input',sha256=sha(R/x)) for x in inputs_],outputs=[])
