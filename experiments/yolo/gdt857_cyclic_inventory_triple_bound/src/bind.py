...
m.update(title='Literal triple bound for nonsingleton cyclic codeword pools',question='Do exact raw prose triples contradict the fixed nonsingleton without-replacement cyclic-pool contract?'     ,claim_ceiling='Conditional deterministic cipher-class counterexample only; transcription-bound whole-group codeword assumption, not homophony generally.',status=status,dependencies=['GDT851'],inputs=[{'path':n,'sha256':sha(ROOT/n),'role':'frozen_source_or_extractor'} for n in up],outputs=[{'path':p.relative_to(ROOT).as_posix(),'sha256':sha(p),'role':'primary_report' if p.name=='REPORT.md' else 'source_or_artifact'} for p in outputs],validation={'status':'PASS' if result.exists() else 'NOT_RUN','artifact':rel+'/artifacts/VALIDATION.json' if result.exists() else None})
source_spec=json.loads((E/'src/SPEC.json').read_text())
for source in source_spec['sources']:m['inputs'].append({'path':source['path'],'sha256':source['sha256'],'role':'published_safe_cache_hash_checked_on_GO'})
