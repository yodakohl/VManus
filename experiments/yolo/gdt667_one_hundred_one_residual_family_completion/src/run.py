#!/usr/bin/env python3
"""Build GDT667: close the 101-form V43 frontier as concrete V44 recipe cards."""
...
BASE_REL = Path("experiments/yolo/gdt667_one_hundred_one_residual_family_completion")
ART = ROOT / BASE_REL / "artifacts"
G666 = Path("experiments/yolo/gdt666_one_hundred_fifty_one_residual_family_completion")
_spec = importlib.util.spec_from_file_location("gdt666_builder_for_gdt667", ROOT / G666 / "src/run.py")
if _spec is None or _spec.loader is None:
    raise RuntimeError("cannot load GDT666 builder")
g666 = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(g666)
TOKENS_REL, CROSS_REL = g666.TOKENS_REL, g666.CROSS_REL
STATUS = "PASS_438_TARGET_POSITIONS__V44_CONCRETE_RECIPE_REGISTER"
...
def read_card_specs(path: Path) -> tuple[dict[str, str], ...]:
    ...
    if not rows or set(rows[0]) != expected:
        raise RuntimeError("bad GDT667 CARD_SPECS.tsv columns")
    surfaces = [row["surface"] for row in rows]
    if len(rows) != 101 or len(set(surfaces)) != 101:
        raise RuntimeError("GDT667 CARD_SPECS.tsv must contain 101 unique surfaces")
    return tuple(rows)
...
OUTPUT_NAMES = (
    ...
    "FAMILY_COMPOSITION_ATLAS.tsv", "FRONTIER_101_COMPLETIONS.tsv",
    ...
    "V44_WORKING_TOKEN_GLOSSARY.tsv", "WORKING_DICTIONARY_V44.tsv",
    "ALL_LINE_CONCRETE_COVERAGE_V44.tsv", "COMPLETE_PASSAGES_V44.tsv",
    "ONE_UNKNOWN_PASSAGES_V44.tsv", "INHERITED_OL_RENDER_REVISIONS.tsv",
    "INHERITED_SOL_RENDER_REVISIONS.tsv", "STEM_MODEL_V44.tsv", "MANUAL_PASSAGE_AUDIT.tsv",
)
...
# These are practical render revisions only.  V43's structural tags remain
# byte-visible in its inherited glossary/dictionary; V44 translates the 19
...
def read_ol_revision_specs(path: Path) -> tuple[dict[str, str], ...]:
    ...
    if not rows or set(rows[0]) != expected:
        raise RuntimeError("bad GDT667 INHERITED_OL_REVISION_SPECS.tsv columns")
    surfaces = [row["surface"] for row in rows]
    if len(rows) != 19 or len(set(surfaces)) != 19 or sum(int(row["positions"]) for row in rows) != 256:
        raise RuntimeError("GDT667 OL revision source must contain 19 unique forms at 256 positions")
...
def read_sol_revision_specs(path: Path) -> tuple[dict[str, str], ...]:
    ...
    expected = {"surface", "positions", "practical_meaning_de", "composition", "retained_rival_de"}
    if not rows or set(rows[0]) != expected:
        raise RuntimeError("bad GDT667 INHERITED_SOL_REVISION_SPECS.tsv columns")
    surfaces = [row["surface"] for row in rows]
    if len(rows) != 2 or len(set(surfaces)) != 2 or sum(int(row["positions"]) for row in rows) != 4:
        raise RuntimeError("GDT667 SOL revision source must contain 2 unique forms at 4 positions")
    return tuple(rows)
...
# all token gloss markers at this locus.  V44 preserves that sentence but must
...
def read_stem_model_specs(path: Path) -> tuple[dict[str, str], ...]:
    ...
    if not rows or set(rows[0]) != expected or len(rows) != 52:
        raise RuntimeError("GDT667 STEM_MODEL_SPECS.tsv must contain the 52-row V44 schema")
    return tuple(rows)
...
def read_manual_passage_specs(path: Path) -> tuple[dict[str, str], ...]:
    ...
    if not rows or set(rows[0]) != expected or len(rows) != 25:
        raise RuntimeError("GDT667 MANUAL_PASSAGE_SPECS.tsv must contain the fixed 25-row schema")
    if [int(row["rank"]) for row in rows] != list(range(1, 26)):
        raise RuntimeError("GDT667 manual passage ranks must be 1..25")
    if len({row["locus"] for row in rows}) != 25:
        raise RuntimeError("GDT667 manual passage loci must be unique")
    return tuple(rows)
...
def line_translation(
    ...
    """Render V44 with reader joins over the complete V43 renderer."""
    working_glosses = list(glosses)
    # GDT666's renderer only applies its positional action cards to the map it
    # receives as the current layer.  Re-promote the archived G666 rows here;
    ...
    inherited = {
        ...
        if not str(row.get("occurrence_id", "")).startswith("G666-")
    }
    current = {
        key: row for key, row in inherited_target_by_token.items()
        if str(row.get("occurrence_id", "")).startswith("G666-")
    }
    ...
    rendered = g666.line_translation(
...
def build(output_dir: Path) -> dict[str, object]:
    ...
    tokens, token_stats = g666.g665.g664.g663.g662.g661.g659.guarded_query(
        TOKENS_REL, pages, "page,locus,token_index,eva,kind,section,language,hand"
    )
    cross, cross_stats = g666.g665.g664.g663.g662.g661.g659.guarded_query(
        CROSS_REL, pages,
    ...
    base_glossary_rows = read_tsv(base_art / "V43_WORKING_TOKEN_GLOSSARY.tsv")
    ...
    if dimensions != (1769, 1173, 4128, 968, 251, 102):
        raise RuntimeError(f"V43 base dimensions drift: {dimensions!r}")
    frontier_order = tuple(dict.fromkeys(row["unknown_surface"] for row in frontier))
    if frontier_order != TARGET_ORDER or len(TARGET_SURFACES) != 101:
        raise RuntimeError("the 101-form frontier or fixed order drifted")
    ...
    if any(surface in base_glossary for surface in TARGET_SURFACES):
        raise RuntimeError("a GDT667 target unexpectedly already has a V43 glossary row")
    ...
    y_occurrences = read_tsv(
        ROOT / g666.g665.g664.g663.g662.g661.g660.G659 / "artifacts/Y_OCCURRENCE_CENSUS.tsv"
    )
    ...
    inherited_target_by_token: dict[tuple[str, int], dict[str, object]] = {}
    inherited_audits = (
        ROOT / g666.g665.g664.g663.g662.g661.G660 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        ROOT / g666.g665.g664.g663.g662.G661 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        ROOT / g666.g665.g664.g663.G662 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        ROOT / g666.g665.g664.G663 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        ROOT / g666.g665.G664 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        ROOT / g666.G665 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
    ...
    if observed_counts != EXPECTED_SURFACE_COUNTS or sum(observed_counts.values()) != 438:
        raise RuntimeError(f"target surface count drift: {observed_counts!r}")
    exact, normalized = g666.g665.g664.g663.g662.g661.g660.stable_maps(tokens, cross_by_locus)
    ...
    for locus in sorted(by_line):
        ...
        for index, token in enumerate(line):
            ...
            item: dict[str, object] = {
                "occurrence_id": f"G667-T{len(occurrence_rows) + 1:04d}",
    ...
    if len(occurrence_rows) != 438 or len(target_by_token) != 438:
    ...
    for base_row in base_coverage:
        ...
        if not (len(line) == len(glosses) == len(sources) == len(states)):
            raise RuntimeError(f"V43 token columns misalign: {locus}")
        ...
        for index, token in enumerate(line):
            ...
            if glosses[index] != f"[{surface}:?]" or sources[index] != "OPEN" or states[index] != "UNKNOWN_SURFACE":
                raise RuntimeError(f"V43 target not open at {locus}.{index + 1}: {surface}")
            glosses[index] = str(occurrence["working_gloss_de"])
            if surface in CONTEXT_SCOPED_SURFACES:
                sources[index] = f"GDT667:{occurrence['rendering_class']}:{occurrence['reader_merge_surface']}"
                states[index] = "KNOWN_CONTEXT_LICENSED" if int(occurrence["reader_exact"]) else "READER_BOUNDARY_UNSTABLE"
            else:
                sources[index] = f"GDT667:EXACT_WHOLE:{surface}"
        ...
        if int(base_row["unknown_tokens"]) - len(target_ordinals) != len(unknown_pairs):
            raise RuntimeError(f"V43->V44 arithmetic drift: {locus}")
    ...
    for row in coverage_rows:
        ...
        if old and old["unknown_surface"] == surface and int(old["unknown_ordinal"]) == ordinal:
            ...
            proposal, basis, strength = f"[{surface}:?]", "NEWLY_EXPOSED_BY_GDT667_NO_NEW_CARD", "OPEN"
        strict = int(
            int(row["ambiguous_tokens"]) == 0 and int(row["reader_unstable_tokens"]) == 0
            and int(row["all_present_exact"]) == 1
        )
    ...
    for spec_row in EXACT_WHOLE_SPECS:
        ...
        glossary_rows.append({
            ...
            "source": "GDT667:CONTEXT_SHORT_DEFAULT" if surface in CONTEXT_SCOPED_SURFACES else "GDT667:EXACT_WHOLE",
        ...
        dictionary_rows.append({
            "entry": f"{surface}@GDT667_DEFAULT", "kind": card_type(surface),
            ...
            "status": "NEW_V44_PROVISIONAL_CONCRETE_RECIPE_DEFAULT",
        })
    glossary_rows.sort(key=lambda row: str(row["surface"]))
    if len(glossary_rows) != 1274:
        raise RuntimeError("V44 glossary dimension drift")
    ...
    for klass, surface, merge_surface, render in context_keys:
        ...
        context_cards.append({
            "card_id": f"G667-C{len(context_cards) + 1:03d}", "rendering_class": klass,
    ...
    next_frontier_counts = [
        {
            ...
            "status": "NEXT_V44_ONE_HOLE_FRONTIER",
        }
    ...
    for rank, row in enumerate(next_frontier_counts, 1):
        row["rank"] = rank
    if len(next_frontier_counts) != 76:
        raise RuntimeError("V44 next-frontier surface count drift")
    ...
    for occurrence in occurrence_rows:
        ...
        audit_rows.append({
            **occurrence,
            "v43_gloss_de": split_pipe(base_row["token_glosses_de"])[ordinal - 1],
            "v44_gloss_de": split_pipe(final_row["token_glosses_de"])[ordinal - 1],
            "v43_scope_state": split_pipe(base_row["scope_states"])[ordinal - 1],
            "v44_scope_state": split_pipe(final_row["scope_states"])[ordinal - 1],
            "v44_working_translation_de": line_translation(
    ...
    if any(str(row["v43_gloss_de"]) != f"[{row['surface']}:?]" for row in audit_rows):
        raise RuntimeError("not every target occurrence was open in V43")
    if any(GENERIC_FILLER.search(str(row["v44_working_translation_de"])) for row in audit_rows):
        raise RuntimeError("generic work filler leaked into GDT667")
    if any("Eigenschafts-/Zustands-/Materialträger" in str(row["v44_working_translation_de"]) for row in audit_rows):
        raise RuntimeError("structural OL meta-gloss leaked into practical V44 prose")
    ...
    for index, spec_row in enumerate(EXACT_WHOLE_SPECS, 1):
        ...
        decision_rows.append({
            "decision_id": f"G667-D{index:03d}", "surface": surface, "family": spec_row["family"],
            ...
            "strength": card_strength(surface), "status": "ACCEPT_V44_REPLACEABLE",
        })
    ...
    for locus in sorted(affected_loci):
        ...
        target_line_rows.append({
            ...
            "zl3b_line": final_row["zl3b_line"], "v43_token_glosses_de": base_row["token_glosses_de"],
            "v44_token_glosses_de": final_row["token_glosses_de"],
            "v44_working_translation_de": line_translation(
                locus, by_line[locus], split_pipe(final_row["token_glosses_de"]),
                y_occurrence_by_token, inherited_target_by_token, target_by_token,
            ),
            "v43_unknown_tokens": base_row["unknown_tokens"], "v44_unknown_tokens": final_row["unknown_tokens"],
            "v44_complete": int(int(final_row["unknown_tokens"]) == 0),
        })
    ...
    for row in frontier:
        ...
        frontier_rows.append({
            ...
            "zl3b_line": row["zl3b_line"], "v43_translation_de": row["proposed_complete_translation_de"],
            "v44_translation_de": line_translation(
                locus, by_line[locus], split_pipe(final_row["token_glosses_de"]),
                y_occurrence_by_token, inherited_target_by_token, target_by_token,
            ),
            "status": "COMPLETE_WITH_PROVISIONAL_CONCRETE_DEFAULT",
        })
    if len(frontier_rows) != 102 or any(f"[{row['surface']}:?]" in str(row["v44_translation_de"]) for row in frontier_rows):
        raise RuntimeError("a GDT667 frontier slot remained open in V44")
    ...
    for spec_row in MANUAL_PASSAGE_SPECS:
        ...
        if int(final_row["unknown_tokens"]) != 0:
            raise RuntimeError(f"manual passage is not V44-complete: {locus}")
        ...
        manual_passage_rows.append({
            **spec_row,
            "automatic_v44_translation_de": automatic,
            "v44_unknown_tokens": final_row["unknown_tokens"],
            "comparison_scope": "manual workshop prose versus deterministic V44 renderer; neither is confirmed plaintext",
        })
    ...
    expected_base = {
        "physical_lines": 4128, "known_token_positions": 23164, "unknown_token_positions": 9175,
        "complete_multi_token_lines": 968, "strict_complete_lines": 257,
        "one_unknown_lines": 251, "strict_one_unknown_lines": 59, "working_glossary_surfaces": 1173,
    }
    if base_metrics != expected_base:
        raise RuntimeError(f"V43 base metrics drift: {base_metrics!r}")
    round_rows = [
        {"version": "V43", "added_cards": "BASE", "dictionary_entries": len(base_dictionary), **base_metrics},
        {"version": "V44", "added_cards": f"101_DEFAULTS+{len(context_cards)}_RENDERINGS+19_OL_REVISIONS+2_SOL_REVISIONS",
    ...
    write_tsv(output_dir / "FRONTIER_101_COMPLETIONS.tsv", frontier_rows, list(frontier_rows[0]))
    ...
    write_tsv(output_dir / "V44_WORKING_TOKEN_GLOSSARY.tsv", glossary_rows, list(base_glossary_rows[0]))
    write_tsv(output_dir / "WORKING_DICTIONARY_V44.tsv", dictionary_rows, list(base_dictionary[0]))
    write_tsv(output_dir / "ALL_LINE_CONCRETE_COVERAGE_V44.tsv", coverage_rows, coverage_fields)
    write_tsv(output_dir / "COMPLETE_PASSAGES_V44.tsv", complete_rows, complete_fields)
    write_tsv(output_dir / "ONE_UNKNOWN_PASSAGES_V44.tsv", one_rows, one_fields)
    write_tsv(
    ...
    write_tsv(
        ...
        (
            ...
            "automatic_v44_translation_de", "v44_unknown_tokens", "comparison_scope",
        ),
    )

    input_paths = (
        G666 / "REPORT.md", G666 / "artifacts/RESULT.json", G666 / "artifacts/PAGE_ALLOWLIST.tsv",
        G666 / "artifacts/V43_WORKING_TOKEN_GLOSSARY.tsv", G666 / "artifacts/WORKING_DICTIONARY_V43.tsv",
        G666 / "artifacts/ALL_LINE_CONCRETE_COVERAGE_V43.tsv", G666 / "artifacts/COMPLETE_PASSAGES_V43.tsv",
        G666 / "artifacts/ONE_UNKNOWN_PASSAGES_V43.tsv", G666 / "artifacts/NEWLY_EXPOSED_ONE_HOLE_LINES.tsv",
        G666 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        g666.G665 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        g666.g665.G664 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        g666.g665.g664.G663 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        g666.g665.g664.g663.G662 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        g666.g665.g664.g663.g662.G661 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        g666.g665.g664.g663.g662.g661.G660 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        g666.g665.g664.g663.g662.g661.g660.G659 / "artifacts/Y_OCCURRENCE_CENSUS.tsv",
    ...
    result_core: dict[str, object] = {
        "schema": "GDT667_ONE_HUNDRED_ONE_RESIDUAL_FAMILY_COMPLETION_RESULT_V1",
        "experiment_id": "GDT667", "status": STATUS,
        "guard": {
        ...
        "card_synthesis": {
            ...
            "inherited_stem_roles_reused": 48,
            "total_stem_roles": len(STEM_MODEL_SPECS),
            "new_stem_roles": len(STEM_MODEL_SPECS) - 48,
            "new_stem_role": "I_FORM_I|SO_SEED_PREP|OM_HANDFUL_PREP|CPH_COMPOSITE",
            "historical_analogue": (
        ...
        "manual_passages": {
            ...
            "v44_complete_rows": sum(int(row["v44_unknown_tokens"]) == 0 for row in manual_passage_rows),
        ...
        "working_dictionary": {
            "v43_entries": len(base_dictionary), "v44_entries": len(dictionary_rows),
            "added_default_entries": len(EXACT_WHOLE_SPECS), "added_rendering_entries": len(context_cards),
            "v43_glossary_surfaces": len(base_glossary_rows), "v44_glossary_surfaces": len(glossary_rows),
        },
        "frontier": {
        ...
        "claim_boundary": (
            "Exploratory replaceable concrete defaults for 101 V43 residual surfaces at 438 inherited positions. "
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        f"GDT667 built: targets={result['targets']['positions']} surfaces=101 "
