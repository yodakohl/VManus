...
def independent_job(args):
    ...
    for r in [primary,other]:
        if r['status']=='sat':
            ...
            assert not saturation_errors(w['aliases'],case['groups'])
...
def lift_job(j):
    ...
    assert not contradictions(code,relations_independent(code))
    assert not saturation_errors(code,relations_independent(code))
...
def main():
    checklock();s,g=inputs();panel=read(A/'PANEL.json');cases=read(A/'PREDICTIONS.json');primary=read(A/'ROWS.json');originals=read(A/'ORIGINAL_CANDIDATES.json');assert panel==read(R/s['source_panel']) and originals==read(R/s['source_candidates'])
    ...
    assert len(cases)==52 and len(primary)==52 and [r['id'] for r in primary]==[r['id'] for r in cases]
    for case in cases:
        assert case['variant']==settings[case['variant_index']]
        for m in case['members']:
            row=original[m['original_id']]
            matches=[dict(zip(('C','G','W'),v)) for v in itertools.permutations(('C','G','W')) if {w:dict(zip(('C','G','W'),v)).get(x,x) for w,x in row['code'].items()}==case['canonical_lexicon']]
            assert matches==[m['original_to_canonical']] and case['variant_index'] in row['valid_variants']
            membership.append((row['id'],case['context_index'],case['variant_index']))
        assert case['groups']==relations_independent(set(case['canonical_lexicon'])|set(panel[case['context_index']]['words']))
    ...
    with (A/'CANONICAL_RESULTS.tsv').open('w',newline='') as f:
        ...
        for c,p,x in zip(cases,primary,checks):w.writerow([c['id'],c['context_index'],c['family'],c['variant_index'],len(c['members']),p['status'],x['independent']['status'],x['status'],0])
    candidate_rows=[]
