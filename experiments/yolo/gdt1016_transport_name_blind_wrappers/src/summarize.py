...
def main():
    ...
    for case,p,ind in zip(cases,ps,inds):
        for engine,r in [('primary',p),('independent',ind['independent'])]:
            ...
            docs.append('')
    if not summaries:
        put('WITNESS_SUMMARY.json',dict(saved_verified_witnesses=0,primary_counts=dict(collections.Counter(r['status'] for r in ps)),independent_counts=dict(collections.Counter(r['independent']['status'] for r in inds)),confirmed_words=0,independent_meaning_capacity=0))
        (A/'READINGS.md').write_text('# No verified positive full reading under this registered law\n\nAll original and unconstrained1013/1014readings remain in their original artifacts.\n')
        print('Zero positive witnesses;all case outcomes retained.')
        return
