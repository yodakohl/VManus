...
def main():
 ...
 checks={'freeze':f['status']=='FROZEN_BEFORE_VOYNICH_SCORE','endpoint':f['endpoint']=='PREDICATE_HEAD_WITH_DEPENDENTS','events':len(e)==8448,'tuples':len(a)==1676,'no_f84':not any(x['page'].startswith('f84') or x['locus'].startswith('f84') for x in e),'scores':all(0<=float(x['nuisance_probability'])<=1 and 0<=float(x['cmp_local_head_signature_probability'])<=1 for x in e),'unassigned':all(x['semantic_state']=='UNASSIGNED' for x in e+a),'candidate_count':len(passed)==r['candidate_tuples'],'candidate_gate':all(int(x['events'])>=12 and int(x['physical_folios'])>=3 and float(x['mean_signature_probability'])>=.5 and float(x['folio_fraction_mean_ge_0_5'])>=.75 and float(x['mean_structure_minus_nuisance'])>=0 for x in passed),'null':len(n)==1 and int(n[0]['null_worlds'])==4096,'f84_flags':not any(r['f84'].values()),'hashes':all(sha(ROOT/p)==v for p,v in r['outputs'].items())}
