#!/usr/bin/env python3
"""Compile a surface through the GDT523 path-local null license."""
...
def main() -> int:
    ...
    null_context = core.train_null_context_model(old_forms)
    g522_missing, g522_weight = core.gdt522_selected_config()
    _, selected_mode, selected_weight = next(
    ...
    for base_index, candidate in enumerate(candidates):
        ...
        gdt521_score = (
            gdt520_score
            + core.G521.SELECTED_WEIGHT * recipe_model.mean_nll(candidate.recipe)
        )
        ...
        path_values, path_null_trace = core.path_features(
            surface, path, analogy, null_context
        )
        null_feature = path_values[selected_mode]
        scored.append(
            {
                "compiler_rank": base_index + 1,
                "recipe": core.recipe_text(candidate.recipe),
                "gdt522_score": gdt522_score,
                "path_null_feature": null_feature,
                "gdt523_score": gdt522_score - selected_weight * null_feature,
    ...
    scored.sort(key=lambda row: (float(row["gdt523_score"]), int(row["compiler_rank"])))
    ...
    result = {
        ...
        "selected_path_null_stage": core.SELECTED_STAGE,
        ...
        "selection_precedence": "EXACT_EVENT>KNOWN_SURFACE_ROLE_OPTION>GDT523_PATH_NULL_TOP1",
        "default_selection": default,
        "guard": "LIGHT_PATH_NULL_LICENSE_DEFAULT__NO_CONFIRMED_LEXEME_OR_PLAINTEXT",
    }
