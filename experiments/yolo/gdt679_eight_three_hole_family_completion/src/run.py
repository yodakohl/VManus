#!/usr/bin/env python3
"""Build the GDT679 24-card occurrence circuit and V53 reader."""
...
GDT678_RESULT_PATH = ROOT / "experiments/yolo/gdt678_seventeen_two_hole_family_completion/artifacts/RESULT.json"
...
def build(output_dir: Path) -> dict[str, object]:
    ...
    gdt678_result = json.loads(GDT678_RESULT_PATH.read_text(encoding="utf-8"))
    v52_lines = read_tsv(V52_PATH)

    assert len(cards) == len({row["surface"] for row in cards}) == 24
    assert len(source_specs) == len({row["locus"] for row in source_specs}) == 8
    assert len(boundary_specs) == 13
    assert len({(row["locus"], int(row["ordinal"])) for row in boundary_specs}) == 13
    assert len(analog_specs) == 7
    assert len(panel) == 4128
    assert len(v52_lines) == 51
    assert all(not row["page"].lower().startswith("f84") for row in panel)
    assert gdt678_result["global_overlay"]["unknown_positions_after"] == 7822
    assert gdt678_result["global_overlay"]["complete_lines_after"] == 1410
    assert gdt678_result["v52_reader"]["unknown_after"] == 93
    ...
    v52_by_locus = {row["locus"]: row for row in v52_lines}
    assert set(source_by_locus).issubset(v52_by_locus)
    ...
    assert len(current_unknown) == 7822
    ...
    v53_rows: list[dict[str, object]] = []
    distribution: Counter[int] = Counter()
    for old in v52_lines:
        ...
        if completed:
            ...
            row: dict[str, object] = {
                ...
                "v53_target_surfaces": completed["closed_surfaces"],
            ...
            practical = old["practical_translation_de"]
            if old["locus"] == "f105r.2":
                practical = practical.replace(
                    "als getrockneten Ansatz weiterführen",
                    "als getrockneten Ansatz halten",
                )
            if old["locus"] == "f77v.7":
                practical = practical.replace(
                    "Als heißen Mittelstufenansatz mit eingeweichtem Drogenholz weiterführen",
                    "Den heißen Mittelstufenansatz mit eingeweichtem Drogenholz mischen",
                )
            row = {
                "page": old["page"],
                "locus": old["locus"],
                ...
                "practical_translation_de": practical,
    ...
    assert sum(int(row["token_count"]) for row in v53_rows) == 479
    assert sum(int(row["new_v53_positions"]) for row in v53_rows) == 24
    assert sum(int(row["residual_unknown_positions"]) for row in v53_rows) == 69
    assert sum(row["complete"] == "1" for row in v53_rows) == 36
    assert distribution == {0: 36, 4: 8, 5: 6, 7: 1}

    after_unknown = current_unknown - target_keys
    assert len(after_unknown) == 7765
    ...
    assert complete_before == 1410
    assert complete_after == 1418
    assert set(newly_closed_loci) == set(source_by_locus)
        ...
        global_closed_rows.append(
            {
                ...
                "practical_translation_de": source_by_locus[locus]["practical_translation_de"],
            }
        )

    prediction_rows = [
        {"prediction_id": "GDT679-P01", "observed_family": "oteos|qoteos|qoteeos", "visible_difference": "initial q and one additional e", "predicted_semantic_difference": "middle cold charge; take it; take the fully cooled charge", "held_positions": "2/2 target-family positions", "next_unseen_test": "another qoteos must retain charge selection and middle cooling"},
        {"prediction_id": "GDT679-P02", "observed_family": "process+qeeey", "visible_difference": "qeeey follows moist, cold or dry middle-state cards", "predicted_semantic_difference": "carry the immediately preceding process to its last stage and close it", "held_positions": "3/3 exact qeeey positions", "next_unseen_test": "a new qeeey should follow a named process and preserve that process identity"},
        {"prediction_id": "GDT679-P03", "observed_family": "saraiin|cthororaiin", "visible_difference": "sar fraction head versus cthor-or portion head", "predicted_semantic_difference": "three parts seed fraction I versus three herb portions", "held_positions": "4/4", "next_unseen_test": "X-ar-aiin should preserve first fraction while X-or-aiin preserves portion"},
        {"prediction_id": "GDT679-P04", "observed_family": "yteeeor|ycheeodaiin", "visible_difference": "process and internal amount after y", "predicted_semantic_difference": "anaphoric cooling of one portion versus measuring three dry-preparation doses", "held_positions": "2/2", "next_unseen_test": "initial y must preserve the following process and amount rather than erase them"},
        {"prediction_id": "GDT679-P05", "observed_family": "kcho|kchod and keeo|keeod", "visible_difference": "terminal d", "predicted_semantic_difference": "close or draw off the immediately preceding preparation block", "held_positions": "5/5 target d-results", "next_unseen_test": "another complete Xod block should close X, not introduce a substance"},
        {"prediction_id": "GDT679-P06", "observed_family": "osheey|orsheey", "visible_difference": "o preparation head versus or portion head", "predicted_semantic_difference": "fully soaked preparation versus one portion fully soaked drug", "held_positions": "2/2", "next_unseen_test": "another exact oX/orX pair should preserve X and change preparation status to portion"},
        {"prediction_id": "GDT679-P07", "observed_family": "olkair|olkaiir and cholkaiin|cholkain", "visible_difference": "fraction-II/III and grade-III/II endings", "predicted_semantic_difference": "third hot wood fraction and hot dry material grade II", "held_positions": "2/2 target positions", "next_unseen_test": "the terminal ladder must change only fraction or grade, not the material head"},
        {"prediction_id": "GDT679-P08", "observed_family": "kc/kchs and daiiiry/daiiirchy", "visible_difference": "both alternate readers restore h-s or ch", "predicted_semantic_difference": "hot-dry species and measured fourth dry fraction", "held_positions": "2/2 bilateral repairs", "next_unseen_test": "a new short ZL3b form may be repaired only when both alternate readers agree on the same concrete family"},
    ]
    ...
    v53_fields = [
        ...
        "new_v51_positions", "new_v52_positions", "new_v53_positions", "residual_unknown_positions",
        "assigned_fraction", "complete", "action_positions", "action_ordinals", "action_surfaces",
        "line_mode", "v53_target_surfaces", "remaining_unknown_surfaces", "zl3b_line",
    ...
    write_tsv(output_dir / "EIGHT_COMPLETED_LINES_V53.tsv", completed_rows, completed_fields)
    write_tsv(output_dir / "V53_51_LINE_READER.tsv", v53_rows, v53_fields)
    ...
    reader_doc = [
        "# GDT679 — eight completed V53 practical readings",
        "",
    ...
    (output_dir / "GDT679_EIGHT_COMPLETED_PRACTICAL_READER.md").write_text(
        "\n".join(reader_doc).rstrip() + "\n", encoding="utf-8"
    )

    action_before = sum(int(row["action_positions"]) for row in v52_lines)
    action_after = sum(int(row["action_positions"]) for row in v53_rows)
    mode_counts = Counter(str(row["line_mode"]) for row in v53_rows)
    assert action_before == 60
    assert action_after == 65
    assert mode_counts == {"ACTION_SEQUENCE": 13, "MIXED_RECORD": 21, "NOMINAL_REGISTER": 10, "QUANTITY_LABEL": 7}

    result: dict[str, object] = {
        "status": "PASS_24_FAMILY_CARDS__57_CONTEXTS_HOLD__8_V53_LINES_CLOSED__V53_69_OPEN",
        "basis": {
            "panel_lines": 4128,
        ...
        "boundary_decisions": {
            ...
            "bilateral_reader_repairs": ["f8r.15 kc/kchs", "f8r.15 daiiiry/daiiirchy"],
            "lowest_unit_rival": "f27r.9 tolg/tolm/chotolg",
        },
        "global_overlay": {
        ...
        "v53_reader": {
            ...
            "assigned_before": 386,
            "assigned_after": 410,
            "assigned_fraction_after": f"{410 / 479:.6f}",
            "complete_before": 28,
            "complete_after": 36,
        ...
        "weakest_cards": [
            {"surface": "tolg", "reason": "ZL3b/RF1b grain ending competes with IT2a tolm handful and a joined RF1b form"},
            {"surface": "saraiin", "reason": "two seed-family contexts hold, but f87r.9 has an RF1b root-material rival"},
            {"surface": "qey", "reason": "one occurrence and its concrete object is supplied by preceding orchey"},
        ],
        "claim_ceiling": (
            "Twenty-four replaceable working cards apply with one practical core to all 57 exact ZL3b occurrences "
            "on the already admitted panel. They close the eight selected V52 three-hole lines; ckhol supplies a "
            "17-position composite-drug material head, and bilateral reader repairs control kc and daiiiry. This is "
            "a concrete mixed learned-whole and productive-component working renderer, not confirmed plaintext, "
            "phonetics, a historical codebook identity, exact plant species, disease, patient, cure, carrier liquid "
            "or manuscript-wide translation. The tolg unit and all named rivals remain replaceable."
        ),
        "files": {},
    }
    artifact_names = [
