#!/usr/bin/env python3
"""Build GDT671: close the 15-form V47 frontier as concrete V48 recipe cards."""
...
BASE_REL = Path("experiments/yolo/gdt671_fifteen_residual_family_completion")
ART = ROOT / BASE_REL / "artifacts"
G670 = Path("experiments/yolo/gdt670_twenty_eight_residual_family_completion")
_spec = importlib.util.spec_from_file_location("gdt670_builder_for_gdt671", ROOT / G670 / "src/run.py")
if _spec is None or _spec.loader is None:
    raise RuntimeError("cannot load GDT668 builder")
g670 = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(g670)
TOKENS_REL, CROSS_REL = g670.TOKENS_REL, g670.CROSS_REL
STATUS = "PASS_71_TARGET_POSITIONS__V48_CONCRETE_RECIPE_REGISTER"
...
def read_card_specs(path: Path) -> tuple[dict[str, str], ...]:
    ...
    if not rows or set(rows[0]) != expected:
        raise RuntimeError("bad GDT671 CARD_SPECS.tsv columns")
    surfaces = [row["surface"] for row in rows]
    if len(rows) != 15 or len(set(surfaces)) != 15:
        raise RuntimeError("GDT671 CARD_SPECS.tsv must contain 15 unique surfaces")
    return tuple(rows)
...
CONTEXT_SCOPED_SURFACES: frozenset[str] = frozenset()
...
OUTPUT_NAMES = (
    ...
    "FAMILY_COMPOSITION_ATLAS.tsv", "FRONTIER_15_COMPLETIONS.tsv",
    ...
    "V48_WORKING_TOKEN_GLOSSARY.tsv", "WORKING_DICTIONARY_V48.tsv",
    "ALL_LINE_CONCRETE_COVERAGE_V48.tsv", "COMPLETE_PASSAGES_V48.tsv",
    "ONE_UNKNOWN_PASSAGES_V48.tsv", "INHERITED_OL_RENDER_REVISIONS.tsv",
    "INHERITED_SOL_RENDER_REVISIONS.tsv", "STEM_MODEL_V48.tsv", "MANUAL_PASSAGE_AUDIT.tsv",
)
...
# These are practical render revisions only.  V47's structural tags remain
# byte-visible in its inherited glossary/dictionary; V48 translates the 19
...
def read_ol_revision_specs(path: Path) -> tuple[dict[str, str], ...]:
    ...
    if not rows or set(rows[0]) != expected:
        raise RuntimeError("bad GDT671 INHERITED_OL_REVISION_SPECS.tsv columns")
    surfaces = [row["surface"] for row in rows]
    if len(rows) != 19 or len(set(surfaces)) != 19 or sum(int(row["positions"]) for row in rows) != 256:
        raise RuntimeError("GDT671 OL revision source must contain 19 unique forms at 256 positions")
...
def read_sol_revision_specs(path: Path) -> tuple[dict[str, str], ...]:
    ...
    expected = {"surface", "positions", "practical_meaning_de", "composition", "retained_rival_de"}
    if not rows or set(rows[0]) != expected:
        raise RuntimeError("bad GDT671 INHERITED_SOL_REVISION_SPECS.tsv columns")
    surfaces = [row["surface"] for row in rows]
    if len(rows) != 2 or len(set(surfaces)) != 2 or sum(int(row["positions"]) for row in rows) != 4:
        raise RuntimeError("GDT671 SOL revision source must contain 2 unique forms at 4 positions")
    return tuple(rows)
...
# all token gloss markers at this locus.  V48 preserves that sentence but must
...
def read_stem_model_specs(path: Path) -> tuple[dict[str, str], ...]:
    ...
    if not rows or set(rows[0]) != expected or len(rows) != 56:
        raise RuntimeError("GDT671 STEM_MODEL_SPECS.tsv must contain the 56-row V48 schema")
    return tuple(rows)
...
def read_manual_passage_specs(path: Path) -> tuple[dict[str, str], ...]:
    ...
    if not rows or set(rows[0]) != expected or len(rows) != 20:
        raise RuntimeError("GDT671 MANUAL_PASSAGE_SPECS.tsv must contain the fixed 20-row schema")
    if [int(row["rank"]) for row in rows] != list(range(1, 21)):
        raise RuntimeError("GDT671 manual passage ranks must be 1..20")
    if len({row["locus"] for row in rows}) != 20:
        raise RuntimeError("GDT671 manual passage loci must be unique")
    return tuple(rows)
...
def line_translation(
    ...
    """Render V48 with reader joins over the complete V47 renderer."""
    working_glosses = list(glosses)
    # GDT666's renderer only applies its positional action cards to the map it
    # receives as the current layer.  Re-promote the archived G670 rows here;
    ...
    inherited = {
        ...
        if not str(row.get("occurrence_id", "")).startswith("G670-")
    }
    current = {
        key: row for key, row in inherited_target_by_token.items()
        if str(row.get("occurrence_id", "")).startswith("G670-")
    }
    ...
    rendered = g670.line_translation(
...
def build(output_dir: Path) -> dict[str, object]:
    ...
    tokens, token_stats = g670.g669.g668.g667.g666.g665.g664.g663.g662.g661.g659.guarded_query(
        TOKENS_REL, pages, "page,locus,token_index,eva,kind,section,language,hand"
    )
    cross, cross_stats = g670.g669.g668.g667.g666.g665.g664.g663.g662.g661.g659.guarded_query(
        CROSS_REL, pages,
    ...
    base_glossary_rows = read_tsv(base_art / "V47_WORKING_TOKEN_GLOSSARY.tsv")
    ...
    if dimensions != (2100, 1415, 4128, 1223, 154, 15):
        raise RuntimeError(f"V47 base dimensions drift: {dimensions!r}")
    frontier_order = tuple(dict.fromkeys(row["unknown_surface"] for row in frontier))
    if frontier_order != TARGET_ORDER or len(TARGET_SURFACES) != 15:
        raise RuntimeError("the 15-form frontier or fixed order drifted")
    ...
    if any(surface in base_glossary for surface in TARGET_SURFACES):
        raise RuntimeError("a GDT671 target unexpectedly already has a V47 glossary row")
    ...
    y_occurrences = read_tsv(
        ROOT / g670.g669.g668.g667.g666.g665.g664.g663.g662.g661.g660.G659 / "artifacts/Y_OCCURRENCE_CENSUS.tsv"
    )
    ...
    inherited_target_by_token: dict[tuple[str, int], dict[str, object]] = {}
    inherited_audits = (
        ROOT / g670.g669.g668.g667.g666.g665.g664.g663.g662.g661.G660 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        ROOT / g670.g669.g668.g667.g666.g665.g664.g663.g662.G661 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        ROOT / g670.g669.g668.g667.g666.g665.g664.g663.G662 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        ROOT / g670.g669.g668.g667.g666.g665.g664.G663 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        ROOT / g670.g669.g668.g667.g666.g665.G664 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        ROOT / g670.g669.g668.g667.g666.G665 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        ROOT / g670.g669.g668.g667.G666 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        ROOT / g670.g669.g668.G667 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        ROOT / g670.g669.G668 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
    ...
    if observed_counts != EXPECTED_SURFACE_COUNTS or sum(observed_counts.values()) != 71:
        raise RuntimeError(f"target surface count drift: {observed_counts!r}")
    if SOLE_LEARNED_SURFACES != {"daiiy", "otoiir"}:
    ...
    expected_compositions = {
        ...
        "olcheol": "O_PREP+L_WOOD+CH_DRY+E_MIDDLE+OL_MATERIAL",
    ...
    if any(EXACT_BY_SURFACE[surface]["composition"] != composition
        ...
        raise RuntimeError("V48 qoeedy/teol/toldy/ychey/ychekch/olcheol/oteedaiin/qotod composition drift")
    if CONTEXT_SCOPED_SURFACES:
        raise RuntimeError("GDT671 has no context-scoped target surfaces")
    exact, normalized = g670.g669.g668.g667.g666.g665.g664.g663.g662.g661.g660.stable_maps(tokens, cross_by_locus)
    ...
    for locus in sorted(by_line):
        ...
        for index, token in enumerate(line):
            ...
            item: dict[str, object] = {
                "occurrence_id": f"G671-T{len(occurrence_rows) + 1:04d}",
    ...
    if len(occurrence_rows) != 71 or len(target_by_token) != 71:
        raise RuntimeError("target occurrence census drift")
    if (len({row["locus"] for row in occurrence_rows}) != 70
            or len({row["page"] for row in occurrence_rows}) != 56):
        raise RuntimeError("target 70-line/56-page census drift")
    ...
    if contextual_rows or contextual_surface_counts or contextual_merge_counts:
        raise RuntimeError("context rendering unexpectedly created for an exact-whole target")
    ychekch_cross = cross_by_locus["f18r.14"]
    if (ychekch_cross["zl3b_clean"].split()[:2] != ["ychekch", "y"]
            or any("ychekchy" not in ychekch_cross[field].split()
                   for field in ("it2a_clean", "rf1b_clean"))):
        raise RuntimeError("bilateral ychekch+y reader-boundary evidence drift")
    ...
    for base_row in base_coverage:
        ...
        if not (len(line) == len(glosses) == len(sources) == len(states)):
            raise RuntimeError(f"V47 token columns misalign: {locus}")
        ...
        for index, token in enumerate(line):
            ...
            if glosses[index] != f"[{surface}:?]" or sources[index] != "OPEN" or states[index] != "UNKNOWN_SURFACE":
                raise RuntimeError(f"V47 target not open at {locus}.{index + 1}: {surface}")
            glosses[index] = str(occurrence["working_gloss_de"])
            if surface in CONTEXT_SCOPED_SURFACES:
                sources[index] = f"GDT671:{occurrence['rendering_class']}:{occurrence['reader_merge_surface']}"
                states[index] = "KNOWN_CONTEXT_LICENSED" if int(occurrence["reader_exact"]) else "READER_BOUNDARY_UNSTABLE"
            else:
                sources[index] = f"GDT671:EXACT_WHOLE:{surface}"
        ...
        if int(base_row["unknown_tokens"]) - len(target_ordinals) != len(unknown_pairs):
            raise RuntimeError(f"V47->V48 arithmetic drift: {locus}")
    ...
    for row in coverage_rows:
        ...
        if old and old["unknown_surface"] == surface and int(old["unknown_ordinal"]) == ordinal:
            ...
            proposal, basis, strength = f"[{surface}:?]", "NEWLY_EXPOSED_BY_GDT671_NO_NEW_CARD", "OPEN"
        strict = int(
            int(row["ambiguous_tokens"]) == 0 and int(row["reader_unstable_tokens"]) == 0
            and int(row["all_present_exact"]) == 1
        )
    ...
    for spec_row in EXACT_WHOLE_SPECS:
        ...
        glossary_rows.append({
            ...
            "source": "GDT671:CONTEXT_SHORT_DEFAULT" if surface in CONTEXT_SCOPED_SURFACES else "GDT671:EXACT_WHOLE",
        ...
        dictionary_rows.append({
            "entry": f"{surface}@GDT671_DEFAULT", "kind": card_type(surface),
            ...
            "status": "NEW_V48_PROVISIONAL_CONCRETE_RECIPE_DEFAULT",
        })
    glossary_rows.sort(key=lambda row: str(row["surface"]))
    if len(glossary_rows) != 1430:
        raise RuntimeError("V48 glossary dimension drift")
    ...
    for klass, surface, merge_surface, render in context_keys:
        ...
        context_cards.append({
            "card_id": f"G671-C{len(context_cards) + 1:03d}", "rendering_class": klass,
        ...
        dictionary_rows.append({
            "entry": f"{surface}@GDT671_{klass}_{merge_surface}", "kind": "PRACTICAL_RENDERING_CARD",
            "working_meaning_de": render, "composition": f"{klass}:{merge_surface}",
            "context_rule": "exact occurrence context; reader join where named",
            "status": "NEW_V48_CONTEXT_RENDER",
        })
    if context_cards or len(dictionary_rows) != 2115:
        raise RuntimeError(
            f"V48 exact-whole dictionary contract drift: contexts={len(context_cards)} "
            f"dictionary={len(dictionary_rows)}"
        )
    ...
    next_frontier_counts = [
        {
            ...
            "status": "NEXT_V48_ONE_HOLE_FRONTIER",
        }
    ...
    expected_next_counts = {
        "qkeeody": 1, "iirain": 1, "charam": 2, "cheykedy": 1,
        "lsheckhy": 2, "oltain": 2, "rshdy": 1, "chorcholsal": 1,
    }
    if (len(newly_one) != 8 or len(next_frontier_counts) != 8
            or {row["surface"]: int(row["full_panel_positions"])
                for row in next_frontier_counts} != expected_next_counts
            or len({row["unknown_surface"] for row in newly_one}) != 8):
        raise RuntimeError(
            f"V48 next-frontier dimensions drift: rows={len(newly_one)} "
            f"surfaces={len(next_frontier_counts)}"
        )
    newly_one.sort(key=lambda row: str(row["locus"]))
    if tuple(row["unknown_surface"] for row in newly_one) != tuple(expected_next_counts):
        raise RuntimeError("V48 eight-row source-frontier order drift")
    ...
    for occurrence in occurrence_rows:
        ...
        audit_rows.append({
            **occurrence,
            "v47_gloss_de": split_pipe(base_row["token_glosses_de"])[ordinal - 1],
            "v48_gloss_de": split_pipe(final_row["token_glosses_de"])[ordinal - 1],
            "v47_scope_state": split_pipe(base_row["scope_states"])[ordinal - 1],
            "v48_scope_state": split_pipe(final_row["scope_states"])[ordinal - 1],
            "v48_working_translation_de": line_translation(
    ...
    if any(str(row["v47_gloss_de"]) != f"[{row['surface']}:?]" for row in audit_rows):
        raise RuntimeError("not every target occurrence was open in V47")
    if any(GENERIC_FILLER.search(str(row["v48_working_translation_de"])) for row in audit_rows):
        raise RuntimeError("generic work filler leaked into GDT671")
    if any("Eigenschafts-/Zustands-/Materialträger" in str(row["v48_working_translation_de"]) for row in audit_rows):
        raise RuntimeError("structural OL meta-gloss leaked into practical V48 prose")
    ...
    for index, spec_row in enumerate(EXACT_WHOLE_SPECS, 1):
        ...
        decision_rows.append({
            "decision_id": f"G671-D{index:03d}", "surface": surface, "family": spec_row["family"],
            ...
            "strength": card_strength(surface), "status": "ACCEPT_V48_REPLACEABLE",
        })
    ...
    for locus in sorted(affected_loci):
        ...
        target_line_rows.append({
            ...
            "zl3b_line": final_row["zl3b_line"], "v47_token_glosses_de": base_row["token_glosses_de"],
            "v48_token_glosses_de": final_row["token_glosses_de"],
            "v48_working_translation_de": line_translation(
                locus, by_line[locus], split_pipe(final_row["token_glosses_de"]),
                y_occurrence_by_token, inherited_target_by_token, target_by_token,
            ),
            "v47_unknown_tokens": base_row["unknown_tokens"], "v48_unknown_tokens": final_row["unknown_tokens"],
            "v48_complete": int(int(final_row["unknown_tokens"]) == 0),
        })
    ...
    for row in frontier:
        ...
        frontier_rows.append({
            ...
            "zl3b_line": row["zl3b_line"], "v47_translation_de": row["proposed_complete_translation_de"],
            "v48_translation_de": line_translation(
                locus, by_line[locus], split_pipe(final_row["token_glosses_de"]),
                y_occurrence_by_token, inherited_target_by_token, target_by_token,
            ),
            "status": "COMPLETE_WITH_PROVISIONAL_CONCRETE_DEFAULT",
        })
    if len(frontier_rows) != 15 or any(f"[{row['surface']}:?]" in str(row["v48_translation_de"]) for row in frontier_rows):
        raise RuntimeError("a GDT671 frontier slot remained open in V48")
    ...
    for spec_row in MANUAL_PASSAGE_SPECS:
        ...
        if int(final_row["unknown_tokens"]) != 0:
            raise RuntimeError(f"manual passage is not V48-complete: {locus}")
        ...
        manual_passage_rows.append({
            **spec_row,
            "automatic_v48_translation_de": automatic,
            "v48_unknown_tokens": final_row["unknown_tokens"],
            "comparison_scope": "manual workshop prose versus deterministic V48 renderer; neither is confirmed plaintext",
        })
    ...
    expected_base = {
        "physical_lines": 4128, "known_token_positions": 24088, "unknown_token_positions": 8251,
        "complete_multi_token_lines": 1223, "strict_complete_lines": 313,
        "one_unknown_lines": 154, "strict_one_unknown_lines": 34, "working_glossary_surfaces": 1415,
    }
    if base_metrics != expected_base:
        raise RuntimeError(f"V47 base metrics drift: {base_metrics!r}")
    expected_final_projection = {
        "physical_lines": 4128,
        "known_token_positions": 24159,
        "unknown_token_positions": 8180,
        "complete_multi_token_lines": 1242,
        "one_unknown_lines": 144,
        "working_glossary_surfaces": 1430,
    }
    if any(final_metrics[key] != value for key, value in expected_final_projection.items()):
        raise RuntimeError(f"V48 projected coverage drift: {final_metrics!r}")
    round_rows = [
        {"version": "V47", "added_cards": "BASE", "dictionary_entries": len(base_dictionary), **base_metrics},
        {"version": "V48", "added_cards": f"15_DEFAULTS+{len(context_cards)}_RENDERINGS+19_OL_REVISIONS+2_SOL_REVISIONS",
    ...
    write_tsv(
        output_dir / "CONTEXT_RENDERING_CARDS.tsv", context_cards,
        ("card_id", "rendering_class", "surface", "reader_merge_surface", "occurrences",
         "working_render_de", "selection_rule", "semantic_effect"),
    )
    ...
    write_tsv(output_dir / "FRONTIER_15_COMPLETIONS.tsv", frontier_rows, list(frontier_rows[0]))
    ...
    write_tsv(output_dir / "V48_WORKING_TOKEN_GLOSSARY.tsv", glossary_rows, list(base_glossary_rows[0]))
    write_tsv(output_dir / "WORKING_DICTIONARY_V48.tsv", dictionary_rows, list(base_dictionary[0]))
    write_tsv(output_dir / "ALL_LINE_CONCRETE_COVERAGE_V48.tsv", coverage_rows, coverage_fields)
    write_tsv(output_dir / "COMPLETE_PASSAGES_V48.tsv", complete_rows, complete_fields)
    write_tsv(output_dir / "ONE_UNKNOWN_PASSAGES_V48.tsv", one_rows, one_fields)
    write_tsv(
    ...
    write_tsv(
        ...
        (
            ...
            "automatic_v48_translation_de", "v48_unknown_tokens", "comparison_scope",
        ),
    )

    input_paths = (
        G670 / "REPORT.md", G670 / "artifacts/RESULT.json", G670 / "artifacts/PAGE_ALLOWLIST.tsv",
        G670 / "artifacts/V47_WORKING_TOKEN_GLOSSARY.tsv", G670 / "artifacts/WORKING_DICTIONARY_V47.tsv",
        G670 / "artifacts/ALL_LINE_CONCRETE_COVERAGE_V47.tsv", G670 / "artifacts/COMPLETE_PASSAGES_V47.tsv",
        G670 / "artifacts/ONE_UNKNOWN_PASSAGES_V47.tsv", G670 / "artifacts/NEWLY_EXPOSED_ONE_HOLE_LINES.tsv",
        G670 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        g670.g669.g668.g667.G666 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        g670.g669.g668.G667 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        g670.g669.G668 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        g670.g669.g668.g667.g666.G665 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        g670.g669.g668.g667.g666.g665.G664 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        g670.g669.g668.g667.g666.g665.g664.G663 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        g670.g669.g668.g667.g666.g665.g664.g663.G662 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        g670.g669.g668.g667.g666.g665.g664.g663.g662.G661 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        g670.g669.g668.g667.g666.g665.g664.g663.g662.g661.G660 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        g670.g669.g668.g667.g666.g665.g664.g663.g662.g661.g660.G659 / "artifacts/Y_OCCURRENCE_CENSUS.tsv",
        ...
        BASE_REL / "src/PASSAGE_SYNTHESIS_AUDIT.md",
    ...
    result_core: dict[str, object] = {
        "schema": "GDT671_FIFTEEN_RESIDUAL_FAMILY_COMPLETION_RESULT_V1",
        "experiment_id": "GDT671", "status": STATUS,
        "guard": {
        ...
        "manual_passages": {
            ...
            "v48_complete_rows": sum(int(row["v48_unknown_tokens"]) == 0 for row in manual_passage_rows),
        ...
        "working_dictionary": {
            "v47_entries": len(base_dictionary), "v48_entries": len(dictionary_rows),
            "added_default_entries": len(EXACT_WHOLE_SPECS), "added_rendering_entries": len(context_cards),
            "v47_glossary_surfaces": len(base_glossary_rows), "v48_glossary_surfaces": len(glossary_rows),
        },
        "frontier": {
        ...
        "claim_boundary": (
            "Exploratory replaceable concrete defaults for 15 V47 residual surfaces at 71 inherited positions. "
            f"The final synthesis uses {len(TARGET_SURFACES - SOLE_LEARNED_SURFACES)} role-composed cards "
            f"and {len(SOLE_LEARNED_SURFACES)} learned exact wholes. All new cards dispatch only on exact "
            "whitespace-delimited surfaces. Bilateral ychekch+y reader evidence is recorded without an invisible terminal Y, "
            "context renderer, or suppression. Nineteen inherited material forms retain their O_PREP+L_WOOD practical renderer at 256 positions, "
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.artifact_dir.resolve() == ART.resolve():
        with tempfile.TemporaryDirectory(prefix="gdt671_replay_") as directory:
    ...
    print(
        f"GDT671 built: targets={result['targets']['positions']} surfaces=15 "
