...
def check_witness(w,case):
    from joint import canonical_original
    args=dict(original_parse=canonical_original(case),parse=w['parse'],variant=w['variant'])
    one=bounded('joint',args);two=bounded('joint',dict(**args,independent=True))
...
def job(case):
    ...
    if result['status']=='sat':result['witness_check']=check_witness(result['witness'],case)
