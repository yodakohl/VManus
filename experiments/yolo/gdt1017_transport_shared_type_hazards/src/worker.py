...
def bounded(mode,payload):
    s,g=inputs();seconds=s['meaning_worker_seconds'] if mode in ('replay','joint') else s['query_worker_seconds']
...
def main():
    ...
    try:
        if mode=='primary':
            ...
            from joint import replay_joint
            out=dict(status='COMPLETE',result=replay_joint(j['original_parse'],j['parse'],j['variant'],s,j.get('independent',False)))
        else:
