...
def main():
    ...
    summaries=[];assignments=[];docs=['# Vollständige gemeinsame Modelllesungen','',
      'Alle gespeicherten und verifizierten positiven Wörterbücher werden vollständig gezeigt.',
      ...
      'alle312Mitglieder und sämtliche tatsächlich geprüften Umbenennungen stehen in den Tabellen.','']
    for case,p,check in zip(cases,primary,checks):
        for engine,r in [('primary',p),('independent',check['independent'])]:
            ...
            summ=dict(case=case['id'],engine=engine,context_index=case['context_index'],class_name=case['class_name'],variant_index=case['variant_index'],groups=len(panel[case['context_index']]['words']),new_spellings=len(newwords),new_THEN_spellings=counts['THEN'],THEN_positions=sum(cl['kind']=='THEN' for cl in parse),longest_THEN_run=longest,cargos=len(result['cargo']),hazard_pairs=len(result['hazards']),asserted_hazard_pairs=len(result['asserted_hazards']),qokedy_hazard_degree=degree,voyage_counts=','.join(map(str,sorted({len(p['trace'])-1 for p in paths}))),consistent_paths=len(paths),original_example=member['original_id'],independent_meaning_capacity=0)
            ...
            docs+=['## '+case['id']+' / '+engine,'','Kontext: '+panel[case['context_index']]['id']+'. Alter Vertreter: '+member['original_id']+'.',
                   ...
                   'Neue Welt: '+str(len(result['cargo']))+' Frachten, '+str(len(result['hazards']))+' wirksame Gefahrenpaare ('+str(len(result['asserted_hazards']))+' davon lokal behauptet), Fahrtenzahlen '+summ['voyage_counts']+'.','']
    ...
    if not summaries:
        put('WITNESS_SUMMARY.json',dict(saved_verified_canonical_witnesses=0,primary_counts=dict(collections.Counter(r['status'] for r in primary)),independent_counts=dict(collections.Counter(r['independent']['status'] for r in checks)),confirmed_words=0))
        (A/'READINGS.md').write_text('# No verified complete positive pair under the new union law\n')
        return
    ...
    summary=dict(primary_counts=dict(collections.Counter(r['status'] for r in primary)),independent_counts=dict(collections.Counter(r['independent']['status'] for r in checks)),saved_verified_canonical_witnesses=len(summaries),new_cargo_counts=dict(collections.Counter(r['cargos'] for r in summaries)),new_hazard_pair_counts=dict(collections.Counter(r['hazard_pairs'] for r in summaries)),new_asserted_hazard_pair_counts=dict(collections.Counter(r['asserted_hazard_pairs'] for r in summaries)),THEN_positions_range=[min(r['THEN_positions'] for r in summaries),max(r['THEN_positions'] for r in summaries)],longest_THEN_run_range=[min(r['longest_THEN_run'] for r in summaries),max(r['longest_THEN_run'] for r in summaries)],new_THEN_spellings_range=[min(r['new_THEN_spellings'] for r in summaries),max(r['new_THEN_spellings'] for r in summaries)],hazard_witnesses=[r for r in summaries if r['hazard_pairs']],independent_meaning_capacity=0,confirmed_words=0)
