...
def independent_job(args):
    ...
    if other['status']=='sat':other['witness_check']=check_witness(other['witness'],case)
...
def lift_job(j):
    ...
    args=dict(original_parse=original['parse'],parse=parsed,variant=case['variant'])
    a=bounded('joint',args);b=bounded('joint',dict(**args,independent=True))
    results=[]
    if a['status']==b['status']=='COMPLETE':
        assert a['result']==b['result'] and a['result']['status']=='COHERENT'
        for result in [a['result']['original'],a['result']]:
            paths=[p for p in result['paths'] if p['consistent']]
            results.append(dict(status='VERIFIED_COHERENT',cargo=result['cargo'],hazards=result['hazards'],asserted_hazards=result['asserted_hazards'],consistent_paths=len(paths),voyage_counts=sorted({len(p['trace'])-1 for p in paths}),example_trace=paths[0]['trace']))
    else:
        results=[dict(status='UNKNOWN_REPLAY',primary=a['status'],independent=b['status']) for _ in range(2)]
