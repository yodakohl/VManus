#!/usr/bin/env python3
"""Independent release validator for GDT646."""
...
BASE = Path("experiments/yolo/gdt646_tcheey_surface_completion")
ART = ROOT / BASE / "artifacts"
RUN = ROOT / BASE / "src/run.py"
MANIFEST = ROOT / BASE / "experiment.json"
VALIDATION = ART / "VALIDATION.json"
G645 = Path("experiments/yolo/gdt645_ranked_five_surface_completion")
G645_ALLOW = G645 / "artifacts/PAGE_ALLOWLIST.tsv"
G645_GLOSSARY = G645 / "artifacts/V22_EXACT_TOKEN_GLOSSARY.tsv"
G645_DICTIONARY = G645 / "artifacts/WORKING_DICTIONARY_V22.tsv"
TOKENS = Path("transcription/voynich_zl3b_tokens.tsv")
CROSS = Path("transcription/voynich_cross_transcription_lines.tsv")

EXPECTED = {
    "tcheey": ("kalt und trocken am Ende des Grades", "t+ch+ee+y", "f35r.5", 6, 6, 6, 6,
               "exact complete ZL3b surface only; t, ch, tch, e, ee, eey and y remain whole-bound"),
}
EXPECTED_ROUNDS = (
    ("BASE_V22", 303, 10345, 21994, 60, 42, 71, 22, 256),
    ("tcheey", 304, 10351, 21988, 61, 42, 70, 22, 257),
)
EXPECTED_COMPLETE = {"f35r.5": ("tcheey", "0")}
EXPECTED_NEW_ONE: dict[str, tuple[str, str, str]] = {}
FAMILY_PAIRS = {
    *(("QUALITY_FORM_LADDER", f"{quality}{form}")
      for quality in ("kch", "tch", "ksh", "tsh") for form in ("y", "ey", "eey")),
    *(("QUALITY_RESULT_LADDER", f"{quality}{form}")
      for quality in ("kch", "tch", "ksh", "tsh") for form in ("dy", "edy", "eedy")),
}
ABSENT = {"ksheedy", "tcheedy", "tsheedy"}
OUTPUTS = (
    ...
    "COMPLETE_PASSAGES_V23.tsv", "ONE_UNKNOWN_PASSAGES_V23.tsv",
    "WORKING_DICTIONARY_V23.tsv", "RESULT.json",
)
HELPERS = tuple(
...
EXPECTED_INPUTS = set(HELPERS) | {
    ...
    "experiments/yolo/gdt641_strict_tch_bound_form_completion/REPORT.md",
    str(G645 / "src/run.py"), str(G645 / "artifacts/PAGE_ALLOWLIST.tsv"),
    str(G645 / "artifacts/ALL_LINE_CONCRETE_COVERAGE_V22.tsv"),
    str(G645 / "artifacts/COMPLETE_PASSAGES_V22.tsv"),
    str(G645 / "artifacts/ONE_UNKNOWN_PASSAGES_V22.tsv"),
    str(G645 / "artifacts/NEWLY_EXPOSED_ONE_HOLE_LINES.tsv"),
    str(G645 / "artifacts/V22_EXACT_TOKEN_GLOSSARY.tsv"),
    str(G645 / "artifacts/WORKING_DICTIONARY_V22.tsv"),
    str(G645 / "artifacts/RESULT.json"), str(G645 / "REPORT.md"), str(TOKENS), str(CROSS),
}
FILLER = re.compile(
...
def load_builder():
    spec = importlib.util.spec_from_file_location("gdt646_builder_validation", RUN)
...
def main() -> int:
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="gdt646_validate_") as tmp:
            replay = Path(tmp)
    ...
    check(manifest.get("experiment_id") == "GDT646", "manifest id")
    ...
    check(result.get("schema") == "GDT646_TCHEEY_SURFACE_COMPLETION_RESULT_V1", "result schema")
    check(result.get("status") == "PASS_1_EXACT_SURFACE__6_POSITIONS__1_NEW_COMPLETE_LINE", "result status")
    ...
    check((ART / "PAGE_ALLOWLIST.tsv").read_bytes() == (ROOT / G645_ALLOW).read_bytes(), "allowlist replay")
    ...
    check(len(deck) == 1 and [row["surface"] for row in deck] == list(EXPECTED), "ordered target")
    for order, row in enumerate(deck, 1):
        meaning, composition, locus, n, p, e, s, scope = EXPECTED[row["surface"]]
        check((row["candidate_id"], int(row["candidate_order"]), row["source_locus"]) == (f"G646-C{order:02d}", order, locus), f"card id:{row['surface']}")
    ...
    check(len(family) == len(pairs) == 24 and pairs == FAMILY_PAIRS, "24-cell atlas")
    check({row["surface"] for row in family if row["surface_status"] == "ABSENT_PREDICTION_HOLD"} == ABSENT, "three held null cells")
    for row in family:
        surface = row["surface"]
    ...
    check(len(components) == 3 and [row["component_id"] for row in components] == [f"G646-B{i:02d}" for i in range(1, 4)], "three components")
    check(len({(row["surface"], row["segment"]) for row in components}) == 3, "unique components")
    ...
    historical = read_tsv(ART / "HISTORICAL_SUBDEGREE_COMPARATOR.tsv")
    check(len(historical) == 5 and [row["comparator_id"] for row in historical] == [f"HC{i:02d}" for i in range(1, 6)], "five historical comparators")
    check({part for row in historical for part in row["subdegree_position"].split(" | ")} == {"ANFANG", "MITTE", "ENDE"}, "historical three-position system")
    check(all(row["date"] == "1415" and row["source_url"].startswith("https://celt.ucc.ie/published/G60000") for row in historical), "historical source/date")

    audits = read_tsv(ART / "ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv")
    variants = read_tsv(ART / "READER_VARIANT_AUDIT.tsv")
    check(len(audits) == len({row["audit_id"] for row in audits}) == 6, "six audits")
    check(Counter(row["surface"] for row in audits) == Counter({surface: values[3] for surface, values in EXPECTED.items()}), "audit census")
    check(sum(int(row["reader_exact"]) for row in audits) == sum(int(row["split_normalized"]) for row in audits) == 6, "six exact targets")
    check(Counter(row["verdict"] for row in audits) == Counter({"CLEAN_CONTEXT_COMPATIBLE": 2, "OPAQUE_OR_UNSTABLE_CONTEXT": 4}), "audit verdicts")
    check(not variants and not {(row["surface"], row["locus"]) for row in audits if row["reader_exact"] == "0"}, "empty warning deck")
    for row in audits:
    ...
    check(len(rounds) == 2 and len(ledger) == 1 and [row["surface"] for row in ledger] == list(EXPECTED), "sequential states")
    ...
    complete_by_locus = {row["locus"]: row for row in read_tsv(ART / "COMPLETE_PASSAGES_V23.tsv")}
    check(len(new_complete) == 1 and {row["locus"] for row in new_complete} == set(EXPECTED_COMPLETE), "one completed line")
    for row in new_complete:
        ...
        check(EXPECTED[surface][0] in row["literal_v23_de"] and not FILLER.search(row["smoothed_working_reading_de"]), f"completion text:{row['locus']}")
    new_one = read_tsv(ART / "NEWLY_EXPOSED_ONE_HOLE_LINES.tsv")
    one_by_locus = {row["locus"]: row for row in read_tsv(ART / "ONE_UNKNOWN_PASSAGES_V23.tsv")}
    check(not new_one and not EXPECTED_NEW_ONE, "no new one-hole lines")
    for row in new_one:
    ...
    old_gloss_rows, gloss_rows = read_tsv(ROOT / G645_GLOSSARY), read_tsv(ART / "V23_EXACT_TOKEN_GLOSSARY.tsv")
    old_gloss = {row["surface"]: row for row in old_gloss_rows}
    glossary = {row["surface"]: row for row in gloss_rows}
    check(len(old_gloss_rows) == len(old_gloss) == 256 and len(gloss_rows) == len(glossary) == 257, "glossary 256 to 257")
    ...
    old_dictionary, dictionary = read_tsv(ROOT / G645_DICTIONARY), read_tsv(ART / "WORKING_DICTIONARY_V23.tsv")
    check(len(old_dictionary) == 303 and len(dictionary) == 304 and dictionary[:303] == old_dictionary, "dictionary prefix")
    check([row["entry"].split("@", 1)[0] for row in dictionary[303:]] == list(EXPECTED), "dictionary tail")
    for order, row in enumerate(ledger, 1):
        check((row["pre_dictionary_sha256"], row["post_dictionary_sha256"]) == (canonical_hash(dictionary[:302 + order]), canonical_hash(dictionary[:303 + order])), f"ledger hashes:{row['surface']}")
    for order, row in enumerate(rounds):
        check(row["dictionary_sha256"] == canonical_hash(dictionary[:303 + order]), f"round hash:{row['surface']}")

    coverage = read_tsv(ART / "ALL_LINE_CONCRETE_COVERAGE_V23.tsv")
    complete = read_tsv(ART / "COMPLETE_PASSAGES_V23.tsv")
    one = read_tsv(ART / "ONE_UNKNOWN_PASSAGES_V23.tsv")
    check(len(coverage) == 4128 and sum(int(row["known_tokens"]) for row in coverage) == 10351 and sum(int(row["unknown_tokens"]) for row in coverage) == 21988, "V23 coverage")
    check(len(complete) == 61 and sum(int(row["strict_complete"]) for row in complete) == 42, "V23 completions")
    check(len(one) == 70 and sum(int(row["strict_eligible"]) for row in one) == 22, "V23 frontier")
    ...
    check(result.get("target_run") == {"candidates": 1, "accepted": 1, "held": 0, "audited_occurrences": 6, "all_reader_exact_occurrences": 6, "split_normalized_occurrences": 6, "thermal_reader_rivals": 0, "hard_collisions": 0, "verdicts": {"CLEAN_CONTEXT_COMPATIBLE": 2, "OPAQUE_OR_UNSTABLE_CONTEXT": 4}, "accepted_surfaces": list(EXPECTED)}, "result target state")
    check(result.get("coverage") == {"base_complete_multi_token_lines": 60, "base_strict_complete_lines": 42, "newly_completed_lines": 1, "newly_exposed_one_hole_lines": 0, "physical_lines": 4128, "known_token_positions": 10351, "unknown_token_positions": 21988, "complete_multi_token_lines": 61, "strict_complete_lines": 42, "one_unknown_lines": 70, "strict_one_unknown_lines": 22, "exact_glossary_surfaces": 257}, "result coverage")
    ...
    check(result.get("working_dictionary") == {"v22_entries": 303, "v23_entries": 304, "accepted_tail_entries": 1, "v22_prefix_sha256": canonical_hash(old_dictionary), "v23_sha256": canonical_hash(dictionary), "base_glossary_surfaces": 256, "v23_glossary_surfaces": 257}, "result dictionary")
    check(result.get("historical_comparator") == {"source_work": "Tadhg O Cuinn, An Irish Materia Medica", "date": 1415, "rows": 5, "subdegree_positions": ["ANFANG", "MITTE", "ENDE"]}, "result historical comparator")

    core = {
        "schema": "GDT646_VALIDATION_V1", "experiment_id": "GDT646",
        "status": "PASS" if not issues else "FAIL", "passed_checks": len(passed),
        "issue_count": len(issues), "issues": issues,
        "summary": {"target_occurrences": len(audits), "reader_exact": sum(int(row["reader_exact"]) for row in audits), "clean_context_compatible": sum(row["verdict"] == "CLEAN_CONTEXT_COMPATIBLE" for row in audits), "new_complete_lines": len(new_complete), "new_one_hole_lines": len(new_one), "v23_dictionary_entries": len(dictionary), "result_content_sha256": result.get("content_sha256")},
    }
    ...
    print(f"GDT646 validation: {validation['status']} checks={len(passed)} issues={len(issues)}")
    for issue in issues:
        print(f"ISSUE {issue}")
