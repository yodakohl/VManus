...
// GDT836 mandatory wholeword-priority extension of frozen GDT834.
...
struct Search {
    ...
    // No language score or cache is touched while proposing initial packages.
    ...
    bool change(const vector<pair<int,int>>& changes,double temperature,bool forced=false) {
        ...
        // An inactive W change can invalidate an unchanged word. Scan globally,
    ...
    void optimize(int steps,int sweeps) {
        ...
        key=bestkey;rebuild();if(!legal()||(strict&&!priority_legal_cached()))throw runtime_error("final illegal key");
    }
