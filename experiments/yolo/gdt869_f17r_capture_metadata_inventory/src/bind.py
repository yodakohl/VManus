...
up=['experiments/semantic_assumptions/acquire_eba001_raw_directional_msi.py','experiments/semantic_assumptions/results/eba001_raw_directional_msi_inventory.json','experiments/semantic_assumptions/results/eba001_raw_directional_msi_report.md','docs/physical_imaging/README_SOURCE_REVIEW_2026-09-06.md']
...
m.update(title='Three f17r capture metadata inventory',question='Do additional embedded metadata explicitly identify illumination for the three known f17r UV captures?',claim_ceiling='Metadata inventory only; no pixel decoding, image observation, material, chronology or meaning',status=status,dependencies=[],inputs=[{'path':n,'sha256':sha(ROOT/n),'role':'frozen_source_or_extractor'} for n in up],outputs=[{'path':p.relative_to(ROOT).as_posix(),'sha256':sha(p),'role':'primary_report' if p.name=='REPORT.md' else 'source_or_artifact'} for p in outputs],validation={'status':validation_status,'artifact':rel+'/artifacts/VALIDATION.json' if validation_file.exists() else None})
