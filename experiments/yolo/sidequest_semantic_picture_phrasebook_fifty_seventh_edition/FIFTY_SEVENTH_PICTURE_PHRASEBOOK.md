# Bild- und Exemplarsprechbuch

Der Meister lehrt neben den Karten zwanzig kurze Inhaltskonventionen. Einige
kommen direkt aus dem Bild, andere aus dem lokalen Rezept- oder Stations-
exemplar. Sie dürfen die flüssige Lesung konkret machen, bleiben aber außerhalb
des Stammes.

## Zwanzig Wendungen

- **die abgebildete Pflanze** (5 Aussagen) — VISIBLE_OWNER_NOUN.
- **der im Artikel gemeinte untere Pflanzenteil** (1 Aussagen) — OWNER_PLUS_ARTICLE_DETAIL.
- **der bild- oder exemplarbestimmte Blütenteil** (6 Aussagen) — OWNER_PLUS_ARTICLE_DETAIL.
- **der aktuell verarbeitete Pflanzenposten** (3 Aussagen) — OWNER_PLUS_ARTICLE_DETAIL.
- **das für den Gang vorgesehene Wasser** (9 Aussagen) — MASTER_EXEMPLAR_MATERIAL.
- **das im Rezept vorgesehene Weinmedium** (3 Aussagen) — MASTER_EXEMPLAR_MATERIAL.
- **das im Rezept vorgesehene Öl** (2 Aussagen) — MASTER_EXEMPLAR_MATERIAL.
- **der aus der Pflanze gewonnene Saft** (1 Aussagen) — MASTER_EXEMPLAR_MATERIAL.
- **das Ergebnis des gerade geführten Auszugsgangs** (8 Aussagen) — PROCESS_RESULT.
- **der laufende Arbeitsansatz** (11 Aussagen) — PROCESS_STATE.
- **die aktuelle Becken- oder Arbeitsflüssigkeit** (17 Aussagen) — OWNER_PROCESS_MEDIUM.
- **das sichtbar adressierte Becken** (37 Aussagen) — VISIBLE_OWNER_NOUN.
- **das sichtbare oder gelernte Arbeitsgefäß** (31 Aussagen) — VISIBLE_OWNER_OR_NOMENCLATOR.
- **das für den Durchgang vorgesehene Tuch** (13 Aussagen) — LEARNED_TOOL_OR_EXEMPLAR.
- **die sichtbar bezeichnete Öffnung** (7 Aussagen) — VISIBLE_OWNER_ADDRESS.
- **der lokale sichtbare Auslauf** (6 Aussagen) — VISIBLE_OWNER_PROCESS.
- **die bezeichnete örtliche Stelle** (31 Aussagen) — VISIBLE_OWNER_ADDRESS.
- **der abgeteilte Teil** (23 Aussagen) — CARD_LEXICON.
- **eine abgeteilte Portion** (9 Aussagen) — CARD_LEXICON.
- **das örtliche Sollmaß** (8 Aussagen) — CARD_LEXICON.

## Besitzerwechsel

- der ganzen breitblättrigen gezähnten Bildpflanze: diese Bildpflanze; aus dieser Pflanze → am gezeigten Pflanzenteil.
- der ganzen dicht blau bekrönten Bildpflanze: diese Bildpflanze; aus dieser Pflanze → am gezeigten Pflanzenteil.
- der ganzen breitblättrigen rispigen Bildpflanze: diese Bildpflanze; aus dieser Pflanze → am gezeigten Pflanzenteil.
- der ganzen mehrköpfigen stacheligen Bildpflanze: diese Bildpflanze; aus dieser Pflanze → am gezeigten Pflanzenteil.
- dem gemeinsamen zweireihigen Becken: diese Station; aus dem sichtbaren Einlauf oder Gefäß → zur sichtbaren Öffnung oder Schale.
- den oberen Paarbecken mit Zylinder: dieses Werkstück; vom Ausgangsposten → zur bezeichneten Arbeitsstelle.
- der mittleren linken Vorrichtung mit Zwischenknoten: diese Station; aus dem sichtbaren Einlauf oder Gefäß → zur sichtbaren Öffnung oder Schale.
- der mittleren rechten Station: diese Station; aus dem sichtbaren Einlauf oder Gefäß → zur sichtbaren Öffnung oder Schale.
- dem unteren grünen Mehrfigurenbecken: dieses Werkstück; vom Ausgangsposten → zur bezeichneten Arbeitsstelle.
- den Randstationen des unteren Beckens: diese Station; aus dem sichtbaren Einlauf oder Gefäß → zur sichtbaren Öffnung oder Schale.
- der oberen offenen Fächerstation: dieses Werkstück; vom Ausgangsposten → zur bezeichneten Arbeitsstelle.
- der mittleren runden Gefäßstation: diese Station; aus dem sichtbaren Einlauf oder Gefäß → zur sichtbaren Öffnung oder Schale.
- der unteren korbförmigen Gefäßstation: diese Station; aus dem sichtbaren Einlauf oder Gefäß → zur sichtbaren Öffnung oder Schale.
- der Lücke zwischen Randstation und Hauptbogen: diese Zwischenstelle; von der vorigen sichtbaren Station → zur folgenden sichtbaren Station.
- dem verbundenen Hauptbogenpaar: dieses verbundene Bogenpaar; vom sichtbaren oberen Anschluss → zum sichtbaren unteren oder seitlichen Anschluss.
- der linken offenen Randstation: dieses Werkstück; vom Ausgangsposten → zur bezeichneten Arbeitsstelle.
- der rechten S-förmigen Mehrfachstation: dieses Werkstück; vom Ausgangsposten → zur bezeichneten Arbeitsstelle.
