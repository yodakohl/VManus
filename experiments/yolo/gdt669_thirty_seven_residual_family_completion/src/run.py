#!/usr/bin/env python3
"""Build GDT669: close the 37-form V45 frontier as concrete V46 recipe cards."""
...
BASE_REL = Path("experiments/yolo/gdt669_thirty_seven_residual_family_completion")
ART = ROOT / BASE_REL / "artifacts"
G668 = Path("experiments/yolo/gdt668_seventy_six_residual_family_completion")
_spec = importlib.util.spec_from_file_location("gdt668_builder_for_gdt669", ROOT / G668 / "src/run.py")
if _spec is None or _spec.loader is None:
    raise RuntimeError("cannot load GDT668 builder")
g668 = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(g668)
TOKENS_REL, CROSS_REL = g668.TOKENS_REL, g668.CROSS_REL
...
def read_card_specs(path: Path) -> tuple[dict[str, str], ...]:
    ...
    if not rows or set(rows[0]) != expected:
        raise RuntimeError("bad GDT669 CARD_SPECS.tsv columns")
    surfaces = [row["surface"] for row in rows]
    if len(rows) != 37 or len(set(surfaces)) != 37:
        raise RuntimeError("GDT669 CARD_SPECS.tsv must contain 37 unique surfaces")
    return tuple(rows)
...
lkchdal=1 okeedal=4 pched=1 dcheey=9 ro=7 shek=10 odl=3 okytaiin=1 oytor=1
ykeor=4 orshy=2 chordy=2 daiinol=1 tcheodal=1 yteain=1 cthal=7 loeees=2
chodys=1 ytair=4 dsho=6 shear=22 eeckhy=1 kchokchy=1 dalar=5 alchdy=2 sh=14
psheedy=1 tchar=4 shedaiin=10 olkair=4 sary=7 dals=8 ckhody=4 lkeol=5
shokol=2 ytchas=1 oeees=6
""")
...
OUTPUT_NAMES = (
    ...
    "FAMILY_COMPOSITION_ATLAS.tsv", "FRONTIER_37_COMPLETIONS.tsv",
    ...
    "V46_WORKING_TOKEN_GLOSSARY.tsv", "WORKING_DICTIONARY_V46.tsv",
    "ALL_LINE_CONCRETE_COVERAGE_V46.tsv", "COMPLETE_PASSAGES_V46.tsv",
    "ONE_UNKNOWN_PASSAGES_V46.tsv", "INHERITED_OL_RENDER_REVISIONS.tsv",
    "INHERITED_SOL_RENDER_REVISIONS.tsv", "STEM_MODEL_V46.tsv", "MANUAL_PASSAGE_AUDIT.tsv",
)
...
# These are practical render revisions only.  V45's structural tags remain
# byte-visible in its inherited glossary/dictionary; V46 translates the 19
...
def read_ol_revision_specs(path: Path) -> tuple[dict[str, str], ...]:
    ...
    if not rows or set(rows[0]) != expected:
        raise RuntimeError("bad GDT669 INHERITED_OL_REVISION_SPECS.tsv columns")
    surfaces = [row["surface"] for row in rows]
    if len(rows) != 19 or len(set(surfaces)) != 19 or sum(int(row["positions"]) for row in rows) != 256:
        raise RuntimeError("GDT669 OL revision source must contain 19 unique forms at 256 positions")
...
def read_sol_revision_specs(path: Path) -> tuple[dict[str, str], ...]:
    ...
    expected = {"surface", "positions", "practical_meaning_de", "composition", "retained_rival_de"}
    if not rows or set(rows[0]) != expected:
        raise RuntimeError("bad GDT669 INHERITED_SOL_REVISION_SPECS.tsv columns")
    surfaces = [row["surface"] for row in rows]
    if len(rows) != 2 or len(set(surfaces)) != 2 or sum(int(row["positions"]) for row in rows) != 4:
        raise RuntimeError("GDT669 SOL revision source must contain 2 unique forms at 4 positions")
    return tuple(rows)
...
# all token gloss markers at this locus.  V46 preserves that sentence but must
...
def read_stem_model_specs(path: Path) -> tuple[dict[str, str], ...]:
    ...
    if not rows or set(rows[0]) != expected or len(rows) != 56:
        raise RuntimeError("GDT669 STEM_MODEL_SPECS.tsv must contain the 56-row V46 schema")
    return tuple(rows)
...
def read_manual_passage_specs(path: Path) -> tuple[dict[str, str], ...]:
    ...
    if not rows or set(rows[0]) != expected or len(rows) != 20:
        raise RuntimeError("GDT669 MANUAL_PASSAGE_SPECS.tsv must contain the fixed 20-row schema")
    if [int(row["rank"]) for row in rows] != list(range(1, 21)):
        raise RuntimeError("GDT669 manual passage ranks must be 1..20")
    if len({row["locus"] for row in rows}) != 20:
        raise RuntimeError("GDT669 manual passage loci must be unique")
    return tuple(rows)
...
def line_translation(
    ...
    """Render V46 with reader joins over the complete V45 renderer."""
    working_glosses = list(glosses)
    # GDT666's renderer only applies its positional action cards to the map it
    # receives as the current layer.  Re-promote the archived G668 rows here;
    ...
    inherited = {
        ...
        if not str(row.get("occurrence_id", "")).startswith("G668-")
    }
    current = {
        key: row for key, row in inherited_target_by_token.items()
        if str(row.get("occurrence_id", "")).startswith("G668-")
    }
    ...
    rendered = g668.line_translation(
...
def build(output_dir: Path) -> dict[str, object]:
    ...
    tokens, token_stats = g668.g667.g666.g665.g664.g663.g662.g661.g659.guarded_query(
        TOKENS_REL, pages, "page,locus,token_index,eva,kind,section,language,hand"
    )
    cross, cross_stats = g668.g667.g666.g665.g664.g663.g662.g661.g659.guarded_query(
        CROSS_REL, pages,
    ...
    base_dictionary = read_tsv(base_art / "WORKING_DICTIONARY_V45.tsv")
    base_glossary_rows = read_tsv(base_art / "V45_WORKING_TOKEN_GLOSSARY.tsv")
    base_coverage = read_tsv(base_art / "ALL_LINE_CONCRETE_COVERAGE_V45.tsv")
    base_complete = read_tsv(base_art / "COMPLETE_PASSAGES_V45.tsv")
    base_one = read_tsv(base_art / "ONE_UNKNOWN_PASSAGES_V45.tsv")
    ...
    if dimensions != (2022, 1350, 4128, 1153, 180, 37):
        raise RuntimeError(f"V45 base dimensions drift: {dimensions!r}")
    frontier_order = tuple(dict.fromkeys(row["unknown_surface"] for row in frontier))
    if frontier_order != TARGET_ORDER or len(TARGET_SURFACES) != 37:
        raise RuntimeError("the 37-form frontier or fixed order drifted")
    ...
    if any(surface in base_glossary for surface in TARGET_SURFACES):
        raise RuntimeError("a GDT669 target unexpectedly already has a V45 glossary row")
    ...
    y_occurrences = read_tsv(
        ROOT / g668.g667.g666.g665.g664.g663.g662.g661.g660.G659 / "artifacts/Y_OCCURRENCE_CENSUS.tsv"
    )
    ...
    inherited_target_by_token: dict[tuple[str, int], dict[str, object]] = {}
    inherited_audits = (
        ROOT / g668.g667.g666.g665.g664.g663.g662.g661.G660 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        ROOT / g668.g667.g666.g665.g664.g663.g662.G661 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        ROOT / g668.g667.g666.g665.g664.g663.G662 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        ROOT / g668.g667.g666.g665.g664.G663 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        ROOT / g668.g667.g666.g665.G664 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        ROOT / g668.g667.g666.G665 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        ROOT / g668.g667.G666 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        ROOT / g668.G667 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
    ...
    if observed_counts != EXPECTED_SURFACE_COUNTS or sum(observed_counts.values()) != 165:
        raise RuntimeError(f"target surface count drift: {observed_counts!r}")
    if SOLE_LEARNED_SURFACES != {"lkchdal", "eeckhy"}:
        raise RuntimeError(f"reader-alias whole set drift: {sorted(SOLE_LEARNED_SURFACES)!r}")
    alias_expectations = {
        "f105v.13": ("lkchdal", "lkeedal"),
        "f51r.7": ("eeckhy", "chckhy"),
    }
    for locus, (zl_surface, reader_surface) in alias_expectations.items():
        aligned = cross_by_locus[locus]
        if (zl_surface not in aligned["zl3b_clean"].split()
                or reader_surface not in aligned["it2a_clean"].split()
                or reader_surface not in aligned["rf1b_clean"].split()):
            raise RuntimeError(f"two-reader alias evidence drift: {locus}")
    if not (surface_counts["oy"] == 12 and surface_counts["oychey"] == 1
            and surface_counts["oytor"] == 1 and surface_counts["oky"] == 89
            and surface_counts["okytaiin"] == 1):
        raise RuntimeError("OY/OKY preparation-family census drift")
    if EXACT_BY_SURFACE["oytor"]["composition"] != "OY_PREP_BASE+T_COLD+OR_PORTION":
        raise RuntimeError("oytor no longer uses the scoped OY preparation base")
    if EXACT_BY_SURFACE["okytaiin"]["composition"] != "OKY_HOT_PREP_BASE+T_COLD+AIIN_III":
        raise RuntimeError("okytaiin no longer uses the scoped OKY hot-preparation base")
    if EXACT_BY_SURFACE["olkair"]["composition"] != "O_PREP+L_WOOD+K_HOT+AIR_FRACTION_II":
        raise RuntimeError("olkair must preserve initial O_PREP+L_WOOD")
    she_spine = {
        "sh": "SH_MOIST", "shek": "SH_MOIST+E_MIDDLE+K_HOT",
        "shear": "SH_MOIST+E_MIDDLE+AR_FRACTION_I",
        "shedaiin": "SH_MOIST+E_MIDDLE+D_MEASURE+AIIN_III",
    }
    if any(EXACT_BY_SURFACE[surface]["composition"] != composition
           for surface, composition in she_spine.items()):
        raise RuntimeError("SH/E high-frequency composition spine drift")
    exact, normalized = g668.g667.g666.g665.g664.g663.g662.g661.g660.stable_maps(tokens, cross_by_locus)
    ...
    for locus in sorted(by_line):
        ...
        for index, token in enumerate(line):
            ...
            item: dict[str, object] = {
                "occurrence_id": f"G669-T{len(occurrence_rows) + 1:04d}",
    ...
    if len(occurrence_rows) != 165 or len(target_by_token) != 165:
    ...
    for base_row in base_coverage:
        ...
        if not (len(line) == len(glosses) == len(sources) == len(states)):
            raise RuntimeError(f"V45 token columns misalign: {locus}")
        ...
        for index, token in enumerate(line):
            ...
            if glosses[index] != f"[{surface}:?]" or sources[index] != "OPEN" or states[index] != "UNKNOWN_SURFACE":
                raise RuntimeError(f"V45 target not open at {locus}.{index + 1}: {surface}")
            glosses[index] = str(occurrence["working_gloss_de"])
            if surface in CONTEXT_SCOPED_SURFACES:
                sources[index] = f"GDT669:{occurrence['rendering_class']}:{occurrence['reader_merge_surface']}"
                states[index] = "KNOWN_CONTEXT_LICENSED" if int(occurrence["reader_exact"]) else "READER_BOUNDARY_UNSTABLE"
            else:
                sources[index] = f"GDT669:EXACT_WHOLE:{surface}"
        ...
        if int(base_row["unknown_tokens"]) - len(target_ordinals) != len(unknown_pairs):
            raise RuntimeError(f"V45->V46 arithmetic drift: {locus}")
    ...
    for row in coverage_rows:
        ...
        if old and old["unknown_surface"] == surface and int(old["unknown_ordinal"]) == ordinal:
            ...
            proposal, basis, strength = f"[{surface}:?]", "NEWLY_EXPOSED_BY_GDT669_NO_NEW_CARD", "OPEN"
        strict = int(
            int(row["ambiguous_tokens"]) == 0 and int(row["reader_unstable_tokens"]) == 0
            and int(row["all_present_exact"]) == 1
        )
    ...
    for spec_row in EXACT_WHOLE_SPECS:
        ...
        glossary_rows.append({
            ...
            "source": "GDT669:CONTEXT_SHORT_DEFAULT" if surface in CONTEXT_SCOPED_SURFACES else "GDT669:EXACT_WHOLE",
        ...
        dictionary_rows.append({
            "entry": f"{surface}@GDT669_DEFAULT", "kind": card_type(surface),
            ...
            "status": "NEW_V46_PROVISIONAL_CONCRETE_RECIPE_DEFAULT",
        })
    glossary_rows.sort(key=lambda row: str(row["surface"]))
    if len(glossary_rows) != 1387:
        raise RuntimeError("V46 glossary dimension drift")
    ...
    for klass, surface, merge_surface, render in context_keys:
        ...
        context_cards.append({
            "card_id": f"G669-C{len(context_cards) + 1:03d}", "rendering_class": klass,
    ...
    next_frontier_counts = [
        {
            ...
            "status": "NEXT_V46_ONE_HOLE_FRONTIER",
        }
    ...
    if (not newly_one or len(newly_one) != len(next_frontier_counts)
        ...
        raise RuntimeError(
            f"V46 next-frontier dimensions drift: rows={len(newly_one)} "
    ...
    for occurrence in occurrence_rows:
        ...
        audit_rows.append({
            **occurrence,
            "v45_gloss_de": split_pipe(base_row["token_glosses_de"])[ordinal - 1],
            "v46_gloss_de": split_pipe(final_row["token_glosses_de"])[ordinal - 1],
            "v45_scope_state": split_pipe(base_row["scope_states"])[ordinal - 1],
            "v46_scope_state": split_pipe(final_row["scope_states"])[ordinal - 1],
            "v46_working_translation_de": line_translation(
    ...
    if any(str(row["v45_gloss_de"]) != f"[{row['surface']}:?]" for row in audit_rows):
        raise RuntimeError("not every target occurrence was open in V45")
    if any(GENERIC_FILLER.search(str(row["v46_working_translation_de"])) for row in audit_rows):
        raise RuntimeError("generic work filler leaked into GDT669")
    if any("Eigenschafts-/Zustands-/Materialträger" in str(row["v46_working_translation_de"]) for row in audit_rows):
        raise RuntimeError("structural OL meta-gloss leaked into practical V46 prose")
    ...
    for index, spec_row in enumerate(EXACT_WHOLE_SPECS, 1):
        ...
        decision_rows.append({
            "decision_id": f"G669-D{index:03d}", "surface": surface, "family": spec_row["family"],
            ...
            "strength": card_strength(surface), "status": "ACCEPT_V46_REPLACEABLE",
        })
    ...
    for locus in sorted(affected_loci):
        ...
        target_line_rows.append({
            ...
            "zl3b_line": final_row["zl3b_line"], "v45_token_glosses_de": base_row["token_glosses_de"],
            "v46_token_glosses_de": final_row["token_glosses_de"],
            "v46_working_translation_de": line_translation(
                locus, by_line[locus], split_pipe(final_row["token_glosses_de"]),
                y_occurrence_by_token, inherited_target_by_token, target_by_token,
            ),
            "v45_unknown_tokens": base_row["unknown_tokens"], "v46_unknown_tokens": final_row["unknown_tokens"],
            "v46_complete": int(int(final_row["unknown_tokens"]) == 0),
        })
    ...
    for row in frontier:
        ...
        frontier_rows.append({
            ...
            "zl3b_line": row["zl3b_line"], "v45_translation_de": row["proposed_complete_translation_de"],
            "v46_translation_de": line_translation(
                locus, by_line[locus], split_pipe(final_row["token_glosses_de"]),
                y_occurrence_by_token, inherited_target_by_token, target_by_token,
            ),
            "status": "COMPLETE_WITH_PROVISIONAL_CONCRETE_DEFAULT",
        })
    if len(frontier_rows) != 37 or any(f"[{row['surface']}:?]" in str(row["v46_translation_de"]) for row in frontier_rows):
        raise RuntimeError("a GDT669 frontier slot remained open in V46")
    ...
    for spec_row in MANUAL_PASSAGE_SPECS:
        ...
        if int(final_row["unknown_tokens"]) != 0:
            raise RuntimeError(f"manual passage is not V46-complete: {locus}")
        ...
        manual_passage_rows.append({
            **spec_row,
            "automatic_v46_translation_de": automatic,
            "v46_unknown_tokens": final_row["unknown_tokens"],
            "comparison_scope": "manual workshop prose versus deterministic V46 renderer; neither is confirmed plaintext",
        })
    ...
    expected_base = {
        "physical_lines": 4128, "known_token_positions": 23832, "unknown_token_positions": 8507,
        "complete_multi_token_lines": 1153, "strict_complete_lines": 299,
        "one_unknown_lines": 180, "strict_one_unknown_lines": 38, "working_glossary_surfaces": 1350,
    }
    if base_metrics != expected_base:
        raise RuntimeError(f"V45 base metrics drift: {base_metrics!r}")
    round_rows = [
        {"version": "V45", "added_cards": "BASE", "dictionary_entries": len(base_dictionary), **base_metrics},
        {"version": "V46", "added_cards": f"37_DEFAULTS+{len(context_cards)}_RENDERINGS+19_OL_REVISIONS+2_SOL_REVISIONS",
    ...
    write_tsv(output_dir / "FRONTIER_37_COMPLETIONS.tsv", frontier_rows, list(frontier_rows[0]))
    ...
    write_tsv(output_dir / "V46_WORKING_TOKEN_GLOSSARY.tsv", glossary_rows, list(base_glossary_rows[0]))
    write_tsv(output_dir / "WORKING_DICTIONARY_V46.tsv", dictionary_rows, list(base_dictionary[0]))
    write_tsv(output_dir / "ALL_LINE_CONCRETE_COVERAGE_V46.tsv", coverage_rows, coverage_fields)
    write_tsv(output_dir / "COMPLETE_PASSAGES_V46.tsv", complete_rows, complete_fields)
    write_tsv(output_dir / "ONE_UNKNOWN_PASSAGES_V46.tsv", one_rows, one_fields)
    write_tsv(
    ...
    write_tsv(
        ...
        (
            ...
            "automatic_v46_translation_de", "v46_unknown_tokens", "comparison_scope",
        ),
    )

    input_paths = (
        G668 / "REPORT.md", G668 / "artifacts/RESULT.json", G668 / "artifacts/PAGE_ALLOWLIST.tsv",
        G668 / "artifacts/V45_WORKING_TOKEN_GLOSSARY.tsv", G668 / "artifacts/WORKING_DICTIONARY_V45.tsv",
        G668 / "artifacts/ALL_LINE_CONCRETE_COVERAGE_V45.tsv", G668 / "artifacts/COMPLETE_PASSAGES_V45.tsv",
        G668 / "artifacts/ONE_UNKNOWN_PASSAGES_V45.tsv", G668 / "artifacts/NEWLY_EXPOSED_ONE_HOLE_LINES.tsv",
        G668 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        g668.g667.G666 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        g668.g667.g666.G665 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        g668.g667.g666.g665.G664 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        g668.g667.g666.g665.g664.G663 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        g668.g667.g666.g665.g664.g663.G662 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        g668.g667.g666.g665.g664.g663.g662.G661 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        g668.g667.g666.g665.g664.g663.g662.g661.G660 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        g668.g667.g666.g665.g664.g663.g662.g661.g660.G659 / "artifacts/Y_OCCURRENCE_CENSUS.tsv",
        ...
        BASE_REL / "src/HISTORICAL_CODEBOOK_MEMO.md",
    ...
    result_core: dict[str, object] = {
        "schema": "GDT669_THIRTY_SEVEN_RESIDUAL_FAMILY_COMPLETION_RESULT_V1",
        "experiment_id": "GDT669", "status": STATUS,
        "guard": {
        ...
        "card_synthesis": {
            "candidate_lenses": [
                "historical_codebook", "practical_apothecary", "stem_compositor", "passage_reader"
            ],
            ...
            "inherited_stem_roles_reused": 54,
            "total_stem_roles": len(STEM_MODEL_SPECS),
            "new_stem_roles": len(STEM_MODEL_SPECS) - 54,
            "new_stem_role": "OY_PREP_BASE|OKY_HOT_PREP_BASE",
            "historical_analogue": (
        ...
        "manual_passages": {
            ...
            "v46_complete_rows": sum(int(row["v46_unknown_tokens"]) == 0 for row in manual_passage_rows),
        ...
        "working_dictionary": {
            "v45_entries": len(base_dictionary), "v46_entries": len(dictionary_rows),
            "added_default_entries": len(EXACT_WHOLE_SPECS), "added_rendering_entries": len(context_cards),
            "v45_glossary_surfaces": len(base_glossary_rows), "v46_glossary_surfaces": len(glossary_rows),
        },
        "frontier": {
        ...
        "claim_boundary": (
            "Exploratory replaceable concrete defaults for 37 V45 residual surfaces at 165 inherited positions. "
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.artifact_dir.resolve() == ART.resolve():
        with tempfile.TemporaryDirectory(prefix="gdt669_replay_") as directory:
    ...
    print(
        f"GDT669 built: targets={result['targets']['positions']} surfaces=37 "
