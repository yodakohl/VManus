#!/usr/bin/env python3
"""Independent source-first release validator for GDT669."""
...
BASE = Path("experiments/yolo/gdt669_thirty_seven_residual_family_completion")
...
OUTPUT_NAMES = (
    ...
    "FAMILY_COMPOSITION_ATLAS.tsv", "STEM_MODEL_V46.tsv",
    "INHERITED_OL_RENDER_REVISIONS.tsv", "INHERITED_SOL_RENDER_REVISIONS.tsv",
    "MANUAL_PASSAGE_AUDIT.tsv",
    "FRONTIER_37_COMPLETIONS.tsv", "TARGET_LINE_TRANSLATIONS.tsv",
    "ROUND_COVERAGE_COUNTS.tsv", "NEWLY_COMPLETED_LINES.tsv",
    "NEWLY_EXPOSED_ONE_HOLE_LINES.tsv", "NEXT_FRONTIER_FULL_PANEL_COUNTS.tsv",
    "V46_WORKING_TOKEN_GLOSSARY.tsv",
    "WORKING_DICTIONARY_V46.tsv", "ALL_LINE_CONCRETE_COVERAGE_V46.tsv",
    "COMPLETE_PASSAGES_V46.tsv", "ONE_UNKNOWN_PASSAGES_V46.tsv",
)

TARGET_ORDER = tuple("""
lkchdal okeedal pched dcheey ro shek odl okytaiin oytor ykeor orshy chordy
daiinol tcheodal yteain cthal loeees chodys ytair dsho shear eeckhy kchokchy
dalar alchdy sh psheedy tchar shedaiin olkair sary dals ckhody lkeol shokol
ytchas oeees
""".split())
...
def main() -> int:
    ...
    # guarded selector before trusting any GDT669 result or generated artifact.
    base_pages = {row["page"] for row in read_tsv(ROOT / G668 / "artifacts/PAGE_ALLOWLIST.tsv")}
    ...
    check("target count sum", sum(EXPECTED_COUNTS.values()) == 165)
    ...
    check("CARD_SPECS 37 unique forms", len(specs) == len(set(spec_order)) == 37)
    check("context-short forms are exactly sh ro odl", CONTEXT_SURFACES == {"sh", "ro", "odl"})
    check("CARD_SPECS all fields populated", all(all(row[field] for field in spec_fields) for row in specs))
    check("CARD_SPECS raw positions", sum(raw_counts[row["surface"]] for row in specs) == 165)
    ...
    stem_model = read_tsv(ART / "STEM_MODEL_V46.tsv")
    ...
    frontier = read_tsv(ART / "FRONTIER_37_COMPLETIONS.tsv")
    target_lines = read_tsv(ART / "TARGET_LINE_TRANSLATIONS.tsv")
    rounds = read_tsv(ART / "ROUND_COVERAGE_COUNTS.tsv")
    glossary = read_tsv(ART / "V46_WORKING_TOKEN_GLOSSARY.tsv")
    dictionary = read_tsv(ART / "WORKING_DICTIONARY_V46.tsv")
    coverage = read_tsv(ART / "ALL_LINE_CONCRETE_COVERAGE_V46.tsv")
    complete = read_tsv(ART / "COMPLETE_PASSAGES_V46.tsv")
    one = read_tsv(ART / "ONE_UNKNOWN_PASSAGES_V46.tsv")
    ...
    base_glossary = read_tsv(base_art / "V45_WORKING_TOKEN_GLOSSARY.tsv")
    ...
    check("decision dimensions", len(deck) == len(accepted) == len(family) == 37)
    ...
    check("architecture position total", sum(int(row["positions"]) for row in architecture) == 165)

    audit_counts = Counter(row["surface"] for row in audit)
    check("occurrence audit dimensions", len(audit) == len(reader) == 165)
    check("occurrence audit exact counts", dict(audit_counts) == EXPECTED_COUNTS, dict(audit_counts))
    check("occurrence IDs are G669", all(row["occurrence_id"].startswith("G669-") for row in audit))
    check("all target slots were V45-open", all(row["v45_gloss_de"] == f"[{row['surface']}:?]" for row in audit))
    check("all target slots are V46-filled", all(
        row["v46_gloss_de"] and not OPEN_MARKER.search(row["v46_gloss_de"]) for row in audit
    ))
    ...
    check("audit practical prose has no generic work filler", not any(
        GENERIC.search(row["v46_working_translation_de"]) for row in audit
    ))
    check("target-line practical prose has no generic work filler", not any(
        GENERIC.search(row["v46_working_translation_de"]) for row in target_lines
    ))
    ...
    alias_lk = [row for row in reader if row["surface"] == "lkchdal"]
    alias_ee = [row for row in reader if row["surface"] == "eeckhy"]
    shed_split = [row for row in reader if row["surface"] == "shedaiin" and row["locus"] == "f82r.32"]
    check("lkchdal is the exact f105v.13 reader alias", len(alias_lk) == 1
          and alias_lk[0]["locus"] == "f105v.13"
          and "lkchdal" in alias_lk[0]["zl3b_line"].split()
          and "lkeedal" in alias_lk[0]["it2a_line"].split()
          and "lkeedal" in alias_lk[0]["rf1b_line"].split())
    check("eeckhy is the exact f51r.7 reader alias", len(alias_ee) == 1
          and alias_ee[0]["locus"] == "f51r.7"
          and "eeckhy" in alias_ee[0]["zl3b_line"].split()
          and "chckhy" in alias_ee[0]["it2a_line"].split()
          and "chckhy" in alias_ee[0]["rf1b_line"].split())
    check("shedaiin keeps productive spelling despite the f82r.32 split", len(shed_split) == 1
          and "shedaiin" in shed_split[0]["zl3b_line"].split()
          and "she daiin" in shed_split[0]["it2a_line"]
          and "she daiin" in shed_split[0]["rf1b_line"]
          and shed_split[0]["split_normalized"] == "1")
    ...
    check("only named GDT669 boundary forms reader-dispatch", all(
        row["surface"] in CONTEXT_SURFACES for row in merge_rows
    ))

    check("stem-model artifact byte-equivalent to source", (ART / "STEM_MODEL_V46.tsv").read_bytes() == STEM_SPECS.read_bytes())
    ...
    check("final synthesis is 35 role-composed plus 2 learned", len(learned_wholes) == 2
          and len(specs) - len(learned_wholes) == 35)
    ...
    check("two reader aliases remain sole exact learned wholes",
          learned_wholes == {"lkchdal", "eeckhy"})

    oy_stem = stems_by_key[("oy", "OY_PREP_BASE")]
    oky_stem = stems_by_key[("oky", "OKY_HOT_PREP_BASE")]
    check("OY extension stays on its three-card family",
          set(oy_stem["examples"].split("|")) == {"oy", "oychey", "oytor"}
          and card_atoms["oytor"] == ["OY_PREP_BASE", "T_COLD", "OR_PORTION"]
          and raw_counts["oy"] == 12 and raw_counts["oychey"] == 1 and raw_counts["oytor"] == 1)
    check("OKY extension stays on its named family",
          set(oky_stem["examples"].split("|")) == {"oky", "okytaiin"}
          and card_atoms["okytaiin"] == ["OKY_HOT_PREP_BASE", "T_COLD", "AIIN_III"]
          and raw_counts["oky"] == 89 and raw_counts["okytaiin"] == 1)
    check("initial olkair obeys O_PREP plus L_WOOD",
          card_atoms["olkair"] == ["O_PREP", "L_WOOD", "K_HOT", "AIR_FRACTION_II"])
    check("tcheodal wording follows its visible operation order",
          specs_by_surface["tcheodal"]["working_meaning_de"]
          == "abkühlen, bis zur Mittelstufe trocknen, ansetzen und Rohdroge I abmessen"
          and card_atoms["tcheodal"]
          == ["T_COLD", "CH_DRY", "E_MIDDLE", "O_PREP", "D_MEASURE", "AL_RAW_I"])

    for surface, expected in {
        "shek": ["SH_MOIST", "E_MIDDLE", "K_HOT"],
        "shear": ["SH_MOIST", "E_MIDDLE", "AR_FRACTION_I"],
        "shedaiin": ["SH_MOIST", "E_MIDDLE", "D_MEASURE", "AIIN_III"],
        "dals": ["D_MEASURE", "AL_RAW_I", "S_TERM_SPECIES"],
        "dalar": ["D_MEASURE", "AL_RAW_I", "AR_FRACTION_I"],
        "ro": ["R_ROOT", "O_PREP"],
        "oeees": ["O_PREP", "EEE_LONG_OR_FINAL", "S_TERM_SPECIES"],
    }.items():
    ...
    check("V46 glossary adds exactly 37 targets", len(glossary) == len(base_glossary) + 37)
    ...
    check("frontier dimensions", len(frontier) == len(base_frontier) == 37)
    ...
    check("frontier completely closed", all(
        ...
        and not OPEN_MARKER.search(row["v46_translation_de"])
        and not GENERIC.search(row["v46_translation_de"])
        for row in frontier
    ))
    ...
    for rank in map(str, range(1, 21)):
        ...
        check(f"manual V46 complete {rank}", row["v46_unknown_tokens"] == "0"
              and row["locus"] in complete_by_locus)
        check(f"manual automatic renderer identity {rank}", row["automatic_v46_translation_de"]
              == complete_by_locus[row["locus"]]["working_translation_de"])
        check(f"manual and automatic channels distinct {rank}", "manual workshop prose versus deterministic V46 renderer" in row["comparison_scope"])
        check(f"manual prose concrete {rank}", not GENERIC.search(row["manual_workshop_translation_de"])
              ...
              and not GENERIC.search(row["automatic_v46_translation_de"])
              and not OPEN_MARKER.search(row["automatic_v46_translation_de"]))
    ...
    check("non-target position census", len(non_target_before) == 32339 - 165, len(non_target_before))
    ...
    check("known delta exactly 165", final_metrics["known_token_positions"] - base_metrics["known_token_positions"] == 165)
    check("unknown delta exactly 165", base_metrics["unknown_token_positions"] - final_metrics["unknown_token_positions"] == 165)
    check("V45 base dimensions", len(base_dictionary) == 2022 and len(base_glossary) == 1350
          and len(base_coverage) == 4128 and len(base_complete) == 1153 and len(base_one) == 180)
    check("V46 dictionary adds exact defaults and context renderings",
    ...
    check("round versions", [row["version"] for row in rounds] == ["V45", "V46"])
    ...
    check("result schema", result["schema"] == "GDT669_THIRTY_SEVEN_RESIDUAL_FAMILY_COMPLETION_RESULT_V1")
    check("result status", result["status"] == "PASS_165_TARGET_POSITIONS__V46_CONCRETE_RECIPE_REGISTER")
    check("result target dimensions", result["targets"]["surface_types"] == 37 and result["targets"]["positions"] == 165)
    check("result target counts", result["targets"]["surface_counts"] == EXPECTED_COUNTS)
    check("result frontier dimensions", result["frontier"]["source_rows"] == result["frontier"]["completed_rows"] == 37
    ...
    check("result dictionary dimensions", result["working_dictionary"]["v45_entries"] == len(base_dictionary) and result["working_dictionary"]["v46_entries"] == len(dictionary))
    check("result glossary dimensions", result["working_dictionary"]["v45_glossary_surfaces"] == len(base_glossary) and result["working_dictionary"]["v46_glossary_surfaces"] == len(glossary))
    ...
    check("result card-synthesis dimensions", result["card_synthesis"]["productive_compositions"] == 35
          and result["card_synthesis"]["learned_exact_wholes"] == 2
          and result["card_synthesis"]["learned_exact_surfaces"] == sorted(learned_wholes)
          and result["card_synthesis"]["inherited_stem_roles_reused"] == 54
          and result["card_synthesis"]["total_stem_roles"] == 56
          and result["card_synthesis"]["new_stem_roles"] == 2
          and result["card_synthesis"]["new_stem_role"] == "OY_PREP_BASE|OKY_HOT_PREP_BASE"
    ...
    check("result manual-passage dimensions", result["manual_passages"]["rows"] == 20
          ...
          and result["manual_passages"]["v46_complete_rows"] == 20
    ...
    with tempfile.TemporaryDirectory(prefix="gdt669_validator_replay_") as directory:
    ...
    validation = {
        "schema": "GDT669_VALIDATION_V1", "experiment_id": "GDT669",
    ...
    print(f"GDT669 validation: {validation['status']} {validation['checks_passed']}/{validation['checks_total']}")
    if failures:
