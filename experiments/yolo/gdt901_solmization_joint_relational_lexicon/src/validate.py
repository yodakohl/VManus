...
def main():
 ...
 if rp.exists():
  ...
  for c in r['cases']:
   dp=BASE/'artifacts'/c['artifact'];assert sha(dp)==c['artifact_sha256'];d=json.loads(gzip.decompress(dp.read_bytes()))
  ...
  independent=json.loads((BASE/'artifacts/INDEPENDENT_ROLE_DOMAINS.json').read_text())
  assert independent['source_sha256']==sha(sp) and independent['target_sha256']==sha(tp) and independent['model_spec_sha256']==sha(mp)
  assert len(independent['cases'])==10
  for case in independent['cases']:
   primary=json.loads(gzip.decompress((BASE/'artifacts'/f"DOMAINS_IT2a_{case['case_index']:02}.json.gz").read_bytes()))
   assert primary['domains']==case['domains']
