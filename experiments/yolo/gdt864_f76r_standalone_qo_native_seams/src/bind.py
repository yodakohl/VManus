...
up=['experiments/yolo/gdt863_standalone_qo_immediate_echo/artifacts/OCCURRENCES.json','docs/visual_overview/FIGURE_ORIENTATION_2026-09-06_SOURCES.json']
lock=E/'src/PREREG_LOCK.json'
if a.register:
    assert not lock.exists()
    bound=[*sorted((E/'src').glob('*.py')),E/'src/SPEC.json',E/'artifacts/SOURCE_LINES.json',E/'METHOD.md',E/'PREREGISTRATION.md']+[ROOT/n for n in up]
...
m.update(title='Two f76r standalone qo native seams',question='Do native viewers confirm space-like seams at two fixed exposed standalone qo examples?',claim_ceiling='Local same-photo spacing only; no authorial word boundary, semantics or software-verified alignment',status=status,dependencies=['GDT863'],inputs=[{'path':n,'sha256':sha(ROOT/n),'role':'frozen_source_or_extractor'} for n in up],outputs=[{'path':p.relative_to(ROOT).as_posix(),'sha256':sha(p),'role':'primary_report' if p.name=='REPORT.md' else 'source_or_artifact'} for p in outputs],validation={'status':validation_status,'artifact':rel+'/artifacts/VALIDATION.json' if validation_file.exists() else None})
