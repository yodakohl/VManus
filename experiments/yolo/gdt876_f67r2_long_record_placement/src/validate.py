...
for name in ['VIEWER_A','VIEWER_B']:
 ...
 for locus,obs in o['observations'].items():
  ...
  assert obs['region'] in {'LEFT_CIRCULAR_BAND','RIGHT_CIRCULAR_BAND','RIGHT_BELOW_HORIZONTAL','OTHER','UNRESOLVED'}
  if not obs['localized']:assert obs['region']=='UNRESOLVED'
seal=json.loads((B/'artifacts/A_SEAL.json').read_text());assert sha(B/'artifacts/VIEWER_A.json')==seal['sha256']
v={'status':'PASS','coverage':'Source hashes, frozen protocol, three-locus reading and observation coverage, root seal. Native judgements not validated by software.','vision_verified_by_software':False};(B/'artifacts/VALIDATION.json').write_text(json.dumps(v,sort_keys=True)+'\n');print(json.dumps(v))
