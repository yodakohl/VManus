#!/usr/bin/env python3
"""Independent source-first release validator for GDT666."""
...
BASE = Path("experiments/yolo/gdt666_one_hundred_fifty_one_residual_family_completion")
...
CANDIDATE_SPECS = tuple(ROOT / BASE / "src" / name for name in (
    ...
    "HISTORICAL_CODEBOOK_ANALOG_MEMO.md",
    "FINAL_CARD_AUDIT.md",
    "OLY_FAMILY_AUDIT.md",
    "G_FREE_CONTEXT_AUDIT.md",
...
OUTPUT_NAMES = (
    ...
    "FAMILY_COMPOSITION_ATLAS.tsv", "STEM_MODEL_V43.tsv",
    "INHERITED_OL_RENDER_REVISIONS.tsv", "INHERITED_SOL_RENDER_REVISIONS.tsv",
    "MANUAL_PASSAGE_AUDIT.tsv",
    "FRONTIER_151_COMPLETIONS.tsv", "TARGET_LINE_TRANSLATIONS.tsv",
    "ROUND_COVERAGE_COUNTS.tsv", "NEWLY_COMPLETED_LINES.tsv",
    "NEWLY_EXPOSED_ONE_HOLE_LINES.tsv", "V43_WORKING_TOKEN_GLOSSARY.tsv",
    "WORKING_DICTIONARY_V43.tsv", "ALL_LINE_CONCRETE_COVERAGE_V43.tsv",
    "COMPLETE_PASSAGES_V43.tsv", "ONE_UNKNOWN_PASSAGES_V43.tsv",
)

TARGET_ORDER = tuple("""
ctharal kochor olkeeshy olan oteaiin otealshey otaly otolkeechdy eeeodaiin alol
lkshedy raraiin cheodar keam dalchd chcthey dcheoty chdam sokal otcheody cheedain
chot ysheed rodam ykeeochody chky chytshy ytol kch qopchaiin choltaiin shdaiin
choees okoiin tolshy ysheol keeees ykol kcheor ychor qoaiin oaldary g chcth daid
qochol cthody qoos yor dkol chm ctholdy deey dshodar chekar arl yshos dchold
ochor kcholqod orchochor chokokor dchdy ycheor chag chololy chekody ay dalain
chtchy qokody qody om keesy qocthedy chosory koddy ycheoky chokan qockhey ke
keeyfar daiiny chokor chotchey chotor skaiiodar yodaiin choldar qoked polshy
koldy ld dlshedy qokalchey qopchey qochy olsheed chcphedy checthey qolal solkeey
dsheedal lchor ldchey cheolkaiin dalky ykedy qoke qolkeshdy otaldy oltal eey
scthey chckhes okaio cheokain oroiiin yefaiin ycheckhey dshoy cthd dainol okalol
cheykain okalchy qolky ldol qopchedy dsholyd qokeoy dytar qolchedy qotee yteeedy
yteeo daiinls acthy chokar chcfhy qoeear qoteeos chotain darala shodaiin odor
cholky ykoldy qokeol kyty daim
""".split())
...
def main() -> int:
    ...
    # guarded selector before trusting any GDT666 result or generated artifact.
    base_pages = {row["page"] for row in read_tsv(ROOT / G665 / "artifacts/PAGE_ALLOWLIST.tsv")}
    ...
    check("target count sum", sum(EXPECTED_COUNTS.values()) == 612)
    ...
    check("CARD_SPECS 151 unique forms", len(specs) == len(set(spec_order)) == 151)
    check("CARD_SPECS all fields populated", all(all(row[field] for field in spec_fields) for row in specs))
    check("CARD_SPECS raw positions", sum(raw_counts[row["surface"]] for row in specs) == 612)
    ...
    check("stem-model source 48 rows", len(stem_specs) == 48)
    ...
    stem_model = read_tsv(ART / "STEM_MODEL_V43.tsv")
    ...
    frontier = read_tsv(ART / "FRONTIER_151_COMPLETIONS.tsv")
    target_lines = read_tsv(ART / "TARGET_LINE_TRANSLATIONS.tsv")
    rounds = read_tsv(ART / "ROUND_COVERAGE_COUNTS.tsv")
    glossary = read_tsv(ART / "V43_WORKING_TOKEN_GLOSSARY.tsv")
    dictionary = read_tsv(ART / "WORKING_DICTIONARY_V43.tsv")
    coverage = read_tsv(ART / "ALL_LINE_CONCRETE_COVERAGE_V43.tsv")
    complete = read_tsv(ART / "COMPLETE_PASSAGES_V43.tsv")
    one = read_tsv(ART / "ONE_UNKNOWN_PASSAGES_V43.tsv")
    ...
    base_glossary = read_tsv(base_art / "V42_WORKING_TOKEN_GLOSSARY.tsv")
    ...
    check("decision dimensions", len(deck) == len(accepted) == len(family) == 151)
    ...
    check("architecture position total", sum(int(row["positions"]) for row in architecture) == 612)

    audit_counts = Counter(row["surface"] for row in audit)
    check("occurrence audit dimensions", len(audit) == len(reader) == 612)
    check("occurrence audit exact counts", dict(audit_counts) == EXPECTED_COUNTS, dict(audit_counts))
    check("occurrence IDs are G666", all(row["occurrence_id"].startswith("G666-") for row in audit))
    check("all target slots were V42-open", all(row["v42_gloss_de"] == f"[{row['surface']}:?]" for row in audit))
    check("all target slots are V43-filled", all(
        row["v43_gloss_de"] and not OPEN_MARKER.search(row["v43_gloss_de"]) for row in audit
    ))
    ...
    check("audit practical prose has no generic work filler", not any(
        GENERIC.search(row["v43_working_translation_de"]) for row in audit
    ))
    check("target-line practical prose has no generic work filler", not any(
        GENERIC.search(row["v43_working_translation_de"]) for row in target_lines
    ))
    ...
    check("only named GDT666 boundary forms reader-dispatch", all(
        row["surface"] in CONTEXT_SURFACES for row in merge_rows
    ))

    check("stem-model artifact byte-equivalent to source", (ART / "STEM_MODEL_V43.tsv").read_bytes() == STEM_SPECS.read_bytes())
    stems_by_key = {(row["stem"], row["structural_role"]): row for row in stem_model}
    check("stem-model artifact 48 rows", len(stem_model) == len(stems_by_key) == 48)
    ...
    check("final synthesis is 129 role-composed plus 22 learned", len(learned_wholes) == 22
          and len(specs) - len(learned_wholes) == 129)
    ...
    check("OLY is a narrow terminal learned action block", oly_actual == {"chololy"}
          and card_atoms["chololy"] == ["CH_DRY", "OL_MATERIAL", "OLY_STRAIN_ACTION"]
          and specs_by_surface["chololy"]["working_meaning_de"] == "seihe den getrockneten Drogenstoff ab")
    ...
    check("g stem restricted to exact three cards", g_stem["practical_default_de"] == "ein Gran"
          ...
          and specs_by_surface["g"]["composition"] == "LEARNED_G_WHOLE"
          and specs_by_surface["g"]["working_meaning_de"] == "vorstehenden Rezeptposten abschließen")
    ...
    check("V43 glossary adds exactly 151 targets", len(glossary) == len(base_glossary) + 151)
    ...
    check("frontier completely closed", all(
        ...
        and not OPEN_MARKER.search(row["v43_translation_de"])
        and not GENERIC.search(row["v43_translation_de"])
        for row in frontier
    ))
    ...
    for rank in map(str, range(1, 31)):
        ...
        check(f"manual V43 complete {rank}", row["v43_unknown_tokens"] == "0"
              and row["locus"] in complete_by_locus)
        check(f"manual automatic renderer identity {rank}", row["automatic_v43_translation_de"]
              == complete_by_locus[row["locus"]]["working_translation_de"])
        check(f"manual and automatic channels distinct {rank}", "manual workshop prose versus deterministic V43 renderer" in row["comparison_scope"])
        check(f"manual prose concrete {rank}", not GENERIC.search(row["manual_workshop_translation_de"])
              ...
              and not GENERIC.search(row["automatic_v43_translation_de"])
              and not OPEN_MARKER.search(row["automatic_v43_translation_de"]))
    ...
    check("non-target position census", len(non_target_before) == 32339 - 612, len(non_target_before))
    ...
    check("known delta exactly 612", final_metrics["known_token_positions"] - base_metrics["known_token_positions"] == 612)
    check("unknown delta exactly 612", base_metrics["unknown_token_positions"] - final_metrics["unknown_token_positions"] == 612)
    ...
    check("round versions", [row["version"] for row in rounds] == ["V42", "V43"])
    ...
    check("result schema", result["schema"] == "GDT666_ONE_HUNDRED_FIFTY_ONE_RESIDUAL_FAMILY_COMPLETION_RESULT_V1")
    check("result status", result["status"] == "PASS_612_TARGET_POSITIONS__V43_CONCRETE_RECIPE_REGISTER")
    check("result target dimensions", result["targets"]["surface_types"] == 151 and result["targets"]["positions"] == 612)
    ...
    check("result dictionary dimensions", result["working_dictionary"]["v42_entries"] == len(base_dictionary) and result["working_dictionary"]["v43_entries"] == len(dictionary))
    check("result glossary dimensions", result["working_dictionary"]["v42_glossary_surfaces"] == len(base_glossary) and result["working_dictionary"]["v43_glossary_surfaces"] == len(glossary))
    ...
    check("result stem-model dimensions", result["stem_model"]["rows"] == 48
          and result["stem_model"]["structural_roles_distinct_from_german_defaults"] is True)
    check("result card-synthesis dimensions", result["card_synthesis"]["productive_compositions"] == 129
          and result["card_synthesis"]["learned_exact_wholes"] == 22
          and result["card_synthesis"]["learned_exact_surfaces"] == sorted(learned_wholes)
          and result["card_synthesis"]["inherited_stem_roles_reused"] == 47
          and result["card_synthesis"]["total_stem_roles"] == 48
          and result["card_synthesis"]["new_stem_roles"] == 1
          and result["card_synthesis"]["new_stem_role"] == "OLY_STRAIN_ACTION"
          and result["card_synthesis"]["candidate_lenses"]
          == ["stem", "recipe", "passage", "historical_workshop"])
    check("result manual-passage dimensions", result["manual_passages"]["rows"] == 30
          ...
          and result["manual_passages"]["v43_complete_rows"] == 30
    ...
    with tempfile.TemporaryDirectory(prefix="gdt666_validator_replay_") as directory:
    ...
    validation = {
        "schema": "GDT666_VALIDATION_V1", "experiment_id": "GDT666",
    ...
    print(f"GDT666 validation: {validation['status']} {validation['checks_passed']}/{validation['checks_total']}")
    if failures:
