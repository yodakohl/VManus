...
def replay(parse,variant,s,independent=False):
    ...
    certificate=inspect_certificate(parse,independent)
    old=any(p['consistent'] for p in paths)
    return dict(status='COHERENT' if old and certificate['valid'] else 'CONTRADICTED',cargo=cargo,hazards=hazards,references=refs,paths=paths,certificate=certificate,old_existential=old)
