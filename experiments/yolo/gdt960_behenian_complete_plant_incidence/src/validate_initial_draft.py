...
def z3_upper(req,domains,unknown):
    ...
    for row,cap in enumerate(unknown):
        cost=[]
        for p in plants:
            terms=[z3.If(vs[p]==code,1 if (req[p]&(1<<row)) and not (km&(1<<row)) else 0,0) for code,km,_ in choices[p]]
            cost.append(z3.Sum(terms) if terms else z3.IntVal(0))
        constraints.append(z3.Sum(cost)<=cap)
    sol=z3.Solver();sol.add(*constraints);status=sol.check();feas={p:[] for p in plants}; witness={}
    if status==z3.sat:
        model=sol.model(); witness={p:next(word for code,km,word in choices[p] if model.eval(vs[p]).as_long()==code) for p in plants}
        for p in plants:
            for code,_,word in choices[p]:
                sol.push();sol.add(vs[p]==code);feas[p].append((word,sol.check()==z3.sat));sol.pop()
...
def main():
    ...
    cases=[];pred_rows=[]; source_names={i:x['name'] for i,x in enumerate(load(G915/'../..'/'gdt957_f66r_complete_geomantic_margin/src/SOURCE.json')['figures'])} if False else None
    for w in windows:
      for model in MOD:
       incidence=models[model]['incidence']; plants=list(incidence)
       for direction in DIR:
        ...
        lower=1
        for mask,count in defaultdict(int).items():pass
        ...
        for p in plants:
            ...
            pred_rows.append({'case_id':cid,'plant':p,'required_rows':str(req[p].bit_count()),'known_support_forms':' | '.join(known_domains[p]),'upper_domain_forms':' | '.join(x[0] for x in ds),'feasible_upper_forms':' | '.join(feasible_forms),'known_support_assignment_count':str(lower),'case_status':status})
    ...
    for model in MOD:
      ...
      eq[model]=[{'rows':list(k),'indistinguishable_source_names':v} for k,v in sorted(bymask.items(),key=lambda kv:(min(kv[0]),len(kv[0]),kv[0]))]
