...
up=['experiments/yolo/gdt848_f104_visual_e_run_audit/artifacts/SOURCES.json']
...
m.update(title='f104 material landmark mirror feasibility',question='Are three distinctive nonink material landmark pairs available for a later independently decided mapping phase?',claim_ceiling='Native material matching feasibility only; no target counterpart comparison, show-through, glyph or meaning claim',status=status,dependencies=['GDT848'],inputs=[{'path':n,'sha256':sha(ROOT/n),'role':'frozen_source_or_extractor'} for n in up],outputs=[{'path':p.relative_to(ROOT).as_posix(),'sha256':sha(p),'role':'primary_report' if p.name=='REPORT.md' else 'source_or_artifact'} for p in outputs],validation={'status':validation_status,'artifact':rel+'/artifacts/VALIDATION.json' if validation_file.exists() else None})
