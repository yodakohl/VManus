...
up=['experiments/yolo/gdt850_qolchedy_join_split_context_inventory/REPORT.md', 'experiments/yolo/gdt851_primitive_tandem_raw_group_discovery/REPORT.md', 'experiments/yolo/gdt851_primitive_tandem_raw_group_discovery/artifacts/SOURCE_ZL3b.json', 'experiments/yolo/gdt851_primitive_tandem_raw_group_discovery/artifacts/SOURCE_IT2a.json', 'experiments/yolo/gdt851_primitive_tandem_raw_group_discovery/artifacts/SOURCE_RF1b.json']
...
m.update(title='Native same-line internal spacing audit',question='Do two fixed f75v.44 sequences differ locally in internal spacing beyond whole-span scaling?'     ,claim_ceiling='Exploratory native layout only; no word meaning or independently verified semantic relation.',status=status,dependencies=['GDT850','GDT851'],inputs=[{'path':n,'sha256':sha(ROOT/n),'role':'frozen_source_or_extractor'} for n in up],outputs=[{'path':p.relative_to(ROOT).as_posix(),'sha256':sha(p),'role':'primary_report' if p.name=='REPORT.md' else 'source_or_artifact'} for p in outputs],validation={'status':'PASS' if result.exists() else 'NOT_RUN','artifact':rel+'/artifacts/VALIDATION.json' if result.exists() else None})
