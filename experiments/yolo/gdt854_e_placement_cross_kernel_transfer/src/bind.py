...
up=['experiments/yolo/gdt851_primitive_tandem_raw_group_discovery/experiment.json', 'experiments/yolo/gdt851_primitive_tandem_raw_group_discovery/src/SPEC.json']
...
m.update(title='Single-e placement cross-kernel context transfer',question='Does one-e placement predict written neighbors across the other kernel and other physical folios beyond a whole-folio polarity null?'     ,claim_ceiling='Written-context directional transfer under folio sign-exchangeability only; no causal, unique-function, language, physical-glyph or meaning claim.',status=status,dependencies=['GDT849','GDT851','GDT853'],inputs=[{'path':n,'sha256':sha(ROOT/n),'role':'frozen_source_or_extractor'} for n in up],outputs=[{'path':p.relative_to(ROOT).as_posix(),'sha256':sha(p),'role':'primary_report' if p.name=='REPORT.md' else 'source_or_artifact'} for p in outputs],validation={'status':'PASS' if result.exists() else 'NOT_RUN','artifact':rel+'/artifacts/VALIDATION.json' if result.exists() else None})
source_spec=json.loads((E/'src/SPEC.json').read_text())
m['inputs'].append({'path':source_spec['source'],'sha256':source_spec['source_sha256'],'role':'published_source_hash_from_GDT851_manifest_checked_on_GO'})
