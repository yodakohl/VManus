...
def solve(paragraphs,lex,g,timeout=20000,witness_limit=2,family="FUNCTIONAL",blocked_layouts=None):
    ...
    for layout in blocked_layouts or []:
        keys={(c['start'],c['end'],c['kind']) for c in layout}
        chosen=[b for key,b in zip(edges[0],flags[0]) if key in keys]
        if len(chosen)!=len(layout):continue  # A pinned-incompatible edge already excludes this layout.
        s.add(z3.Or([z3.Not(b) for b in chosen]))
