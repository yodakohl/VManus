#!/usr/bin/env python3
"""Build V89 by repairing thirteen exact indexed-share cores."""
...
def rename_v88_fields(row: dict[str, str]) -> dict[str, str]:
    return {v89_key(key): value for key, value in row.items()}


def family_v89_key(value: str) -> str:
    return value.replace("v84", "v89").replace("V84", "V89")
...
def resolve_evidence(
    ...
    bonus = {row["family_id"]: int(row["family_bonus"]) for row in source_families}
    deltas: dict[str, int] = {}
    for spec in specs:
        ...
        deltas[source_id] = sum(bonus[family] for family in families)
    ...
    for source in source_rows:
        ...
        if spec:
            ...
            row.update({
                ...
                "source_gdts": append_pipe(source["source_gdts"], "GDT716"),
                "positive_evidence_de": (
                    "GDT716 bindet die indexierte Anteilsleiter und trennt den lokalen Referenten: "
                    + spec["evidence_de"] + " || " + source["positive_evidence_de"]
                ),
                "counterevidence_de": (
                    "GDT716 Gegenbeleg: " + spec["counterevidence_de"]
                ...
                "v89_prior_lexical_core_de": source["v88_lexical_core_de"],
            })
        else:
            row.update({
                "v89_audit_decision": "NOT_IN_GDT716_TRANCHE",
                "v89_evidence_class": "INHERITED_V88",
                "v89_open_semantic_slots": "NOT_EVALUATED",
                "v89_component_global_export_allowed": "NOT_EVALUATED",
                "v89_prior_lexical_core_de": source["v88_lexical_core_de"],
            })
        output.append(row)
...
def build_contexts(
    ...
    for source in source_rows:
        ...
        row.update({
            "v89_reading_id": lexical["v89_reading_id"],
            "v89_lexical_core_de": lexical["v89_lexical_core_de"],
            "v89_context_realization_de": spec["v89_context_realization_de"] if spec else source["v88_context_realization_de"],
            "v89_repair_mode": spec["repair_mode"] if spec else source["v88_repair_mode"],
            "v89_resolved_debt_atom": spec["resolved_debt_atom"] if spec else source["v88_resolved_debt_atom"],
            "v89_lexical_score": lexical["working_model_score_0_100_not_probability"],
            "v89_lexical_level": lexical["working_model_level"],
            "v89_context_score": lexical["context_realization_score_0_100_not_probability"],
            "v89_context_level": lexical["context_realization_level"],
            "v89_semantic_scope": lexical["semantic_scope"],
            "v89_semantic_applicability": lexical["semantic_applicability"],
            "v89_global_export_scope": lexical["global_export_scope"],
            "v89_lexical_bound_span_ids": lexical["bound_span_ids"],
            "v89_unconditional_global_export_allowed": lexical["unconditional_global_export_allowed"],
            "v89_historical_confirmation": HISTORICAL,
            "v89_audit_decision": "REVISE" if spec else "NOT_IN_GDT716_TRANCHE",
            "v89_evidence_class": spec["evidence_class"] if spec else "INHERITED_V88",
            "v89_open_semantic_slots": spec["open_semantic_slots"] if spec else "NOT_EVALUATED",
            "v89_component_global_export_allowed": "0" if spec else "NOT_EVALUATED",
            "v89_local_context_hypothesis": spec["local_context_hypothesis"] if spec else "NONE",
            "v89_expected_left_surface": spec["expected_left_surface"] if spec else "NONE",
        })
        output.append(row)
    ...
    for source in source_rows:
        ...
        if spec:
            ...
            row.update({
                "v89_reading_id": lexical["v89_reading_id"],
                "v89_lexical_core_de": lexical["v89_lexical_core_de"],
                "v89_context_realization_de": lexical["v89_context_realizations_de"],
                "new_lexical_score": lexical["working_model_score_0_100_not_probability"],
                "new_lexical_level": lexical["working_model_level"],
                "new_context_score": lexical["context_realization_score_0_100_not_probability"],
                "new_context_level": lexical["context_realization_level"],
                "v89_audit_decision": "HELD_FOR_LATER_REPAIR",
                "v89_evidence_class": "INHERITED_V88",
                "v89_open_semantic_slots": "NOT_EVALUATED",
            })
        output.append(row)
...
def build_delta(
    ...
    for spec in specs:
        ...
        output.append({
            ...
            "old_lexical_core_de": source["v88_lexical_core_de"],
            "v89_lexical_core_de": target["v89_lexical_core_de"],
            "v89_context_realization_de": spec["v89_context_realization_de"],
            "old_score": source["working_model_score_0_100_not_probability"],
            "v89_score": target["working_model_score_0_100_not_probability"],
            "old_level": source["working_model_level"],
            "v89_level": target["working_model_level"],
            "family_ids": spec["family_ids"],
...
def build_family(
    ...
    source = next(row for row in source_families if row["family_id"] == "F_R")
    row: dict[str, Any] = {family_v89_key(key): value for key, value in source.items()}
    row.update({
        "selected_source_readings": len(specs),
        "selected_lexical_readings": len(specs),
        "selected_positions": len(specs),
        "selected_pages": len({spec["expected_page"] for spec in specs}),
        "selected_source_reading_ids": ordered_compact(spec["source_reading_id"] for spec in specs),
        "selected_surfaces": ordered_compact(target_by_source[spec["source_reading_id"]]["surface"] for spec in specs),
        "selected_v89_reading_ids": ordered_compact(target_by_source[spec["source_reading_id"]]["v89_reading_id"] for spec in specs),
        "selected_v89_levels": ordered_compact(target_by_source[spec["source_reading_id"]]["working_model_level"] for spec in specs),
        "automatic_historical_credit": 0,
        "historical_confirmation": HISTORICAL,
        "v89_decisions": "REVISE",
    })
    return [row]
...
def build_complete(source_rows: list[dict[str, str]], lexical: list[dict[str, Any]]) -> list[dict[str, Any]]:
    output: list[dict[str, Any]] = []
    for source in source_rows:
        if source["current_layer"] == "ACTIVE_V88_LEXICAL_CORE":
            continue
        row: dict[str, Any] = rename_v88_fields(source)
        row.update({
            "v89_audit_decision": "OUTSIDE_ACTIVE_V89_TRANCHE",
            "v89_evidence_class": "INHERITED_GLOBAL_V48",
            "v89_open_semantic_slots": "NOT_EVALUATED",
            "v89_component_global_export_allowed": "NOT_EVALUATED",
        })
        output.append(row)
    for row in lexical:
        output.append({
            "surface": row["surface"], "reading_id": row["v89_reading_id"],
            "working_meaning_de": row["v89_lexical_core_de"], "current_layer": "ACTIVE_V89_LEXICAL_CORE",
            ...
            "v89_context_realizations_de": row["v89_context_realizations_de"],
            "source_reading_ids": row["source_reading_ids"], "v89_audit_decision": row["v89_audit_decision"],
            "v89_evidence_class": row["v89_evidence_class"], "v89_open_semantic_slots": row["v89_open_semantic_slots"],
            "v89_component_global_export_allowed": row["v89_component_global_export_allowed"],
        })
...
def render_f7r2(contexts: list[dict[str, Any]], directives: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for source in source_rows:
        ...
        if directive:
            ...
            rendered = source["v89_context_realization_de"]
...
def report_text(result: dict[str, Any], delta: list[dict[str, Any]]) -> str:
    table = "\n".join(
        f"| `{row['surface']}` | {row['v89_lexical_core_de']} | {row['v89_context_realization_de']} | {row['old_score']}→{row['v89_score']} |"
        for row in delta
    )
    return f"""# GDT716 — V89 indexed-share core/context repair

Status: `{STATUS}`

## Ergebnis

Die dreizehn Karten folgen jetzt derselben vorhersagbaren Regel: R waehlt den
indexierten Anteil; Zustand, Rahmen oder Aktion bleiben sichtbar; ein konkreter
Stoffpatient steht nur im exakten Kontext.

| Form | portabler Kern | konkrete Fundstelle | Score |
|---|---|---|---:|
{table}

`oar/oair` bilden Zubereitungsanteil I/II, `char/chair` den trockenen Anteil
I/II. `otarar` behaelt beide sichtbaren R-Schritte als Anteil und Unteranteil.
`qodar`, `qokaiir` und `qochar` behalten ihre exakten Verben. Kein Kern braucht
mehr den generischen Kopf "Droge".

Jede Lesung erhaelt genau einmal `F_R +8`; alle dreizehn bleiben W1. Es gibt
keinen Bonus fuer geloeschte Substantive, flüssigere Saetze oder Aktionsprosa.

## Bestand

- 13 Lesungen / 13 Positionen / 8 Seiten
- {result['primary_evidence_bindings']} exakt wiederholte Primaerevidenzbindungen
- 311 nicht betroffene Kerne und 466 nicht betroffene Kontexte unveraendert
- beide Spans und acht f7r.2-Ausgabeeinheiten erhalten
- V89 aktiv: `{json.dumps(result['active_level_counts'], ensure_ascii=False, sort_keys=True)}`
- komplettes Woerterbuch: {result['complete_readings']} Lesungen; jede mit
  Score, Confidence-Stufe, Evidenz und Gegenbeleg
- verbleibende schwache Einzelreparaturen: {result['remaining_unreviewed_weak_readings']}

## Grenze

Anteil ist ein gebundener Arbeitswert der exakten Karten, kein bestaetigtes
historisches Wort und kein frei exportiertes `ar`. Lokale Kraut-, Holz-,
Portions- und Zubereitungsbezüge sind Fundstellenhypothesen. Alle historischen
...
def main() -> int:
    ...
    assert (len(source_lexical), len(source_context), len(source_complete), len(source_census), len(source_spans), len(source_families), len(specs), len(bindings)) == (324, 479, 1586, 91, 2, 13, 13, 29)
    ...
    families = build_family(source_families, specs, lexical_by_source)
    ...
    assert len(census) == 84 and Counter(row["disposition"] for row in census) == Counter({"HELD_FOR_LATER_REPAIR": 71, "REVISED_IN_V89": 13})
    assert len(families) == 1 and families[0]["family_id"] == "F_R"
    ...
    lexical_fields = unique_fields([v89_key(field) for field in source_lexical[0]] + ["v89_audit_decision", "v89_evidence_class", "v89_open_semantic_slots", "v89_component_global_export_allowed", "v89_prior_lexical_core_de"])
    context_fields = unique_fields([v89_key(field) for field in source_context[0]] + ["v89_audit_decision", "v89_evidence_class", "v89_open_semantic_slots", "v89_component_global_export_allowed", "v89_local_context_hypothesis", "v89_expected_left_surface"])
    census_fields = unique_fields([v89_key(field) for field in source_census[0]] + ["v89_audit_decision", "v89_evidence_class", "v89_open_semantic_slots"])
    family_fields = unique_fields([family_v89_key(field) for field in source_families[0]] + ["v89_decisions"])
    complete_fields = unique_fields([v89_key(field) for field in source_complete[0]] + ["v89_audit_decision", "v89_evidence_class", "v89_open_semantic_slots", "v89_component_global_export_allowed"])

    write_tsv(ART / "V89_324_ACTIVE_LEXICAL_READINGS.tsv", lexical_fields, lexical)
    write_tsv(ART / "V89_479_CONTEXT_REALIZATIONS.tsv", context_fields, contexts)
    write_tsv(ART / "V89_84_HELD_READING_AUDIT.tsv", census_fields, census)
    write_tsv(ART / "V89_13_INDEXED_SHARE_CORE_CONTEXT_DELTA.tsv", list(delta[0]), delta)
    write_tsv(ART / "V89_29_PRIMARY_EVIDENCE_BINDINGS.tsv", list(evidence[0]), evidence)
    write_tsv(ART / "V89_1_FAMILY_EVIDENCE.tsv", family_fields, families)
    write_tsv(ART / "V89_2_BOUND_SPAN_RENDERER.tsv", list(spans[0]), spans)
    write_tsv(ART / "V89_2_ONE_SHOT_RENDER_DIRECTIVES.tsv", list(directives[0]), directives)
    write_tsv(ART / "V89_8_F7R2_RENDERED_UNITS.tsv", list(rendered[0]), rendered)
    write_tsv(ART / "V89_COMPLETE_WORD_CONFIDENCE.tsv", complete_fields, complete)
    ...
    result = {
        ...
        "primary_evidence_bindings": len(evidence), "score_family": "F_R",
        "score_delta_each": 8, "score_promotions": {row["source_reading_id"]: deltas[row["source_reading_id"]] for row in specs},
        "active_lexical_readings": len(lexical), "active_positions": len(contexts),
        "active_level_counts": dict(sorted(active_levels.items())),
        "remaining_unreviewed_weak_readings": sum(row["v89_audit_decision"] == "HELD_FOR_LATER_REPAIR" for row in census),
