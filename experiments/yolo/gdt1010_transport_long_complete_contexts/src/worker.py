...
def main():
    ...
    try:
        if mode=='primary':
            import solver
            out=solver.solve(j['paragraphs'],{},g,timeout=s['query_milliseconds'],witness_limit=s['witness_limit'],family='FUNCTIONAL')
        elif mode=='independent':
            import independent
            out=independent.check(j['paragraphs'],{},g,timeout=s['independent_milliseconds'],family='FUNCTIONAL')
        else:
