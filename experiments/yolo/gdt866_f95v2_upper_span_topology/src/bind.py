...
up=['docs/visual_overview/ORIENTATION_2026-09-06_SOURCES.json','docs/visual_overview/ORIENTATION_2026-09-06.md']
lock=E/'src/PREREG_LOCK.json'
if a.register:
    assert not lock.exists()
    bound=[*sorted((E/'src').glob('*.py')),E/'src/SPEC.json',E/'SOURCES.json',E/'METHOD.md',E/'PREREGISTRATION.md']+[ROOT/n for n in up]
...
m.update(title='f95v2 upper span native topology',question='Does the fixed upper span link two tall uprights across a complete intervening visually separated group?',claim_ceiling='One native same-photo descriptive topology only; no glyph, abbreviation, meaning or pen-motion inference',status=status,dependencies=['GDT859','GDT861'],inputs=[{'path':n,'sha256':sha(ROOT/n),'role':'frozen_source_or_extractor'} for n in up],outputs=[{'path':p.relative_to(ROOT).as_posix(),'sha256':sha(p),'role':'primary_report' if p.name=='REPORT.md' else 'source_or_artifact'} for p in outputs],validation={'status':validation_status,'artifact':rel+'/artifacts/VALIDATION.json' if validation_file.exists() else None})
