#!/usr/bin/env python3
"""Build GDT664: close the 140-form V40 frontier as concrete V41 recipe cards."""
...
BASE_REL = Path("experiments/yolo/gdt664_one_hundred_forty_residual_family_completion")
ART = ROOT / BASE_REL / "artifacts"
G663 = Path("experiments/yolo/gdt663_one_hundred_two_residual_family_completion")
_spec = importlib.util.spec_from_file_location("gdt663_builder_for_gdt664", ROOT / G663 / "src/run.py")
if _spec is None or _spec.loader is None:
    raise RuntimeError("cannot load GDT663 builder")
g663 = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(g663)
TOKENS_REL, CROSS_REL = g663.TOKENS_REL, g663.CROSS_REL
...
def read_card_specs(path: Path) -> tuple[dict[str, str], ...]:
    ...
    if not rows or set(rows[0]) != expected:
        raise RuntimeError("bad GDT664 CARD_SPECS.tsv columns")
    surfaces = [row["surface"] for row in rows]
    if len(rows) != 140 or len(set(surfaces)) != 140:
        raise RuntimeError("GDT664 CARD_SPECS.tsv must contain 140 unique surfaces")
    return tuple(rows)
...
CONTEXT_SCOPED_SURFACES = frozenset({"o", "ch", "qok", "m", "sa"})
...
LOW_SURFACES = frozenset({
    "raralchl", "ychklkaiin", "o", "oeeesoy", "cfhar", "okaial", "oo", "chokolg",
    "oleeeb", "qodaim", "kcharg", "m", "ajy", "lr", "chkag", "qoeeeety", "qoeey",
})
STRONG_SURFACES = frozenset({
    "opchedy", "qok", "lkaiin", "olkeey", "air", "chodaiin", "orain", "ch", "o",
    "cheky", "ykar", "olchedy", "olshedy", "olchy", "qokeed", "chees", "qokam",
    "ytar", "lky", "olkeedy", "qoeey", "okam",
})
...
qokeo=8 chsy=1 keear=4 dalam=4 dsheol=9 shcthey=4 qorchain=1 teeedy=4 teeodain=1
odys=1 qopchdy=16 qotam=11 okeol=44 sa=3 oeeeody=1 olar=11 air=56 chodaiin=42
choar=2 chorar=1 ykanam=1 orain=16 chery=1 oleey=2 sheed=5 ch=13 qodchy=2 octhol=1
yky=12 chyty=4 dshy=6 o=146 oeeesoy=1 kchochaiin=1 okshor=1 cfhar=1 okaial=1
okeeaiin=1 cthod=3 cthorchy=1 ykeedar=1 dytchor=1 ycheo=9 ked=1 oo=1 qokeeaiin=2
schaiin=1 od=8 chkaly=2 qooldy=1 choror=2 ykchon=1 qoykam=1 daram=6 arol=8 lkol=3
daiidy=5 kchain=1 lchar=3 ykolody=1 dcholdy=1 chokolg=1 dochod=1 oleeeb=1 oshol=1
olkeeody=5 cheky=55 qodaim=1 ykar=31 kcharg=1 qotoeey=2 m=6 ydain=4 ajy=1 oldair=1
qokechdy=3 dara=1 olchedy=34 olshedy=17 olchy=9 cheolchey=2 qolkchey=1 qokeed=16
chees=32 oin=7 qykaiin=1 sokol=1 ykain=11 lshckhy=1 lr=10 qokam=25 solor=1 chkag=1
salchey=1 dytain=1 tedyol=1 dykshy=1 saltar=1 yshey=11 olsheol=3 loly=5 ytar=21
lky=19 salshey=1 olkeedy=42 solkeedy=3 techedy=1 chcthdy=7 cheolkain=1 rolkeedy=1
ydair=2 olpchdy=1 ytas=1 teody=7 chokain=10 qoeeeety=1 qoeey=12 opol=3 chean=2
cheain=2 chodey=1 okam=27 ctheor=4
""")
...
OUTPUT_NAMES = (
    ...
    "FAMILY_COMPOSITION_ATLAS.tsv", "FRONTIER_140_COMPLETIONS.tsv",
    "TARGET_LINE_TRANSLATIONS.tsv", "ROUND_COVERAGE_COUNTS.tsv",
    "NEWLY_COMPLETED_LINES.tsv", "NEWLY_EXPOSED_ONE_HOLE_LINES.tsv",
    "V41_WORKING_TOKEN_GLOSSARY.tsv", "WORKING_DICTIONARY_V41.tsv",
    "ALL_LINE_CONCRETE_COVERAGE_V41.tsv", "COMPLETE_PASSAGES_V41.tsv",
    "ONE_UNKNOWN_PASSAGES_V41.tsv", "INHERITED_OL_RENDER_REVISIONS.tsv",
    "INHERITED_SOL_RENDER_REVISIONS.tsv", "STEM_MODEL_V41.tsv", "MANUAL_PASSAGE_AUDIT.tsv",
)
...
# These are practical render revisions only.  V40's structural tags remain
# byte-visible in its inherited glossary/dictionary; V41 translates the 19
...
def read_ol_revision_specs(path: Path) -> tuple[dict[str, str], ...]:
    ...
    if not rows or set(rows[0]) != expected:
        raise RuntimeError("bad GDT664 INHERITED_OL_REVISION_SPECS.tsv columns")
    surfaces = [row["surface"] for row in rows]
    if len(rows) != 19 or len(set(surfaces)) != 19 or sum(int(row["positions"]) for row in rows) != 256:
        raise RuntimeError("GDT664 OL revision source must contain 19 unique forms at 256 positions")
...
def read_sol_revision_specs(path: Path) -> tuple[dict[str, str], ...]:
    ...
    expected = {"surface", "positions", "practical_meaning_de", "composition", "retained_rival_de"}
    if not rows or set(rows[0]) != expected:
        raise RuntimeError("bad GDT664 INHERITED_SOL_REVISION_SPECS.tsv columns")
    surfaces = [row["surface"] for row in rows]
    if len(rows) != 2 or len(set(surfaces)) != 2 or sum(int(row["positions"]) for row in rows) != 4:
        raise RuntimeError("GDT664 SOL revision source must contain 2 unique forms at 4 positions")
    return tuple(rows)
...
# all token gloss markers at this locus.  V41 preserves that sentence but must
...
def read_stem_model_specs(path: Path) -> tuple[dict[str, str], ...]:
    ...
    if not rows or set(rows[0]) != expected or len(rows) != 36:
        raise RuntimeError("GDT664 STEM_MODEL_SPECS.tsv must contain the fixed 36-row schema")
    return tuple(rows)
...
def read_manual_passage_specs(path: Path) -> tuple[dict[str, str], ...]:
    ...
    if not rows or set(rows[0]) != expected or len(rows) != 30:
        raise RuntimeError("GDT664 MANUAL_PASSAGE_SPECS.tsv must contain the fixed 30-row schema")
    if [int(row["rank"]) for row in rows] != list(range(1, 31)):
        raise RuntimeError("GDT664 manual passage ranks must be 1..30")
    if len({row["locus"] for row in rows}) != 30:
        raise RuntimeError("GDT664 manual passage loci must be unique")
    return tuple(rows)
...
def contextual_merge_candidate(
    ...
    if ordered:
        ...
        if surface == "o":
            ...
            render = f"gib einen Teil Saatgut zu {right_meaning}" if direction == "RIGHT" else f"gib einen Teil Saatgut zu {left_meaning}"
        readers = "+".join(sorted(grouped[direction, merged]))
...
def line_translation(
    ...
    """Render V41 with reader joins, V40 action cards and practical OL revision."""
    working_glosses = list(glosses)
    # GDT663's renderer only applies its positional action cards to the map it
    # receives as the current layer.  Re-promote the archived G663 rows here;
    # older G660-G662 rows remain the inherited structural layer exactly as in
    # the V40 build.
    inherited = {
        key: row for key, row in inherited_target_by_token.items()
        if not str(row.get("occurrence_id", "")).startswith("G663-")
    }
    current = {
        key: row for key, row in inherited_target_by_token.items()
        if str(row.get("occurrence_id", "")).startswith("G663-")
    }
    current.update(target_by_token)
    revision_expectations: dict[tuple[str, int], str] = {}
    legacy_fixed_replacements: list[tuple[str, str]] = []
    # Revise renderings occurrence-locally, leaving the inherited structural
    # glossary and dictionary untouched.
    for index, token in enumerate(line):
        revision = INHERITED_RENDER_REVISIONS.get(token["eva"])
        if revision is None:
            continue
        practical_meaning, _composition = revision
        working_glosses[index] = practical_meaning
        key = (locus, int(token["token_index"]))
        revision_expectations[key] = practical_meaning
        if locus in LEGACY_FIXED_TRANSLATION_LOCI:
            legacy_fixed_replacements.append((practicalize(glosses[index]), practical_meaning))
            continue
        # Promote every revised occurrence into the outermost positional
        # renderer.  Merely changing the input gloss is insufficient because
        # older V13/GDT6xx occurrence cards can replace it later (e.g. olor as
        # "Zutat").  A synthetic V41 row wins that dispatch without mutating
        # the inherited structural glossary.
        seed = current.get(key) or inherited.get(key) or {}
        inherited.pop(key, None)
        current[key] = {
            **seed,
            "occurrence_id": f"G664-REV-{locus}-{token['token_index']}",
            "locus": locus,
            "token_index": token["token_index"],
            "ordinal": index + 1,
            "position": position_label(index + 1, len(line)),
            "surface": token["eva"],
            "working_render_de": practical_meaning,
            "reader_merge_direction": "NONE",
        }
    ...
    for key in suppress:
        ...
        revision_expectations.pop(key, None)
    rendered = g663.line_translation(
        locus, line, working_glosses, y_occurrence_by_token, inherited, current
    )
    for old_render, new_render in legacy_fixed_replacements:
        if rendered.count(old_render) != 1:
            raise RuntimeError(
                f"legacy fixed-line revision anchor drift at {locus}: {old_render!r}"
            )
        rendered = rendered.replace(old_render, new_render, 1)
    rendered = practicalize(rendered)
    expected_counts = Counter(revision_expectations.values())
    for expected_render, expected_count in expected_counts.items():
        if rendered.count(expected_render) < expected_count:
            raise RuntimeError(
                f"V41 inherited practical revision lost at {locus}: {expected_render!r}"
            )
    return rendered
...
def build(output_dir: Path) -> dict[str, object]:
    ...
    tokens, token_stats = g663.g662.g661.g659.guarded_query(
        TOKENS_REL, pages, "page,locus,token_index,eva,kind,section,language,hand"
    )
    cross, cross_stats = g663.g662.g661.g659.guarded_query(
        CROSS_REL, pages,
    ...
    base_glossary_rows = read_tsv(base_art / "V40_WORKING_TOKEN_GLOSSARY.tsv")
    ...
    if dimensions != (976, 734, 4128, 449, 333, 146):
        raise RuntimeError(f"V40 base dimensions drift: {dimensions!r}")
    frontier_order = tuple(dict.fromkeys(row["unknown_surface"] for row in frontier))
    if frontier_order != TARGET_ORDER or len(TARGET_SURFACES) != 140:
        raise RuntimeError("the 140-form frontier or fixed order drifted")
    ...
    if any(surface in base_glossary for surface in TARGET_SURFACES):
        raise RuntimeError("a GDT664 target unexpectedly already has a V40 glossary row")
    ...
    y_occurrences = read_tsv(
        ROOT / g663.g662.g661.g660.G659 / "artifacts/Y_OCCURRENCE_CENSUS.tsv"
    )
    ...
    inherited_target_by_token: dict[tuple[str, int], dict[str, object]] = {}
    inherited_audits = (
        ROOT / g663.g662.g661.G660 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        ROOT / g663.g662.G661 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        ROOT / g663.G662 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
    ...
    if observed_counts != EXPECTED_SURFACE_COUNTS or sum(observed_counts.values()) != 1141:
        raise RuntimeError(f"target surface count drift: {observed_counts!r}")
    exact, normalized = g663.g662.g661.g660.stable_maps(tokens, cross_by_locus)
    ...
    for locus in sorted(by_line):
        ...
        for index, token in enumerate(line):
            ...
            item: dict[str, object] = {
                "occurrence_id": f"G664-T{len(occurrence_rows) + 1:04d}",
    ...
    if len(occurrence_rows) != 1141 or len(target_by_token) != 1141:
    ...
    if contextual_surface_counts != Counter({"o": 146, "ch": 13, "qok": 13, "m": 6, "sa": 3}):
    ...
    for base_row in base_coverage:
        ...
        if not (len(line) == len(glosses) == len(sources) == len(states)):
            raise RuntimeError(f"V40 token columns misalign: {locus}")
        ...
        for index, token in enumerate(line):
            ...
            if glosses[index] != f"[{surface}:?]" or sources[index] != "OPEN" or states[index] != "UNKNOWN_SURFACE":
                raise RuntimeError(f"V40 target not open at {locus}.{index + 1}: {surface}")
            glosses[index] = str(occurrence["working_gloss_de"])
            if surface in CONTEXT_SCOPED_SURFACES:
                sources[index] = f"GDT664:{occurrence['rendering_class']}:{occurrence['reader_merge_surface']}"
                states[index] = "KNOWN_CONTEXT_LICENSED" if int(occurrence["reader_exact"]) else "READER_BOUNDARY_UNSTABLE"
            else:
                sources[index] = f"GDT664:EXACT_WHOLE:{surface}"
        ...
        if int(base_row["unknown_tokens"]) - len(target_ordinals) != len(unknown_pairs):
            raise RuntimeError(f"V40->V41 arithmetic drift: {locus}")
    ...
    for row in coverage_rows:
        ...
        if old and old["unknown_surface"] == surface and int(old["unknown_ordinal"]) == ordinal:
            ...
            proposal, basis, strength = f"[{surface}:?]", "NEWLY_EXPOSED_BY_GDT664_NO_NEW_CARD", "OPEN"
        strict = int(
            int(row["ambiguous_tokens"]) == 0 and int(row["reader_unstable_tokens"]) == 0
            and int(row["all_present_exact"]) == 1
        )
    ...
    for spec_row in EXACT_WHOLE_SPECS:
        ...
        glossary_rows.append({
            ...
            "source": "GDT664:CONTEXT_SHORT_DEFAULT" if surface in CONTEXT_SCOPED_SURFACES else "GDT664:EXACT_WHOLE",
        ...
        dictionary_rows.append({
            "entry": f"{surface}@GDT664_DEFAULT", "kind": card_type(surface),
            ...
            "status": "NEW_V41_PROVISIONAL_CONCRETE_RECIPE_DEFAULT",
        })
    glossary_rows.sort(key=lambda row: str(row["surface"]))
    if len(glossary_rows) != 874:
        raise RuntimeError("V41 glossary dimension drift")
    ...
    for klass, surface, merge_surface, render in context_keys:
        ...
        context_cards.append({
            "card_id": f"G664-C{len(context_cards) + 1:03d}", "rendering_class": klass,
    ...
    for kind in (
        ...
        architecture_rows.append({
            ...
            "dispatch_rule": "exact whole; five short forms additionally use attested alternate-reader joins",
        })
    ...
    for occurrence in occurrence_rows:
        ...
        audit_rows.append({
            **occurrence,
            "v40_gloss_de": split_pipe(base_row["token_glosses_de"])[ordinal - 1],
            "v41_gloss_de": split_pipe(final_row["token_glosses_de"])[ordinal - 1],
            "v40_scope_state": split_pipe(base_row["scope_states"])[ordinal - 1],
            "v41_scope_state": split_pipe(final_row["scope_states"])[ordinal - 1],
            "v41_working_translation_de": line_translation(
    ...
    if any(str(row["v40_gloss_de"]) != f"[{row['surface']}:?]" for row in audit_rows):
        raise RuntimeError("not every target occurrence was open in V40")
    if any(GENERIC_FILLER.search(str(row["v41_working_translation_de"])) for row in audit_rows):
        raise RuntimeError("generic work filler leaked into GDT664")
    if any("Eigenschafts-/Zustands-/Materialträger" in str(row["v41_working_translation_de"]) for row in audit_rows):
        raise RuntimeError("structural OL meta-gloss leaked into practical V41 prose")
    ...
    for index, spec_row in enumerate(EXACT_WHOLE_SPECS, 1):
        ...
        decision_rows.append({
            "decision_id": f"G664-D{index:03d}", "surface": surface, "family": spec_row["family"],
            ...
            "strength": card_strength(surface), "status": "ACCEPT_V41_REPLACEABLE",
        })
    ...
    for locus in sorted(affected_loci):
        ...
        target_line_rows.append({
            ...
            "zl3b_line": final_row["zl3b_line"], "v40_token_glosses_de": base_row["token_glosses_de"],
            "v41_token_glosses_de": final_row["token_glosses_de"],
            "v41_working_translation_de": line_translation(
                locus, by_line[locus], split_pipe(final_row["token_glosses_de"]),
                y_occurrence_by_token, inherited_target_by_token, target_by_token,
            ),
            "v40_unknown_tokens": base_row["unknown_tokens"], "v41_unknown_tokens": final_row["unknown_tokens"],
            "v41_complete": int(int(final_row["unknown_tokens"]) == 0),
        })
    ...
    for row in frontier:
        ...
        frontier_rows.append({
            ...
            "zl3b_line": row["zl3b_line"], "v40_translation_de": row["proposed_complete_translation_de"],
            "v41_translation_de": line_translation(
                locus, by_line[locus], split_pipe(final_row["token_glosses_de"]),
                y_occurrence_by_token, inherited_target_by_token, target_by_token,
            ),
            "status": "COMPLETE_WITH_PROVISIONAL_CONCRETE_DEFAULT",
        })
    if len(frontier_rows) != 146 or any(f"[{row['surface']}:?]" in str(row["v41_translation_de"]) for row in frontier_rows):
        raise RuntimeError("a GDT664 frontier slot remained open in V41")
    frontier_o = [row for row in frontier_rows if row["surface"] == "o"]
    if len(frontier_o) != 3 or any(row["reader_merge_surface"] == "NONE" for row in frontier_o):
        raise RuntimeError("the three frontier o slots must all use visible alternate-reader joins")
    ...
    for spec_row in MANUAL_PASSAGE_SPECS:
        ...
        if int(final_row["unknown_tokens"]) != 0:
            raise RuntimeError(f"manual passage is not V41-complete: {locus}")
        ...
        manual_passage_rows.append({
            **spec_row,
            "automatic_v41_translation_de": automatic,
            "v41_unknown_tokens": final_row["unknown_tokens"],
            "comparison_scope": "manual workshop prose versus deterministic V41 renderer; neither is confirmed plaintext",
        })
    ...
    expected_base = {
        "physical_lines": 4128, "known_token_positions": 20417, "unknown_token_positions": 11922,
        "complete_multi_token_lines": 449, "strict_complete_lines": 147,
        "one_unknown_lines": 333, "strict_one_unknown_lines": 80, "working_glossary_surfaces": 734,
    }
    if base_metrics != expected_base:
        raise RuntimeError(f"V40 base metrics drift: {base_metrics!r}")
    round_rows = [
        {"version": "V40", "added_cards": "BASE", "dictionary_entries": len(base_dictionary), **base_metrics},
        {"version": "V41", "added_cards": f"140_DEFAULTS+{len(context_cards)}_RENDERINGS+19_OL_REVISIONS+2_SOL_REVISIONS",
    ...
    write_tsv(output_dir / "FRONTIER_140_COMPLETIONS.tsv", frontier_rows, list(frontier_rows[0]))
    ...
    write_tsv(output_dir / "V41_WORKING_TOKEN_GLOSSARY.tsv", glossary_rows, list(base_glossary_rows[0]))
    write_tsv(output_dir / "WORKING_DICTIONARY_V41.tsv", dictionary_rows, list(base_dictionary[0]))
    write_tsv(output_dir / "ALL_LINE_CONCRETE_COVERAGE_V41.tsv", coverage_rows, coverage_fields)
    write_tsv(output_dir / "COMPLETE_PASSAGES_V41.tsv", complete_rows, complete_fields)
    write_tsv(output_dir / "ONE_UNKNOWN_PASSAGES_V41.tsv", one_rows, one_fields)
    write_tsv(
    ...
    write_tsv(
        ...
        (
            ...
            "automatic_v41_translation_de", "v41_unknown_tokens", "comparison_scope",
        ),
    )

    input_paths = (
        G663 / "REPORT.md", G663 / "artifacts/RESULT.json", G663 / "artifacts/PAGE_ALLOWLIST.tsv",
        G663 / "artifacts/V40_WORKING_TOKEN_GLOSSARY.tsv", G663 / "artifacts/WORKING_DICTIONARY_V40.tsv",
        G663 / "artifacts/ALL_LINE_CONCRETE_COVERAGE_V40.tsv", G663 / "artifacts/COMPLETE_PASSAGES_V40.tsv",
        G663 / "artifacts/ONE_UNKNOWN_PASSAGES_V40.tsv", G663 / "artifacts/NEWLY_EXPOSED_ONE_HOLE_LINES.tsv",
        G663 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        g663.G662 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        g663.g662.G661 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        g663.g662.g661.G660 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        g663.g662.g661.g660.G659 / "artifacts/Y_OCCURRENCE_CENSUS.tsv",
    ...
    result_core: dict[str, object] = {
        "schema": "GDT664_ONE_HUNDRED_FORTY_RESIDUAL_FAMILY_COMPLETION_RESULT_V1",
        "experiment_id": "GDT664", "status": STATUS,
        "guard": {
        ...
        "context_short_forms": {
            ...
            "free_defaults": {"o": "Ansatzwasser", "ch": "trockne", "qok": "erhitze", "m": "eine Handvoll", "sa": "Saatzugabe"},
        ...
        "manual_passages": {
            ...
            "v41_complete_rows": sum(int(row["v41_unknown_tokens"]) == 0 for row in manual_passage_rows),
        ...
        "working_dictionary": {
            "v40_entries": len(base_dictionary), "v41_entries": len(dictionary_rows),
            "added_default_entries": len(EXACT_WHOLE_SPECS), "added_rendering_entries": len(context_cards),
            "v40_glossary_surfaces": len(base_glossary_rows), "v41_glossary_surfaces": len(glossary_rows),
        },
        ...
        "claim_boundary": (
            "Exploratory replaceable concrete defaults for 140 V40 residual surfaces at 1141 inherited positions. "
            "Productive compounds, learned actions and learned wholes coexist. Visible alternate-reader joins win "
            "for o/ch/qok/m/sa; otherwise their concrete defaults remain Ansatzwasser, trocknen, erhitzen, Handvoll and "
            "Saatzugabe. Nineteen inherited material forms receive an O_PREP+L_WOOD practical renderer at 256 positions, "
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.artifact_dir.resolve() == ART.resolve():
        with tempfile.TemporaryDirectory(prefix="gdt664_replay_") as directory:
    ...
    print(
        f"GDT664 built: targets={result['targets']['positions']} surfaces=140 "
