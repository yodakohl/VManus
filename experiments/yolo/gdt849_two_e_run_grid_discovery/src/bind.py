...
up=['experiments/yolo/gdt632_cth_interfix_lattice/REPORT.md', 'experiments/yolo/gdt633_cth_interfix_semantic_contrasts/REPORT.md', 'experiments/yolo/gdt651_ckh_four_shell_family_migration/REPORT.md', 'experiments/yolo/gdt847_source_count_contract_reconciliation/REPORT.md', 'experiments/yolo/gdt829_repeated_passage_reflow_capacity/src/run.py', 'experiments/semantic_assumptions/results/source_separator_transcription.tsv', 'experiments/yolo/gdt631_prefixed_cth_quality_parts/artifacts/PAGE_ALLOWLIST.tsv']
...
m.update(title='Two-position e-run complete grid discovery',question='What joint variation exists at two literal e positions within fixed ch/sh and cth/ckh forms?'     ,claim_ceiling='Descriptive exact written forms only; no confirmatory significance, morpheme, number or meaning.',status=status,dependencies=['GDT632','GDT633','GDT651','GDT829','GDT847'],inputs=[{'path':n,'sha256':sha(ROOT/n),'role':'frozen_source_or_extractor'} for n in up],outputs=[{'path':p.relative_to(ROOT).as_posix(),'sha256':sha(p),'role':'primary_report' if p.name=='REPORT.md' else 'source_or_artifact'} for p in outputs],validation={'status':'PASS' if result.exists() else 'NOT_RUN','artifact':rel+'/artifacts/VALIDATION.json' if result.exists() else None})
