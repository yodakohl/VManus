...
up=['docs/visual_overview/SOURCES.json','experiments/yolo/gdt841_star_centre_visual_reliability/PREREGISTRATION.md','experiments/yolo/gdt841_star_centre_visual_reliability/src/run.py','experiments/yolo/gdt841_star_centre_visual_reliability/src/validate.py']
...
m.update(title='Complete outer-ring star-centre extension',question='Does the fixed star-centre distinction transfer to the complete outer ten on the same photograph?'    ,claim_ceiling='Visual readability capacity on one photograph only; no held label prediction, original pigment absence or semantics.',status=status,dependencies=['GDT841'],inputs=[{'path':n,'sha256':sha(ROOT/n),'role':'frozen_source_or_extractor'} for n in up],outputs=[{'path':p.relative_to(ROOT).as_posix(),'sha256':sha(p),'role':'primary_report' if p.name=='REPORT.md' else 'source_or_artifact'} for p in outputs],validation={'status':'PASS' if result.exists() else 'NOT_RUN','artifact':rel+'/artifacts/VALIDATION.json' if result.exists() else None})
