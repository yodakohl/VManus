#!/usr/bin/env python3
"""Compile a surface with stems, boundary licenses and short atom histories."""
...
def main() -> int:
    ...
    bigram = core.G518.train_ngram(running, "source_statement_id", recipe_field, order=2)
    trigram = core.G518.train_ngram(running, "source_statement_id", recipe_field, order=3)
    ...
    recipe_model = core.train_recipe_ngram(
        running, recipe_field, core.SELECTED_ORDER, core.SELECTED_ALPHA
    )
    ...
    for base_index, candidate in enumerate(candidates):
        ...
        anchor_cost, path = core.G520.alignment_path(surface, candidate.recipe, matrix)
        ...
        scored.append(
            {
                ...
                "order5_composition_nll": order5_nll,
                "gdt521_score": gdt520_score + core.SELECTED_WEIGHT * order5_nll,
                "alignment_trace": core.G520.path_text(surface, path),
                "derivation": core.G517.path_text(candidate),
            }
        )
    scored.sort(key=lambda row: (float(row["gdt521_score"]), int(row["compiler_rank"])))
    ...
    result = {
        ...
        "recipe_history_training_surface_count": recipe_model.training_surface_count,
        "recipe_history_order": recipe_model.order,
        "recipe_history_count": len(recipe_model.histories),
        "exact_event_match_count": len(exact),
        "known_surface_option_count": len(known),
        "reranked_candidates": scored[: max(1, args.top)],
        "selection_precedence": "EXACT_EVENT>KNOWN_SURFACE_ROLE_OPTION>GDT521_SHORT_HISTORY_TOP1",
        "default_selection": default,
        "guard": "SHORT_RECIPE_HISTORY_DEFAULT__NO_CONFIRMED_LEXEME_OR_PLAINTEXT",
    }
