"""Post-exposure separator-enum correction; original locked run and results preserved."""
...
def line_records(data,spec):
    ...
    for key,gs in raw.items():
        ...
        definite_seams=all(a['right_separator']==b['left_separator']=='DEFINITE_SPACE' for a,b in zip(gs,gs[1:]))
...
def main():
    ...
    tsv(E/'artifacts/corrected/ALL_TITLE_BODY_CONSEQUENCES.tsv',evidence)
    tsv(E/'artifacts/corrected/ALL_NAME_CANDIDATES.tsv',candidates)
    tsv(E/'artifacts/corrected/COMPLETE_RECORDS.tsv',rendered)
    tsv(E/'artifacts/corrected/CANDIDATE_TABLE.tsv',result)
    dump(E/'artifacts/corrected/GRAPHS.json',{'source_edges':edges,'graphs':graphs})
    dump(E/'artifacts/corrected/RESULT.json',{'experiment':'GDT952','source_edges':len(edges),'assignment_space_per_case':479001600,
