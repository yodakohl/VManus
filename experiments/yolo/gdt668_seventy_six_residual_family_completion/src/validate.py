#!/usr/bin/env python3
"""Independent source-first release validator for GDT668."""
...
BASE = Path("experiments/yolo/gdt668_seventy_six_residual_family_completion")
...
OUTPUT_NAMES = (
    ...
    "FAMILY_COMPOSITION_ATLAS.tsv", "STEM_MODEL_V45.tsv",
    "INHERITED_OL_RENDER_REVISIONS.tsv", "INHERITED_SOL_RENDER_REVISIONS.tsv",
    "MANUAL_PASSAGE_AUDIT.tsv",
    "FRONTIER_76_COMPLETIONS.tsv", "TARGET_LINE_TRANSLATIONS.tsv",
    "ROUND_COVERAGE_COUNTS.tsv", "NEWLY_COMPLETED_LINES.tsv",
    "NEWLY_EXPOSED_ONE_HOLE_LINES.tsv", "NEXT_FRONTIER_FULL_PANEL_COUNTS.tsv",
    "V45_WORKING_TOKEN_GLOSSARY.tsv",
    "WORKING_DICTIONARY_V45.tsv", "ALL_LINE_CONCRETE_COVERAGE_V45.tsv",
    "COMPLETE_PASSAGES_V45.tsv", "ONE_UNKNOWN_PASSAGES_V45.tsv",
)

TARGET_ORDER = tuple("""
aeky teolshy cheoraiin okairy ycheedy cheary shedain chcthor lkeeol polchdy
pchodain ykeedan loar oteeey olchedaiin oeeody chold soshy koshet dairin sholo
keaiin dolcheey okeor chod doty opchol otchyd oeeeos okoeor qotcho eaiin
dorchory otcheor cths dols qokchaiin ytoiin tody ykchom chkchy dcheeey qod
cheeor chelal chkor cheeckhody ykchor talchos ykey olkaly ychos chekam ocheain
rom solkedy qolchey okees san keeal skain qokoin daral qcthdys lchpchdy
lcheckhy otechd sqokeodaiin dalchom odam teols chotey cheeol olkeeoldy ksheody
rokyd
""".split())
...
def main() -> int:
    ...
    # guarded selector before trusting any GDT668 result or generated artifact.
    base_pages = {row["page"] for row in read_tsv(ROOT / G667 / "artifacts/PAGE_ALLOWLIST.tsv")}
    ...
    check("target count sum", sum(EXPECTED_COUNTS.values()) == 230)
    ...
    check("CARD_SPECS 76 unique forms", len(specs) == len(set(spec_order)) == 76)
    check("CARD_SPECS all fields populated", all(all(row[field] for field in spec_fields) for row in specs))
    check("CARD_SPECS raw positions", sum(raw_counts[row["surface"]] for row in specs) == 230)
    ...
    check("stem-model source 54 rows", len(stem_specs) == 54)
    ...
    stem_model = read_tsv(ART / "STEM_MODEL_V45.tsv")
    ...
    glossary = read_tsv(ART / "V45_WORKING_TOKEN_GLOSSARY.tsv")
    dictionary = read_tsv(ART / "WORKING_DICTIONARY_V45.tsv")
    coverage = read_tsv(ART / "ALL_LINE_CONCRETE_COVERAGE_V45.tsv")
    complete = read_tsv(ART / "COMPLETE_PASSAGES_V45.tsv")
    one = read_tsv(ART / "ONE_UNKNOWN_PASSAGES_V45.tsv")
    ...
    base_glossary = read_tsv(base_art / "V44_WORKING_TOKEN_GLOSSARY.tsv")
    ...
    check("decision dimensions", len(deck) == len(accepted) == len(family) == 76)
    ...
    check("architecture position total", sum(int(row["positions"]) for row in architecture) == 230)

    audit_counts = Counter(row["surface"] for row in audit)
    check("occurrence audit dimensions", len(audit) == len(reader) == 230)
    check("occurrence audit exact counts", dict(audit_counts) == EXPECTED_COUNTS, dict(audit_counts))
    check("occurrence IDs are G668", all(row["occurrence_id"].startswith("G668-") for row in audit))
    check("all target slots were V44-open", all(row["v44_gloss_de"] == f"[{row['surface']}:?]" for row in audit))
    check("all target slots are V45-filled", all(
        row["v45_gloss_de"] and not OPEN_MARKER.search(row["v45_gloss_de"]) for row in audit
    ))
    ...
    check("audit practical prose has no generic work filler", not any(
        GENERIC.search(row["v45_working_translation_de"]) for row in audit
    ))
    check("target-line practical prose has no generic work filler", not any(
        GENERIC.search(row["v45_working_translation_de"]) for row in target_lines
    ))
    ...
    sqokeo_reader = [row for row in reader if row["surface"] == "sqokeodaiin"]
    check("sqokeodaiin has the exact f88v.20 dual-reader split", len(sqokeo_reader) == 1
          and sqokeo_reader[0]["locus"] == "f88v.20"
          and sqokeo_reader[0]["zl3b_line"].startswith("sqokeodaiin ")
          and sqokeo_reader[0]["it2a_line"].startswith("sqokeo daiin ")
          and sqokeo_reader[0]["rf1b_line"].startswith("sqokeo daiin ")
          and sqokeo_reader[0]["split_normalized"] == "1")
    ...
    check("only named GDT668 boundary forms reader-dispatch", all(
        row["surface"] in CONTEXT_SURFACES for row in merge_rows
    ))

    check("stem-model artifact byte-equivalent to source", (ART / "STEM_MODEL_V45.tsv").read_bytes() == STEM_SPECS.read_bytes())
    stems_by_key = {(row["stem"], row["structural_role"]): row for row in stem_model}
    check("stem-model artifact 54 rows", len(stem_model) == len(stems_by_key) == 54)
    ...
    check("final synthesis is 74 role-composed plus 2 learned", len(learned_wholes) == 2
          and len(specs) - len(learned_wholes) == 74)
    ...
    yd_actual = {surface for surface, atoms in card_atoms.items() if "YD_POST_CLOSE" in atoms}
    check("YD post-close remains an exact terminal block", yd_actual == {"otchyd", "rokyd"}
          and card_atoms["otchyd"] == ["O_PREP", "T_COLD", "CH_DRY", "YD_POST_CLOSE"]
          and card_atoms["rokyd"] == ["R_ROOT", "O_PREP", "K_HOT", "YD_POST_CLOSE"]
          and all(card_atoms[surface][-1] == "YD_POST_CLOSE" and surface.endswith("yd")
                  for surface in yd_actual))
    ...
    check("SQOKEO is the exact reader-separated left block",
          ...
          and card_atoms["sqokeodaiin"] == ["SQOKEO_SEED_HEAT_PREP", "D_MEASURE", "AIIN_III"]
          and "S_SEED" not in card_atoms["sqokeodaiin"])
    check("two difficult cards remain concrete learned wholes", learned_wholes == {"dairin", "eaiin"})

    for surface, expected in {
        "okeor": ["O_PREP", "K_HOT", "E_MIDDLE", "OR_PORTION"],
        "cheeor": ["CH_DRY", "EE_END", "OR_PORTION"],
        "shedain": ["SH_MOIST", "E_MIDDLE", "D_MEASURE", "AIN_II"],
        "cheeol": ["CH_DRY", "EE_END", "OL_MATERIAL"],
        "okees": ["O_PREP", "K_HOT", "EE_END", "S_TERM_SPECIES"],
        "qolchey": ["QOL_ADD", "CH_DRY", "E_MIDDLE", "Y_START_OR_CLOSE"],
        "qotcho": ["QO_COMMAND", "T_COLD", "CH_DRY", "O_PREP"],
        "tody": ["T_COLD", "O_PREP", "DY_FINISHED"],
    }.items():
    ...
    check("V45 glossary adds exactly 76 targets", len(glossary) == len(base_glossary) + 76
          and len(glossary) == 1350)
    ...
    check("frontier dimensions", len(frontier) == len(base_frontier) == 76)
    ...
    check("frontier completely closed", all(
        ...
        and not OPEN_MARKER.search(row["v45_translation_de"])
        and not GENERIC.search(row["v45_translation_de"])
        for row in frontier
    ))
    expected_next_surfaces = {row["unknown_surface"] for row in new_one}
    check("next-frontier dimensions", len(new_one) == len(next_frontier) == 37
          and len(expected_next_surfaces) == 37)
    ...
    for rank in map(str, range(1, 21)):
        ...
        check(f"manual V45 complete {rank}", row["v45_unknown_tokens"] == "0"
              and row["locus"] in complete_by_locus)
        check(f"manual automatic renderer identity {rank}", row["automatic_v45_translation_de"]
              == complete_by_locus[row["locus"]]["working_translation_de"])
        check(f"manual and automatic channels distinct {rank}", "manual workshop prose versus deterministic V45 renderer" in row["comparison_scope"])
        check(f"manual prose concrete {rank}", not GENERIC.search(row["manual_workshop_translation_de"])
              ...
              and not GENERIC.search(row["automatic_v45_translation_de"])
              and not OPEN_MARKER.search(row["automatic_v45_translation_de"]))
    ...
    check("non-target position census", len(non_target_before) == 32339 - 230, len(non_target_before))
    ...
    check("known delta exactly 230", final_metrics["known_token_positions"] - base_metrics["known_token_positions"] == 230)
    check("unknown delta exactly 230", base_metrics["unknown_token_positions"] - final_metrics["unknown_token_positions"] == 230)
    check("V45 dictionary has 2022 entries", len(dictionary) == 2022)
    ...
    check("round versions", [row["version"] for row in rounds] == ["V44", "V45"])
    ...
    check("result schema", result["schema"] == "GDT668_SEVENTY_SIX_RESIDUAL_FAMILY_COMPLETION_RESULT_V1")
    check("result status", result["status"] == "PASS_230_TARGET_POSITIONS__V45_CONCRETE_RECIPE_REGISTER")
    check("result target dimensions", result["targets"]["surface_types"] == 76 and result["targets"]["positions"] == 230)
    check("result target counts", result["targets"]["surface_counts"] == EXPECTED_COUNTS)
    check("result frontier dimensions", result["frontier"]["source_rows"] == result["frontier"]["completed_rows"] == 76
          and result["frontier"]["next_rows"] == result["frontier"]["next_unique_surfaces"] == 37
    ...
    check("result dictionary dimensions", result["working_dictionary"]["v44_entries"] == len(base_dictionary) and result["working_dictionary"]["v45_entries"] == len(dictionary))
    check("result glossary dimensions", result["working_dictionary"]["v44_glossary_surfaces"] == len(base_glossary) and result["working_dictionary"]["v45_glossary_surfaces"] == len(glossary))
    ...
    check("result stem-model dimensions", result["stem_model"]["rows"] == 54
          and result["stem_model"]["structural_roles_distinct_from_german_defaults"] is True)
    check("result card-synthesis dimensions", result["card_synthesis"]["productive_compositions"] == 74
          and result["card_synthesis"]["learned_exact_wholes"] == 2
          and result["card_synthesis"]["learned_exact_surfaces"] == sorted(learned_wholes)
          and result["card_synthesis"]["inherited_stem_roles_reused"] == 52
          and result["card_synthesis"]["total_stem_roles"] == 54
          and result["card_synthesis"]["new_stem_roles"] == 2
          and result["card_synthesis"]["new_stem_role"] == "YD_POST_CLOSE|SQOKEO_SEED_HEAT_PREP"
    ...
    check("result manual-passage dimensions", result["manual_passages"]["rows"] == 20
          ...
          and result["manual_passages"]["v45_complete_rows"] == 20
    ...
    with tempfile.TemporaryDirectory(prefix="gdt668_validator_replay_") as directory:
    ...
    validation = {
        "schema": "GDT668_VALIDATION_V1", "experiment_id": "GDT668",
    ...
    print(f"GDT668 validation: {validation['status']} {validation['checks_passed']}/{validation['checks_total']}")
    if failures:
