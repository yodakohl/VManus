#!/usr/bin/env python3
"""Build GDT668: close the 76-form V44 frontier as concrete V45 recipe cards."""
...
BASE_REL = Path("experiments/yolo/gdt668_seventy_six_residual_family_completion")
ART = ROOT / BASE_REL / "artifacts"
G667 = Path("experiments/yolo/gdt667_one_hundred_one_residual_family_completion")
_spec = importlib.util.spec_from_file_location("gdt667_builder_for_gdt668", ROOT / G667 / "src/run.py")
if _spec is None or _spec.loader is None:
    raise RuntimeError("cannot load GDT667 builder")
g667 = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(g667)
TOKENS_REL, CROSS_REL = g667.TOKENS_REL, g667.CROSS_REL
STATUS = "PASS_230_TARGET_POSITIONS__V45_CONCRETE_RECIPE_REGISTER"
...
def read_card_specs(path: Path) -> tuple[dict[str, str], ...]:
    ...
    if not rows or set(rows[0]) != expected:
        raise RuntimeError("bad GDT668 CARD_SPECS.tsv columns")
    surfaces = [row["surface"] for row in rows]
    if len(rows) != 76 or len(set(surfaces)) != 76:
        raise RuntimeError("GDT668 CARD_SPECS.tsv must contain 76 unique surfaces")
    return tuple(rows)
...
aeky=1 teolshy=1 cheoraiin=3 okairy=1 ycheedy=7 cheary=1 shedain=12 chcthor=1
lkeeol=2 polchdy=2 pchodain=3 ykeedan=1 loar=1 oteeey=7 olchedaiin=1 oeeody=1
chold=1 soshy=2 koshet=1 dairin=1 sholo=1 keaiin=1 dolcheey=1 okeor=15 chod=8
doty=1 opchol=6 otchyd=2 oeeeos=1 okoeor=1 qotcho=9 eaiin=1 dorchory=1
otcheor=2 cths=1 dols=1 qokchaiin=2 ytoiin=4 tody=9 ykchom=1 chkchy=6
dcheeey=1 qod=8 cheeor=13 chelal=1 chkor=3 cheeckhody=1 ykchor=5 talchos=1
ykey=5 olkaly=1 ychos=3 chekam=1 ocheain=1 rom=1 solkedy=2 qolchey=9 okees=9
san=3 keeal=1 skain=1 qokoin=1 daral=3 qcthdys=1 lchpchdy=1 lcheckhy=3
otechd=1 sqokeodaiin=1 dalchom=1 odam=5 teols=1 chotey=3 cheeol=9 olkeeoldy=1
ksheody=4 rokyd=1
""")
...
OUTPUT_NAMES = (
    ...
    "V45_WORKING_TOKEN_GLOSSARY.tsv", "WORKING_DICTIONARY_V45.tsv",
    "ALL_LINE_CONCRETE_COVERAGE_V45.tsv", "COMPLETE_PASSAGES_V45.tsv",
    "ONE_UNKNOWN_PASSAGES_V45.tsv", "INHERITED_OL_RENDER_REVISIONS.tsv",
    "INHERITED_SOL_RENDER_REVISIONS.tsv", "STEM_MODEL_V45.tsv", "MANUAL_PASSAGE_AUDIT.tsv",
)
...
# These are practical render revisions only.  V44's structural tags remain
# byte-visible in its inherited glossary/dictionary; V45 translates the 19
...
def read_ol_revision_specs(path: Path) -> tuple[dict[str, str], ...]:
    ...
    if not rows or set(rows[0]) != expected:
        raise RuntimeError("bad GDT668 INHERITED_OL_REVISION_SPECS.tsv columns")
    surfaces = [row["surface"] for row in rows]
    if len(rows) != 19 or len(set(surfaces)) != 19 or sum(int(row["positions"]) for row in rows) != 256:
        raise RuntimeError("GDT668 OL revision source must contain 19 unique forms at 256 positions")
...
def read_sol_revision_specs(path: Path) -> tuple[dict[str, str], ...]:
    ...
    expected = {"surface", "positions", "practical_meaning_de", "composition", "retained_rival_de"}
    if not rows or set(rows[0]) != expected:
        raise RuntimeError("bad GDT668 INHERITED_SOL_REVISION_SPECS.tsv columns")
    surfaces = [row["surface"] for row in rows]
    if len(rows) != 2 or len(set(surfaces)) != 2 or sum(int(row["positions"]) for row in rows) != 4:
        raise RuntimeError("GDT668 SOL revision source must contain 2 unique forms at 4 positions")
    return tuple(rows)
...
# all token gloss markers at this locus.  V45 preserves that sentence but must
...
def read_stem_model_specs(path: Path) -> tuple[dict[str, str], ...]:
    ...
    if not rows or set(rows[0]) != expected or len(rows) != 54:
        raise RuntimeError("GDT668 STEM_MODEL_SPECS.tsv must contain the 54-row V45 schema")
    return tuple(rows)
...
def read_manual_passage_specs(path: Path) -> tuple[dict[str, str], ...]:
    ...
    if not rows or set(rows[0]) != expected or len(rows) != 20:
        raise RuntimeError("GDT668 MANUAL_PASSAGE_SPECS.tsv must contain the fixed 20-row schema")
    if [int(row["rank"]) for row in rows] != list(range(1, 21)):
        raise RuntimeError("GDT668 manual passage ranks must be 1..20")
    if len({row["locus"] for row in rows}) != 20:
        raise RuntimeError("GDT668 manual passage loci must be unique")
    return tuple(rows)
...
def line_translation(
    ...
    """Render V45 with reader joins over the complete V44 renderer."""
    working_glosses = list(glosses)
    # GDT666's renderer only applies its positional action cards to the map it
    # receives as the current layer.  Re-promote the archived G667 rows here;
    ...
    inherited = {
        ...
        if not str(row.get("occurrence_id", "")).startswith("G667-")
    }
    current = {
        key: row for key, row in inherited_target_by_token.items()
        if str(row.get("occurrence_id", "")).startswith("G667-")
    }
    ...
    rendered = g667.line_translation(
...
def build(output_dir: Path) -> dict[str, object]:
    ...
    tokens, token_stats = g667.g666.g665.g664.g663.g662.g661.g659.guarded_query(
        TOKENS_REL, pages, "page,locus,token_index,eva,kind,section,language,hand"
    )
    cross, cross_stats = g667.g666.g665.g664.g663.g662.g661.g659.guarded_query(
        CROSS_REL, pages,
    ...
    base_dictionary = read_tsv(base_art / "WORKING_DICTIONARY_V44.tsv")
    base_glossary_rows = read_tsv(base_art / "V44_WORKING_TOKEN_GLOSSARY.tsv")
    base_coverage = read_tsv(base_art / "ALL_LINE_CONCRETE_COVERAGE_V44.tsv")
    base_complete = read_tsv(base_art / "COMPLETE_PASSAGES_V44.tsv")
    base_one = read_tsv(base_art / "ONE_UNKNOWN_PASSAGES_V44.tsv")
    ...
    if dimensions != (1924, 1274, 4128, 1073, 222, 76):
        raise RuntimeError(f"V44 base dimensions drift: {dimensions!r}")
    ...
    if frontier_order != TARGET_ORDER or len(TARGET_SURFACES) != 76:
        raise RuntimeError("the 76-form frontier or fixed order drifted")
    ...
    if any(surface in base_glossary for surface in TARGET_SURFACES):
        raise RuntimeError("a GDT668 target unexpectedly already has a V44 glossary row")
    ...
    y_occurrences = read_tsv(
        ROOT / g667.g666.g665.g664.g663.g662.g661.g660.G659 / "artifacts/Y_OCCURRENCE_CENSUS.tsv"
    )
    ...
    inherited_target_by_token: dict[tuple[str, int], dict[str, object]] = {}
    inherited_audits = (
        ROOT / g667.g666.g665.g664.g663.g662.g661.G660 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        ROOT / g667.g666.g665.g664.g663.g662.G661 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        ROOT / g667.g666.g665.g664.g663.G662 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        ROOT / g667.g666.g665.g664.G663 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        ROOT / g667.g666.g665.G664 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        ROOT / g667.g666.G665 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        ROOT / g667.G666 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
    ...
    if observed_counts != EXPECTED_SURFACE_COUNTS or sum(observed_counts.values()) != 230:
        raise RuntimeError(f"target surface count drift: {observed_counts!r}")
    exact, normalized = g667.g666.g665.g664.g663.g662.g661.g660.stable_maps(tokens, cross_by_locus)
    ...
    for locus in sorted(by_line):
        ...
        for index, token in enumerate(line):
            ...
            item: dict[str, object] = {
                "occurrence_id": f"G668-T{len(occurrence_rows) + 1:04d}",
    ...
    if len(occurrence_rows) != 230 or len(target_by_token) != 230:
    ...
    for base_row in base_coverage:
        ...
        if not (len(line) == len(glosses) == len(sources) == len(states)):
            raise RuntimeError(f"V44 token columns misalign: {locus}")
        ...
        for index, token in enumerate(line):
            ...
            if glosses[index] != f"[{surface}:?]" or sources[index] != "OPEN" or states[index] != "UNKNOWN_SURFACE":
                raise RuntimeError(f"V44 target not open at {locus}.{index + 1}: {surface}")
            glosses[index] = str(occurrence["working_gloss_de"])
            if surface in CONTEXT_SCOPED_SURFACES:
                sources[index] = f"GDT668:{occurrence['rendering_class']}:{occurrence['reader_merge_surface']}"
                states[index] = "KNOWN_CONTEXT_LICENSED" if int(occurrence["reader_exact"]) else "READER_BOUNDARY_UNSTABLE"
            else:
                sources[index] = f"GDT668:EXACT_WHOLE:{surface}"
        ...
        if int(base_row["unknown_tokens"]) - len(target_ordinals) != len(unknown_pairs):
            raise RuntimeError(f"V44->V45 arithmetic drift: {locus}")
    ...
    for row in coverage_rows:
        ...
        if old and old["unknown_surface"] == surface and int(old["unknown_ordinal"]) == ordinal:
            ...
            proposal, basis, strength = f"[{surface}:?]", "NEWLY_EXPOSED_BY_GDT668_NO_NEW_CARD", "OPEN"
        strict = int(
            int(row["ambiguous_tokens"]) == 0 and int(row["reader_unstable_tokens"]) == 0
            and int(row["all_present_exact"]) == 1
        )
    ...
    for spec_row in EXACT_WHOLE_SPECS:
        ...
        glossary_rows.append({
            ...
            "source": "GDT668:CONTEXT_SHORT_DEFAULT" if surface in CONTEXT_SCOPED_SURFACES else "GDT668:EXACT_WHOLE",
        ...
        dictionary_rows.append({
            "entry": f"{surface}@GDT668_DEFAULT", "kind": card_type(surface),
            ...
            "status": "NEW_V45_PROVISIONAL_CONCRETE_RECIPE_DEFAULT",
        })
    glossary_rows.sort(key=lambda row: str(row["surface"]))
    if len(glossary_rows) != 1350:
        raise RuntimeError("V45 glossary dimension drift")
    ...
    for klass, surface, merge_surface, render in context_keys:
        ...
        context_cards.append({
            "card_id": f"G668-C{len(context_cards) + 1:03d}", "rendering_class": klass,
    ...
    next_frontier_counts = [
        {
            ...
            "status": "NEXT_V45_ONE_HOLE_FRONTIER",
        }
    ...
    for rank, row in enumerate(next_frontier_counts, 1):
        row["rank"] = rank
    if len(newly_one) != 37 or len(next_frontier_counts) != 37:
        raise RuntimeError(
            f"V45 next-frontier dimensions drift: rows={len(newly_one)} "
    ...
    for occurrence in occurrence_rows:
        ...
        audit_rows.append({
            **occurrence,
            "v44_gloss_de": split_pipe(base_row["token_glosses_de"])[ordinal - 1],
            "v45_gloss_de": split_pipe(final_row["token_glosses_de"])[ordinal - 1],
            "v44_scope_state": split_pipe(base_row["scope_states"])[ordinal - 1],
            "v45_scope_state": split_pipe(final_row["scope_states"])[ordinal - 1],
            "v45_working_translation_de": line_translation(
    ...
    if any(str(row["v44_gloss_de"]) != f"[{row['surface']}:?]" for row in audit_rows):
        raise RuntimeError("not every target occurrence was open in V44")
    if any(GENERIC_FILLER.search(str(row["v45_working_translation_de"])) for row in audit_rows):
        raise RuntimeError("generic work filler leaked into GDT668")
    if any("Eigenschafts-/Zustands-/Materialträger" in str(row["v45_working_translation_de"]) for row in audit_rows):
        raise RuntimeError("structural OL meta-gloss leaked into practical V45 prose")
    ...
    for index, spec_row in enumerate(EXACT_WHOLE_SPECS, 1):
        ...
        decision_rows.append({
            "decision_id": f"G668-D{index:03d}", "surface": surface, "family": spec_row["family"],
            ...
            "strength": card_strength(surface), "status": "ACCEPT_V45_REPLACEABLE",
        })
    ...
    for locus in sorted(affected_loci):
        ...
        target_line_rows.append({
            ...
            "zl3b_line": final_row["zl3b_line"], "v44_token_glosses_de": base_row["token_glosses_de"],
            "v45_token_glosses_de": final_row["token_glosses_de"],
            "v45_working_translation_de": line_translation(
                locus, by_line[locus], split_pipe(final_row["token_glosses_de"]),
                y_occurrence_by_token, inherited_target_by_token, target_by_token,
            ),
            "v44_unknown_tokens": base_row["unknown_tokens"], "v45_unknown_tokens": final_row["unknown_tokens"],
            "v45_complete": int(int(final_row["unknown_tokens"]) == 0),
        })
    ...
    for row in frontier:
        ...
        frontier_rows.append({
            ...
            "zl3b_line": row["zl3b_line"], "v44_translation_de": row["proposed_complete_translation_de"],
            "v45_translation_de": line_translation(
                locus, by_line[locus], split_pipe(final_row["token_glosses_de"]),
                y_occurrence_by_token, inherited_target_by_token, target_by_token,
            ),
            "status": "COMPLETE_WITH_PROVISIONAL_CONCRETE_DEFAULT",
        })
    if len(frontier_rows) != 76 or any(f"[{row['surface']}:?]" in str(row["v45_translation_de"]) for row in frontier_rows):
        raise RuntimeError("a GDT668 frontier slot remained open in V45")
    ...
    for spec_row in MANUAL_PASSAGE_SPECS:
        ...
        if int(final_row["unknown_tokens"]) != 0:
            raise RuntimeError(f"manual passage is not V45-complete: {locus}")
        ...
        manual_passage_rows.append({
            **spec_row,
            "automatic_v45_translation_de": automatic,
            "v45_unknown_tokens": final_row["unknown_tokens"],
            "comparison_scope": "manual workshop prose versus deterministic V45 renderer; neither is confirmed plaintext",
        })
    ...
    expected_base = {
        "physical_lines": 4128, "known_token_positions": 23602, "unknown_token_positions": 8737,
        "complete_multi_token_lines": 1073, "strict_complete_lines": 281,
        "one_unknown_lines": 222, "strict_one_unknown_lines": 50, "working_glossary_surfaces": 1274,
    }
    if base_metrics != expected_base:
        raise RuntimeError(f"V44 base metrics drift: {base_metrics!r}")
    round_rows = [
        {"version": "V44", "added_cards": "BASE", "dictionary_entries": len(base_dictionary), **base_metrics},
        {"version": "V45", "added_cards": f"76_DEFAULTS+{len(context_cards)}_RENDERINGS+19_OL_REVISIONS+2_SOL_REVISIONS",
    ...
    write_tsv(output_dir / "V45_WORKING_TOKEN_GLOSSARY.tsv", glossary_rows, list(base_glossary_rows[0]))
    write_tsv(output_dir / "WORKING_DICTIONARY_V45.tsv", dictionary_rows, list(base_dictionary[0]))
    write_tsv(output_dir / "ALL_LINE_CONCRETE_COVERAGE_V45.tsv", coverage_rows, coverage_fields)
    write_tsv(output_dir / "COMPLETE_PASSAGES_V45.tsv", complete_rows, complete_fields)
    write_tsv(output_dir / "ONE_UNKNOWN_PASSAGES_V45.tsv", one_rows, one_fields)
    write_tsv(
    ...
    write_tsv(
        ...
        (
            ...
            "automatic_v45_translation_de", "v45_unknown_tokens", "comparison_scope",
        ),
    )

    input_paths = (
        G667 / "REPORT.md", G667 / "artifacts/RESULT.json", G667 / "artifacts/PAGE_ALLOWLIST.tsv",
        G667 / "artifacts/V44_WORKING_TOKEN_GLOSSARY.tsv", G667 / "artifacts/WORKING_DICTIONARY_V44.tsv",
        G667 / "artifacts/ALL_LINE_CONCRETE_COVERAGE_V44.tsv", G667 / "artifacts/COMPLETE_PASSAGES_V44.tsv",
        G667 / "artifacts/ONE_UNKNOWN_PASSAGES_V44.tsv", G667 / "artifacts/NEWLY_EXPOSED_ONE_HOLE_LINES.tsv",
        G667 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        g667.G666 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        g667.g666.G665 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        g667.g666.g665.G664 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        g667.g666.g665.g664.G663 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        g667.g666.g665.g664.g663.G662 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        g667.g666.g665.g664.g663.g662.G661 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        g667.g666.g665.g664.g663.g662.g661.G660 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        g667.g666.g665.g664.g663.g662.g661.g660.G659 / "artifacts/Y_OCCURRENCE_CENSUS.tsv",
        ...
        BASE_REL / "src/RECIPE_MASTER_MEMO.md",
    ...
    result_core: dict[str, object] = {
        "schema": "GDT668_SEVENTY_SIX_RESIDUAL_FAMILY_COMPLETION_RESULT_V1",
        "experiment_id": "GDT668", "status": STATUS,
        "guard": {
        ...
        "card_synthesis": {
            "candidate_lenses": ["stem_compositor", "recipe_master", "passage_reader"],
            ...
            "inherited_stem_roles_reused": 52,
            "total_stem_roles": len(STEM_MODEL_SPECS),
            "new_stem_roles": len(STEM_MODEL_SPECS) - 52,
            "new_stem_role": "YD_POST_CLOSE|SQOKEO_SEED_HEAT_PREP",
            "historical_analogue": (
        ...
        "manual_passages": {
            ...
            "v45_complete_rows": sum(int(row["v45_unknown_tokens"]) == 0 for row in manual_passage_rows),
        ...
        "working_dictionary": {
            "v44_entries": len(base_dictionary), "v45_entries": len(dictionary_rows),
            "added_default_entries": len(EXACT_WHOLE_SPECS), "added_rendering_entries": len(context_cards),
            "v44_glossary_surfaces": len(base_glossary_rows), "v45_glossary_surfaces": len(glossary_rows),
        },
        "frontier": {
        ...
        "claim_boundary": (
            "Exploratory replaceable concrete defaults for 76 V44 residual surfaces at 230 inherited positions. "
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.artifact_dir.resolve() == ART.resolve():
        with tempfile.TemporaryDirectory(prefix="gdt668_replay_") as directory:
    ...
    print(
        f"GDT668 built: targets={result['targets']['positions']} surfaces=76 "
