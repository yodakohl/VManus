#!/usr/bin/env python3
"""Build GDT670: close the 28-form V46 frontier as concrete V47 recipe cards."""
...
BASE_REL = Path("experiments/yolo/gdt670_twenty_eight_residual_family_completion")
ART = ROOT / BASE_REL / "artifacts"
G669 = Path("experiments/yolo/gdt669_thirty_seven_residual_family_completion")
_spec = importlib.util.spec_from_file_location("gdt669_builder_for_gdt670", ROOT / G669 / "src/run.py")
if _spec is None or _spec.loader is None:
    raise RuntimeError("cannot load GDT668 builder")
g669 = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(g669)
TOKENS_REL, CROSS_REL = g669.TOKENS_REL, g669.CROSS_REL
STATUS = "PASS_91_TARGET_POSITIONS__V47_CONCRETE_RECIPE_REGISTER"
...
def read_card_specs(path: Path) -> tuple[dict[str, str], ...]:
    ...
    if not rows or set(rows[0]) != expected:
        raise RuntimeError("bad GDT670 CARD_SPECS.tsv columns")
    surfaces = [row["surface"] for row in rows]
    if len(rows) != 28 or len(set(surfaces)) != 28:
        raise RuntimeError("GDT670 CARD_SPECS.tsv must contain 28 unique surfaces")
    return tuple(rows)
...
lsheedain=1 ytey=10 qoiiin=4 tshaiin=1 ockhoees=1 ycthar=1 oeor=1
oschotshl=1 kchaiin=4 ka=2 shkaiin=5 choeky=1 cphy=9 dcheodaiin=1
qokokchy=1 otair=22 otarain=2 chekain=7 olaiiny=1 shkain=2 ofalsheky=1
chedol=5 qetal=1 rtain=1 otodar=3 alsy=1 chearory=1 secheeol=1
""")
...
OUTPUT_NAMES = (
    ...
    "FAMILY_COMPOSITION_ATLAS.tsv", "FRONTIER_28_COMPLETIONS.tsv",
    "TARGET_LINE_TRANSLATIONS.tsv", "ROUND_COVERAGE_COUNTS.tsv",
    "NEWLY_COMPLETED_LINES.tsv", "NEWLY_EXPOSED_ONE_HOLE_LINES.tsv",
    "NEXT_FRONTIER_FULL_PANEL_COUNTS.tsv",
    "V47_WORKING_TOKEN_GLOSSARY.tsv", "WORKING_DICTIONARY_V47.tsv",
    "ALL_LINE_CONCRETE_COVERAGE_V47.tsv", "COMPLETE_PASSAGES_V47.tsv",
    "ONE_UNKNOWN_PASSAGES_V47.tsv", "INHERITED_OL_RENDER_REVISIONS.tsv",
    "INHERITED_SOL_RENDER_REVISIONS.tsv", "STEM_MODEL_V47.tsv", "MANUAL_PASSAGE_AUDIT.tsv",
)
...
# These are practical render revisions only.  V46's structural tags remain
# byte-visible in its inherited glossary/dictionary; V47 translates the 19
...
def read_ol_revision_specs(path: Path) -> tuple[dict[str, str], ...]:
    ...
    if not rows or set(rows[0]) != expected:
        raise RuntimeError("bad GDT670 INHERITED_OL_REVISION_SPECS.tsv columns")
    surfaces = [row["surface"] for row in rows]
    if len(rows) != 19 or len(set(surfaces)) != 19 or sum(int(row["positions"]) for row in rows) != 256:
        raise RuntimeError("GDT670 OL revision source must contain 19 unique forms at 256 positions")
...
def read_sol_revision_specs(path: Path) -> tuple[dict[str, str], ...]:
    ...
    expected = {"surface", "positions", "practical_meaning_de", "composition", "retained_rival_de"}
    if not rows or set(rows[0]) != expected:
        raise RuntimeError("bad GDT670 INHERITED_SOL_REVISION_SPECS.tsv columns")
    surfaces = [row["surface"] for row in rows]
    if len(rows) != 2 or len(set(surfaces)) != 2 or sum(int(row["positions"]) for row in rows) != 4:
        raise RuntimeError("GDT670 SOL revision source must contain 2 unique forms at 4 positions")
    return tuple(rows)
...
# all token gloss markers at this locus.  V47 preserves that sentence but must
...
def read_stem_model_specs(path: Path) -> tuple[dict[str, str], ...]:
    ...
    if not rows or set(rows[0]) != expected or len(rows) != 56:
        raise RuntimeError("GDT670 STEM_MODEL_SPECS.tsv must contain the 56-row V47 schema")
    return tuple(rows)
...
def read_manual_passage_specs(path: Path) -> tuple[dict[str, str], ...]:
    ...
    if not rows or set(rows[0]) != expected or len(rows) != 20:
        raise RuntimeError("GDT670 MANUAL_PASSAGE_SPECS.tsv must contain the fixed 20-row schema")
    if [int(row["rank"]) for row in rows] != list(range(1, 21)):
        raise RuntimeError("GDT670 manual passage ranks must be 1..20")
    if len({row["locus"] for row in rows}) != 20:
        raise RuntimeError("GDT670 manual passage loci must be unique")
    return tuple(rows)
...
def contextual_merge_candidate(
    ...
    if ordered:
        ...
        elif surface == "ka" and direction == "RIGHT" and merged == "karar":
            render = "erhitzte Wurzelfraktion I"
        else:
...
def line_translation(
    ...
    """Render V47 with reader joins over the complete V46 renderer."""
    working_glosses = list(glosses)
    # GDT666's renderer only applies its positional action cards to the map it
    # receives as the current layer.  Re-promote the archived G669 rows here;
    ...
    inherited = {
        ...
        if not str(row.get("occurrence_id", "")).startswith("G669-")
    }
    current = {
        key: row for key, row in inherited_target_by_token.items()
        if str(row.get("occurrence_id", "")).startswith("G669-")
    }
    ...
    rendered = g669.line_translation(
...
def build(output_dir: Path) -> dict[str, object]:
    ...
    tokens, token_stats = g669.g668.g667.g666.g665.g664.g663.g662.g661.g659.guarded_query(
        TOKENS_REL, pages, "page,locus,token_index,eva,kind,section,language,hand"
    )
    cross, cross_stats = g669.g668.g667.g666.g665.g664.g663.g662.g661.g659.guarded_query(
        CROSS_REL, pages,
    ...
    base_dictionary = read_tsv(base_art / "WORKING_DICTIONARY_V46.tsv")
    base_glossary_rows = read_tsv(base_art / "V46_WORKING_TOKEN_GLOSSARY.tsv")
    base_coverage = read_tsv(base_art / "ALL_LINE_CONCRETE_COVERAGE_V46.tsv")
    base_complete = read_tsv(base_art / "COMPLETE_PASSAGES_V46.tsv")
    base_one = read_tsv(base_art / "ONE_UNKNOWN_PASSAGES_V46.tsv")
    ...
    if dimensions != (2071, 1387, 4128, 1191, 170, 28):
        raise RuntimeError(f"V46 base dimensions drift: {dimensions!r}")
    frontier_order = tuple(dict.fromkeys(row["unknown_surface"] for row in frontier))
    if frontier_order != TARGET_ORDER or len(TARGET_SURFACES) != 28:
        raise RuntimeError("the 28-form frontier or fixed order drifted")
    ...
    if any(surface in base_glossary for surface in TARGET_SURFACES):
        raise RuntimeError("a GDT670 target unexpectedly already has a V46 glossary row")
    ...
    y_occurrences = read_tsv(
        ROOT / g669.g668.g667.g666.g665.g664.g663.g662.g661.g660.G659 / "artifacts/Y_OCCURRENCE_CENSUS.tsv"
    )
    ...
    inherited_target_by_token: dict[tuple[str, int], dict[str, object]] = {}
    inherited_audits = (
        ROOT / g669.g668.g667.g666.g665.g664.g663.g662.g661.G660 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        ROOT / g669.g668.g667.g666.g665.g664.g663.g662.G661 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        ROOT / g669.g668.g667.g666.g665.g664.g663.G662 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        ROOT / g669.g668.g667.g666.g665.g664.G663 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        ROOT / g669.g668.g667.g666.g665.G664 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        ROOT / g669.g668.g667.g666.G665 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        ROOT / g669.g668.g667.G666 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        ROOT / g669.g668.G667 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        ROOT / g669.G668 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
    ...
    if observed_counts != EXPECTED_SURFACE_COUNTS or sum(observed_counts.values()) != 91:
        raise RuntimeError(f"target surface count drift: {observed_counts!r}")
    if SOLE_LEARNED_SURFACES != {"oschotshl", "qetal", "secheeol"}:
        raise RuntimeError(f"learned exact whole set drift: {sorted(SOLE_LEARNED_SURFACES)!r}")
    expected_compositions = {
        "qoiiin": "QO_COMMAND+O_PREP+IIIN_FORM_IV",
        "kchaiin": "K_HOT+CH_DRY+AIIN_III",
        "shkaiin": "SH_MOIST+K_HOT+AIIN_III",
        "shkain": "SH_MOIST+K_HOT+AIN_II",
        "otair": "O_PREP+T_COLD+AIR_FRACTION_II",
        "otarain": "O_PREP+T_COLD+AR_FRACTION_I+AIN_II",
        "otodar": "O_PREP+T_COLD+O_PREP+D_MEASURE+AR_FRACTION_I",
        "olaiiny": "O_PREP+L_WOOD+AIIN_III+Y_START_OR_CLOSE",
        "ofalsheky": "O_PREP+F_FLOWER+AL_RAW_I+SH_MOIST+E_MIDDLE+K_HOT+Y_START_OR_CLOSE",
    }
    if any(EXACT_BY_SURFACE[surface]["composition"] != composition
           for surface, composition in expected_compositions.items()):
        raise RuntimeError("V47 OT/SHK/KCH/qoiiin/olaiiny/ofal composition drift")
    if EXACT_BY_SURFACE["otodar"]["working_meaning_de"] != (
        "Kaltansatz erneut ansetzen und die erste Fraktion abmessen"
    ):
        raise RuntimeError("otodar must render both visible O_PREP blocks")
    if CONTEXT_SCOPED_SURFACES != {"ka"}:
        raise RuntimeError("ka must be the sole context-scoped short form")
    exact, normalized = g669.g668.g667.g666.g665.g664.g663.g662.g661.g660.stable_maps(tokens, cross_by_locus)
    ...
    for locus in sorted(by_line):
        ...
        # At f40r.10 both alternate readers bind ZL3b's visible ``ka ar`` as
        # ``karar``.  The additional reader ``r`` is not an invisible stem in
        # the card: this is one explicitly bounded reader alias for local
        # rendering, while the exact ``ka`` card remains K_HOT+A_PART_OR_LINK.
        if (locus == "f40r.10" and words[4:6] == ["ka", "ar"]
                and all("karar" in cross_by_locus[locus][field].split()
                        for field in ("it2a_clean", "rf1b_clean"))):
            local_merge_evidence[4] = [
                ("RIGHT", "karar", "IT2a"), ("RIGHT", "karar", "RF1b")
            ]
        for index, token in enumerate(line):
            ...
            item: dict[str, object] = {
                "occurrence_id": f"G670-T{len(occurrence_rows) + 1:04d}",
    ...
    if len(occurrence_rows) != 91 or len(target_by_token) != 91:
        raise RuntimeError("target occurrence census drift")
    if (len({row["locus"] for row in occurrence_rows}) != 90
            or len({row["page"] for row in occurrence_rows}) != 64
            or sum(row["locus"] == "f105v.30" and row["surface"] == "otair"
                   for row in occurrence_rows) != 2):
        raise RuntimeError("target 90-line/64-page census or double-otair locus drift")
    ...
    ka_rows = [row for row in contextual_rows if row["surface"] == "ka"]
    ka_merges = [row for row in ka_rows if row["reader_merge_direction"] == "RIGHT"]
    ka_free = [row for row in ka_rows if row["reader_merge_direction"] == "NONE"]
    if (len(ka_merges) != 1 or len(ka_free) != 1
            or ka_merges[0]["locus"] != "f40r.10"
            or ka_merges[0]["reader_merge_surface"] != "karar"
            or ka_merges[0]["working_render_de"] != "erhitzte Wurzelfraktion I"
            or ka_free[0]["locus"] != "f32v.3"
            or ka_free[0]["working_render_de"] != "ein erhitzter Teil"):
        raise RuntimeError(f"ka local RIGHT-merge/free-position contract drift: {ka_rows!r}")
    ...
    for base_row in base_coverage:
        ...
        if not (len(line) == len(glosses) == len(sources) == len(states)):
            raise RuntimeError(f"V46 token columns misalign: {locus}")
        ...
        for index, token in enumerate(line):
            ...
            if glosses[index] != f"[{surface}:?]" or sources[index] != "OPEN" or states[index] != "UNKNOWN_SURFACE":
                raise RuntimeError(f"V46 target not open at {locus}.{index + 1}: {surface}")
            glosses[index] = str(occurrence["working_gloss_de"])
            if surface in CONTEXT_SCOPED_SURFACES:
                sources[index] = f"GDT670:{occurrence['rendering_class']}:{occurrence['reader_merge_surface']}"
                states[index] = "KNOWN_CONTEXT_LICENSED" if int(occurrence["reader_exact"]) else "READER_BOUNDARY_UNSTABLE"
            else:
                sources[index] = f"GDT670:EXACT_WHOLE:{surface}"
        ...
        if int(base_row["unknown_tokens"]) - len(target_ordinals) != len(unknown_pairs):
            raise RuntimeError(f"V46->V47 arithmetic drift: {locus}")
    ...
    for row in coverage_rows:
        ...
        if old and old["unknown_surface"] == surface and int(old["unknown_ordinal"]) == ordinal:
            ...
            proposal, basis, strength = f"[{surface}:?]", "NEWLY_EXPOSED_BY_GDT670_NO_NEW_CARD", "OPEN"
        strict = int(
            int(row["ambiguous_tokens"]) == 0 and int(row["reader_unstable_tokens"]) == 0
            and int(row["all_present_exact"]) == 1
        )
    ...
    for spec_row in EXACT_WHOLE_SPECS:
        ...
        glossary_rows.append({
            ...
            "source": "GDT670:CONTEXT_SHORT_DEFAULT" if surface in CONTEXT_SCOPED_SURFACES else "GDT670:EXACT_WHOLE",
        ...
        dictionary_rows.append({
            "entry": f"{surface}@GDT670_DEFAULT", "kind": card_type(surface),
            ...
            "status": "NEW_V47_PROVISIONAL_CONCRETE_RECIPE_DEFAULT",
        })
    glossary_rows.sort(key=lambda row: str(row["surface"]))
    if len(glossary_rows) != 1415:
        raise RuntimeError("V47 glossary dimension drift")
    ...
    for klass, surface, merge_surface, render in context_keys:
        ...
        context_cards.append({
            "card_id": f"G670-C{len(context_cards) + 1:03d}", "rendering_class": klass,
    ...
    next_frontier_groups: dict[str, list[dict[str, object]]] = defaultdict(list)
    for row in newly_one:
        next_frontier_groups[str(row["unknown_surface"])].append(row)
    next_frontier_counts = [
        {
            "rank": 0,
            "surface": surface,
            "full_panel_positions": len(tokens_by_surface[surface]),
            "newly_exposed_lines": len(rows),
            "first_locus": min(str(row["locus"]) for row in rows),
            "status": "NEXT_V47_ONE_HOLE_FRONTIER",
        }
        for surface, rows in next_frontier_groups.items()
    ]
    next_frontier_counts.sort(
        key=lambda row: (-int(row["full_panel_positions"]), str(row["surface"]))
    )
    for rank, row in enumerate(next_frontier_counts, 1):
        row["rank"] = rank
    if (not newly_one or len(newly_one) != len(next_frontier_counts)
            or len({row["unknown_surface"] for row in newly_one}) != len(newly_one)):
        raise RuntimeError(
            f"V47 next-frontier dimensions drift: rows={len(newly_one)} "
            f"surfaces={len(next_frontier_counts)}"
        )
    ...
    for occurrence in occurrence_rows:
        ...
        audit_rows.append({
            **occurrence,
            "v46_gloss_de": split_pipe(base_row["token_glosses_de"])[ordinal - 1],
            "v47_gloss_de": split_pipe(final_row["token_glosses_de"])[ordinal - 1],
            "v46_scope_state": split_pipe(base_row["scope_states"])[ordinal - 1],
            "v47_scope_state": split_pipe(final_row["scope_states"])[ordinal - 1],
            "v47_working_translation_de": line_translation(
    ...
    if any(str(row["v46_gloss_de"]) != f"[{row['surface']}:?]" for row in audit_rows):
        raise RuntimeError("not every target occurrence was open in V46")
    if any(GENERIC_FILLER.search(str(row["v47_working_translation_de"])) for row in audit_rows):
        raise RuntimeError("generic work filler leaked into GDT670")
    if any("Eigenschafts-/Zustands-/Materialträger" in str(row["v47_working_translation_de"]) for row in audit_rows):
        raise RuntimeError("structural OL meta-gloss leaked into practical V47 prose")
    ...
    for index, spec_row in enumerate(EXACT_WHOLE_SPECS, 1):
        ...
        decision_rows.append({
            "decision_id": f"G670-D{index:03d}", "surface": surface, "family": spec_row["family"],
            ...
            "strength": card_strength(surface), "status": "ACCEPT_V47_REPLACEABLE",
        })
    ...
    for locus in sorted(affected_loci):
        ...
        target_line_rows.append({
            ...
            "zl3b_line": final_row["zl3b_line"], "v46_token_glosses_de": base_row["token_glosses_de"],
            "v47_token_glosses_de": final_row["token_glosses_de"],
            "v47_working_translation_de": line_translation(
                locus, by_line[locus], split_pipe(final_row["token_glosses_de"]),
                y_occurrence_by_token, inherited_target_by_token, target_by_token,
            ),
            "v46_unknown_tokens": base_row["unknown_tokens"], "v47_unknown_tokens": final_row["unknown_tokens"],
            "v47_complete": int(int(final_row["unknown_tokens"]) == 0),
        })
    ...
    for row in frontier:
        ...
        frontier_rows.append({
            ...
            "zl3b_line": row["zl3b_line"], "v46_translation_de": row["proposed_complete_translation_de"],
            "v47_translation_de": line_translation(
                locus, by_line[locus], split_pipe(final_row["token_glosses_de"]),
                y_occurrence_by_token, inherited_target_by_token, target_by_token,
            ),
            "status": "COMPLETE_WITH_PROVISIONAL_CONCRETE_DEFAULT",
        })
    if len(frontier_rows) != 28 or any(f"[{row['surface']}:?]" in str(row["v47_translation_de"]) for row in frontier_rows):
        raise RuntimeError("a GDT670 frontier slot remained open in V47")
    ...
    for spec_row in MANUAL_PASSAGE_SPECS:
        ...
        if int(final_row["unknown_tokens"]) != 0:
            raise RuntimeError(f"manual passage is not V47-complete: {locus}")
        ...
        manual_passage_rows.append({
            **spec_row,
            "automatic_v47_translation_de": automatic,
            "v47_unknown_tokens": final_row["unknown_tokens"],
            "comparison_scope": "manual workshop prose versus deterministic V47 renderer; neither is confirmed plaintext",
        })
    ...
    expected_base = {
        "physical_lines": 4128, "known_token_positions": 23997, "unknown_token_positions": 8342,
        "complete_multi_token_lines": 1191, "strict_complete_lines": 306,
        "one_unknown_lines": 170, "strict_one_unknown_lines": 38, "working_glossary_surfaces": 1387,
    }
    if base_metrics != expected_base:
        raise RuntimeError(f"V46 base metrics drift: {base_metrics!r}")
    expected_final_projection = {
        "physical_lines": 4128,
        "known_token_positions": 24088,
        "unknown_token_positions": 8251,
        "complete_multi_token_lines": 1223,
        "one_unknown_lines": 154,
        "working_glossary_surfaces": 1415,
    }
    if any(final_metrics[key] != value for key, value in expected_final_projection.items()):
        raise RuntimeError(f"V47 projected coverage drift: {final_metrics!r}")
    round_rows = [
        {"version": "V46", "added_cards": "BASE", "dictionary_entries": len(base_dictionary), **base_metrics},
        {"version": "V47", "added_cards": f"28_DEFAULTS+{len(context_cards)}_RENDERINGS+19_OL_REVISIONS+2_SOL_REVISIONS",
    ...
    write_tsv(output_dir / "FRONTIER_28_COMPLETIONS.tsv", frontier_rows, list(frontier_rows[0]))
    ...
    write_tsv(
        output_dir / "NEXT_FRONTIER_FULL_PANEL_COUNTS.tsv",
        next_frontier_counts,
        ("rank", "surface", "full_panel_positions", "newly_exposed_lines", "first_locus", "status"),
    )
    write_tsv(output_dir / "V47_WORKING_TOKEN_GLOSSARY.tsv", glossary_rows, list(base_glossary_rows[0]))
    write_tsv(output_dir / "WORKING_DICTIONARY_V47.tsv", dictionary_rows, list(base_dictionary[0]))
    write_tsv(output_dir / "ALL_LINE_CONCRETE_COVERAGE_V47.tsv", coverage_rows, coverage_fields)
    write_tsv(output_dir / "COMPLETE_PASSAGES_V47.tsv", complete_rows, complete_fields)
    write_tsv(output_dir / "ONE_UNKNOWN_PASSAGES_V47.tsv", one_rows, one_fields)
    write_tsv(
    ...
    write_tsv(
        ...
        (
            ...
            "automatic_v47_translation_de", "v47_unknown_tokens", "comparison_scope",
        ),
    )

    input_paths = (
        G669 / "REPORT.md", G669 / "artifacts/RESULT.json", G669 / "artifacts/PAGE_ALLOWLIST.tsv",
        G669 / "artifacts/V46_WORKING_TOKEN_GLOSSARY.tsv", G669 / "artifacts/WORKING_DICTIONARY_V46.tsv",
        G669 / "artifacts/ALL_LINE_CONCRETE_COVERAGE_V46.tsv", G669 / "artifacts/COMPLETE_PASSAGES_V46.tsv",
        G669 / "artifacts/ONE_UNKNOWN_PASSAGES_V46.tsv", G669 / "artifacts/NEWLY_EXPOSED_ONE_HOLE_LINES.tsv",
        G669 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        g669.g668.g667.G666 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        g669.g668.G667 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        g669.G668 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        g669.g668.g667.g666.G665 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        g669.g668.g667.g666.g665.G664 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        g669.g668.g667.g666.g665.g664.G663 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        g669.g668.g667.g666.g665.g664.g663.G662 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        g669.g668.g667.g666.g665.g664.g663.g662.G661 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        g669.g668.g667.g666.g665.g664.g663.g662.g661.G660 / "artifacts/ALL_OCCURRENCE_SEMANTIC_AUDIT.tsv",
        g669.g668.g667.g666.g665.g664.g663.g662.g661.g660.G659 / "artifacts/Y_OCCURRENCE_CENSUS.tsv",
        ...
        BASE_REL / "src/CARD_SPECS_APOTHECARY_CANDIDATE.tsv",
        BASE_REL / "src/CARD_SPECS_PASSAGE_CANDIDATE.tsv",
        BASE_REL / "src/CARD_SPECS_STEM_CANDIDATE.tsv",
        BASE_REL / "src/APOTHECARY_MEMO.md",
        BASE_REL / "src/PASSAGE_READER_MEMO.md",
        BASE_REL / "src/PASSAGE_SELECTION_MEMO.md",
        BASE_REL / "src/STEM_COMPOSITOR_MEMO.md",
        BASE_REL / "src/STEM_EXTENSION_CANDIDATES.tsv",
        BASE_REL / "src/FINAL_CARD_AUDIT.md",
        BASE_REL / "src/MANUAL_PASSAGE_SPECS_READER.tsv",
        BASE_REL / "src/FINAL_PASSAGE_AUDIT.md",
        TOKENS_REL, CROSS_REL,
    )
    output_paths = [output_dir / name for name in OUTPUT_NAMES]
    result_core: dict[str, object] = {
        "schema": "GDT670_TWENTY_EIGHT_RESIDUAL_FAMILY_COMPLETION_RESULT_V1",
        "experiment_id": "GDT670", "status": STATUS,
        "guard": {
        ...
        "card_synthesis": {
            "candidate_lenses": [
                "practical_apothecary", "stem_compositor", "passage_reader"
            ],
            ...
            "inherited_stem_roles_reused": 56,
            "total_stem_roles": len(STEM_MODEL_SPECS),
            "new_stem_roles": len(STEM_MODEL_SPECS) - 56,
            "new_stem_role": "NONE",
            "historical_analogue": (
        ...
        "manual_passages": {
            ...
            "v47_complete_rows": sum(int(row["v47_unknown_tokens"]) == 0 for row in manual_passage_rows),
        ...
        "working_dictionary": {
            "v46_entries": len(base_dictionary), "v47_entries": len(dictionary_rows),
            "added_default_entries": len(EXACT_WHOLE_SPECS), "added_rendering_entries": len(context_cards),
            "v46_glossary_surfaces": len(base_glossary_rows), "v47_glossary_surfaces": len(glossary_rows),
        },
        "frontier": {
            "source_rows": len(frontier), "completed_rows": len(frontier_rows),
            "unfilled_target_slots": 0, "next_rows": len(newly_one),
            "next_unique_surfaces": len(next_frontier_counts),
            "next_leaders": [
                {"surface": row["surface"], "full_panel_positions": row["full_panel_positions"]}
                for row in next_frontier_counts[:10]
            ],
        },
        ...
        "claim_boundary": (
            "Exploratory replaceable concrete defaults for 28 V46 residual surfaces at 91 inherited positions. "
            f"The final synthesis uses {len(TARGET_SURFACES - SOLE_LEARNED_SURFACES)} role-composed cards "
            ...
            "while naked ol stays a learned exact whole and oly/olyly stay actions. Two inherited sol forms retain "
            "the manuscript-internal sol=seed renderer at four positions; the salt rival remains recorded. Practical German renderers do not "
            "alter the structural glossary. No confirmed plaintext, language, phonetics, exact plant "
            "identity, patient, disease, cure, carrier liquid, new page, image, f1r, f84 or f84r is asserted."
        ),
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.artifact_dir.resolve() == ART.resolve():
        with tempfile.TemporaryDirectory(prefix="gdt670_replay_") as directory:
    ...
    print(
        f"GDT670 built: targets={result['targets']['positions']} surfaces=28 "
