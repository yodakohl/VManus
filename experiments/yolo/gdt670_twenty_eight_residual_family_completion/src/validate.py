#!/usr/bin/env python3
"""Independent source-first release validator for GDT670."""
...
BASE = Path("experiments/yolo/gdt670_twenty_eight_residual_family_completion")
...
OUTPUT_NAMES = (
    ...
    "FAMILY_COMPOSITION_ATLAS.tsv", "STEM_MODEL_V47.tsv",
    "INHERITED_OL_RENDER_REVISIONS.tsv", "INHERITED_SOL_RENDER_REVISIONS.tsv",
    "MANUAL_PASSAGE_AUDIT.tsv",
    "FRONTIER_28_COMPLETIONS.tsv", "TARGET_LINE_TRANSLATIONS.tsv",
    "ROUND_COVERAGE_COUNTS.tsv", "NEWLY_COMPLETED_LINES.tsv",
    "NEWLY_EXPOSED_ONE_HOLE_LINES.tsv", "NEXT_FRONTIER_FULL_PANEL_COUNTS.tsv",
    "V47_WORKING_TOKEN_GLOSSARY.tsv",
    "WORKING_DICTIONARY_V47.tsv", "ALL_LINE_CONCRETE_COVERAGE_V47.tsv",
    "COMPLETE_PASSAGES_V47.tsv", "ONE_UNKNOWN_PASSAGES_V47.tsv",
)

TARGET_ORDER = tuple("""
lsheedain ytey qoiiin tshaiin ockhoees ycthar oeor oschotshl kchaiin ka
shkaiin choeky cphy dcheodaiin qokokchy otair otarain chekain olaiiny shkain
ofalsheky chedol qetal rtain otodar alsy chearory secheeol
""".split())
...
lsheedain=1 ytey=10 qoiiin=4 tshaiin=1 ockhoees=1 ycthar=1 oeor=1 oschotshl=1
kchaiin=4 ka=2 shkaiin=5 choeky=1 cphy=9 dcheodaiin=1 qokokchy=1 otair=22
otarain=2 chekain=7 olaiiny=1 shkain=2 ofalsheky=1 chedol=5 qetal=1 rtain=1
otodar=3 alsy=1 chearory=1 secheeol=1
""")
...
def main() -> int:
    ...
    # guarded selector before trusting any GDT670 result or generated artifact.
    base_pages = {row["page"] for row in read_tsv(ROOT / G669 / "artifacts/PAGE_ALLOWLIST.tsv")}
    ...
    check("target count sum", sum(EXPECTED_COUNTS.values()) == 91)
    ...
    check("CARD_SPECS 28 unique forms", len(specs) == len(set(spec_order)) == 28)
    check("context-short form is exactly ka", CONTEXT_SURFACES == {"ka"})
    check("CARD_SPECS all fields populated", all(all(row[field] for field in spec_fields) for row in specs))
    check("CARD_SPECS raw positions", sum(raw_counts[row["surface"]] for row in specs) == 91)
    ...
    stem_model = read_tsv(ART / "STEM_MODEL_V47.tsv")
    ...
    frontier = read_tsv(ART / "FRONTIER_28_COMPLETIONS.tsv")
    target_lines = read_tsv(ART / "TARGET_LINE_TRANSLATIONS.tsv")
    rounds = read_tsv(ART / "ROUND_COVERAGE_COUNTS.tsv")
    glossary = read_tsv(ART / "V47_WORKING_TOKEN_GLOSSARY.tsv")
    dictionary = read_tsv(ART / "WORKING_DICTIONARY_V47.tsv")
    coverage = read_tsv(ART / "ALL_LINE_CONCRETE_COVERAGE_V47.tsv")
    complete = read_tsv(ART / "COMPLETE_PASSAGES_V47.tsv")
    one = read_tsv(ART / "ONE_UNKNOWN_PASSAGES_V47.tsv")
    ...
    base_glossary = read_tsv(base_art / "V46_WORKING_TOKEN_GLOSSARY.tsv")
    ...
    check("decision dimensions", len(deck) == len(accepted) == len(family) == 28)
    ...
    check("architecture position total", sum(int(row["positions"]) for row in architecture) == 91)

    audit_counts = Counter(row["surface"] for row in audit)
    check("occurrence audit dimensions", len(audit) == len(reader) == 91)
    check("occurrence audit exact counts", dict(audit_counts) == EXPECTED_COUNTS, dict(audit_counts))
    check("target occurrence geometry", len({row["locus"] for row in audit}) == 90
          and len({row["page"] for row in audit}) == 64)
    check("f105v.30 contains the two otair positions", sum(
        row["surface"] == "otair" and row["locus"] == "f105v.30" for row in audit
    ) == 2)
    check("occurrence IDs are G670", all(row["occurrence_id"].startswith("G670-") for row in audit))
    check("all target slots were V46-open", all(row["v46_gloss_de"] == f"[{row['surface']}:?]" for row in audit))
    check("all target slots are V47-filled", all(
        row["v47_gloss_de"] and not OPEN_MARKER.search(row["v47_gloss_de"]) for row in audit
    ))
    ...
    check("audit practical prose has no generic work filler", not any(
        GENERIC.search(row["v47_working_translation_de"]) for row in audit
    ))
    check("target-line practical prose has no generic work filler", not any(
        GENERIC.search(row["v47_working_translation_de"]) for row in target_lines
    ))
    ...
    ka_rows = [row for row in reader if row["surface"] == "ka"]
    ka_merge = [row for row in ka_rows if row["locus"] == "f40r.10"]
    ka_free = [row for row in ka_rows if row["locus"] == "f32v.3"]
    check("ka has exactly its two named positions", len(ka_rows) == 2
          and {row["locus"] for row in ka_rows} == {"f40r.10", "f32v.3"})
    check("ka f40r.10 has bilateral RIGHT merge karar", len(ka_merge) == 1
          and ka_merge[0]["reader_merge_direction"] == "RIGHT"
          and ka_merge[0]["reader_merge_surface"] == "karar"
          and set(ka_merge[0]["reader_merge_readers"].split("+")) == {"IT2a", "RF1b"})
    check("ka f32v.3 remains free", len(ka_free) == 1
          and ka_free[0]["reader_merge_direction"] == "NONE"
          and ka_free[0]["reader_merge_surface"] == "NONE")

    ofal_rows = [row for row in reader if row["surface"] == "ofalsheky"]
    otar_rows = [row for row in reader if row["surface"] == "otarain"]
    chedol_rows = [row for row in reader if row["surface"] == "chedol"]
    check("ofalsheky has bilateral ofal sheky support", len(ofal_rows) == 1
          and "ofalsheky" in ofal_rows[0]["zl3b_line"].split()
          and "ofal sheky" in ofal_rows[0]["it2a_line"]
          and "ofal sheky" in ofal_rows[0]["rf1b_line"])
    check("otarain has bilateral otar ain support", len(otar_rows) == 2
          and any("otar ain" in row["it2a_line"] and "otar ain" in row["rf1b_line"]
                  for row in otar_rows))
    check("chedol retains the RF1b ched l rival", len(chedol_rows) == 5
          and any("ched l" in row["rf1b_line"] for row in chedol_rows)
          and specs_by_surface["chedol"]["strongest_rival_de"]
          == "RF1b: bis zur Mittelstufe trocknen und abschließen; ein Pfund")
    ...
    for row in audit:
        ...
        if merged == "NONE":
            ...
            continue
        if row["surface"] == "ka" and row["locus"] == "f40r.10":
            # This named local alignment is not plain concatenation: ZL `ka ar`
            # corresponds to the single token `karar` in both alternate
            # readers.  Seal the observed substitution directly rather than
            # weakening the generic exact-concatenation aligner.
            check(f"named ka merge direction {row['occurrence_id']}", direction == "RIGHT", direction)
            check(f"named ka source span {row['occurrence_id']}", row["right_surface"] == "ar")
            check(f"named ka alternate token {row['occurrence_id']}", merged == "karar"
                  and "karar" in row["it2a_line"].split()
                  and "karar" in row["rf1b_line"].split())
            check(f"named ka reader witnesses {row['occurrence_id']}",
                  set(row["reader_merge_readers"].split("+")) == {"IT2a", "RF1b"})
            check(f"merge decision {row['occurrence_id']}",
                  row["reader_merge_decision"] == "ACCEPT_LOCAL_MINIMUM_ALIGNMENT")
            check(f"merge class explicit {row['occurrence_id']}",
                  row["rendering_class"] == "KA_READER_MERGE_RIGHT_NOVEL")
            continue
    ...
    check("only named GDT670 boundary forms reader-dispatch", all(
        row["surface"] in CONTEXT_SURFACES for row in merge_rows
    ))

    check("stem-model artifact byte-equivalent to source", (ART / "STEM_MODEL_V47.tsv").read_bytes() == STEM_SPECS.read_bytes())
    ...
    check("final synthesis is 25 role-composed plus 3 learned", len(learned_wholes) == 3
          and len(specs) - len(learned_wholes) == 25)
    ...
    check("learned exact set is frozen",
          learned_wholes == {"oschotshl", "qetal", "secheeol"})
    check("no new role is introduced", len(stem_model) == 56
          and STEM_SPECS.read_bytes() == (ROOT / G669 / "artifacts/STEM_MODEL_V46.tsv").read_bytes())
    check("initial olaiiny obeys O_PREP plus L_WOOD",
          card_atoms["olaiiny"]
          == ["O_PREP", "L_WOOD", "AIIN_III", "Y_START_OR_CLOSE"])
    check("qoiiin retains explicit O_PREP",
          card_atoms["qoiiin"] == ["QO_COMMAND", "O_PREP", "IIIN_FORM_IV"])
    check("ofalsheky uses only its named F licence",
          card_atoms["ofalsheky"]
          == ["O_PREP", "F_FLOWER", "AL_RAW_I", "SH_MOIST", "E_MIDDLE",
              "K_HOT", "Y_START_OR_CLOSE"]
          and all("F_FLOWER" not in atoms for surface, atoms in card_atoms.items()
                  if surface != "ofalsheky"))
    check("otodar wording preserves repeated preparation",
          specs_by_surface["otodar"]["working_meaning_de"]
          == "Kaltansatz erneut ansetzen und die erste Fraktion abmessen"
          and card_atoms["otodar"]
          == ["O_PREP", "T_COLD", "O_PREP", "D_MEASURE", "AR_FRACTION_I"])

    for surface, expected in {
        "otair": ["O_PREP", "T_COLD", "AIR_FRACTION_II"],
        "ytey": ["Y_REFERENCE", "T_COLD", "E_MIDDLE", "Y_START_OR_CLOSE"],
        "cphy": ["CPH_COMPOSITE", "Y_START_OR_CLOSE"],
        "chekain": ["CH_DRY", "E_MIDDLE", "K_HOT", "AIN_II"],
        "shkaiin": ["SH_MOIST", "K_HOT", "AIIN_III"],
        "kchaiin": ["K_HOT", "CH_DRY", "AIIN_III"],
        "chedol": ["CH_DRY", "E_MIDDLE", "D_MEASURE", "OL_MATERIAL"],
    }.items():
    ...
    check("V47 glossary adds exactly 28 targets", len(glossary) == len(base_glossary) + 28
          and len(glossary) == 1415)
    ...
    check("frontier dimensions", len(frontier) == len(base_frontier) == 28)
    ...
    check("frontier completely closed", all(
        ...
        and not OPEN_MARKER.search(row["v47_translation_de"])
        and not GENERIC.search(row["v47_translation_de"])
        for row in frontier
    ))
    expected_next_surfaces = {row["unknown_surface"] for row in new_one}
    check("next-frontier dimensions are builder-derived", len(new_one) == len(next_frontier)
          and len(expected_next_surfaces) == len(next_frontier))
    ...
    for rank in map(str, range(1, 21)):
        ...
        check(f"manual V47 complete {rank}", row["v47_unknown_tokens"] == "0"
              and row["locus"] in complete_by_locus)
        check(f"manual automatic renderer identity {rank}", row["automatic_v47_translation_de"]
              == complete_by_locus[row["locus"]]["working_translation_de"])
        check(f"manual and automatic channels distinct {rank}", "manual workshop prose versus deterministic V47 renderer" in row["comparison_scope"])
        check(f"manual prose concrete {rank}", not GENERIC.search(row["manual_workshop_translation_de"])
              ...
              and not GENERIC.search(row["automatic_v47_translation_de"])
              and not OPEN_MARKER.search(row["automatic_v47_translation_de"]))
    ...
    check("non-target position census", len(non_target_before) == 32339 - 91, len(non_target_before))
    ...
    check("known delta exactly 91", final_metrics["known_token_positions"] - base_metrics["known_token_positions"] == 91)
    check("unknown delta exactly 91", base_metrics["unknown_token_positions"] - final_metrics["unknown_token_positions"] == 91)
    check("V46 base dimensions", base_metrics == {
        "physical_lines": 4128, "known_token_positions": 23997,
        "unknown_token_positions": 8342, "complete_multi_token_lines": 1191,
        "strict_complete_lines": 306, "one_unknown_lines": 170,
        "strict_one_unknown_lines": 38, "working_glossary_surfaces": 1387,
    } and len(base_dictionary) == 2071)
    check("V47 projected coverage totals", final_metrics["known_token_positions"] == 24088
          and final_metrics["unknown_token_positions"] == 8251
          and final_metrics["complete_multi_token_lines"] == 1223
          and final_metrics["one_unknown_lines"] == 154
          and final_metrics["working_glossary_surfaces"] == 1415)
    check("V47 dictionary adds exact defaults and context renderings",
          len(dictionary) == len(base_dictionary) + len(specs) + len(context))
    ...
    check("round versions", [row["version"] for row in rounds] == ["V46", "V47"])
    ...
    check("result schema", result["schema"] == "GDT670_TWENTY_EIGHT_RESIDUAL_FAMILY_COMPLETION_RESULT_V1")
    check("result status", result["status"] == "PASS_91_TARGET_POSITIONS__V47_CONCRETE_RECIPE_REGISTER")
    check("result target dimensions", result["targets"]["surface_types"] == 28 and result["targets"]["positions"] == 91)
    check("result target counts", result["targets"]["surface_counts"] == EXPECTED_COUNTS)
    check("result frontier dimensions", result["frontier"]["source_rows"] == result["frontier"]["completed_rows"] == 28
    ...
    check("result dictionary dimensions", result["working_dictionary"]["v46_entries"] == len(base_dictionary) and result["working_dictionary"]["v47_entries"] == len(dictionary))
    check("result glossary dimensions", result["working_dictionary"]["v46_glossary_surfaces"] == len(base_glossary) and result["working_dictionary"]["v47_glossary_surfaces"] == len(glossary))
    ...
    check("result card-synthesis dimensions", result["card_synthesis"]["productive_compositions"] == 25
          and result["card_synthesis"]["learned_exact_wholes"] == 3
    ...
    check("result manual-passage dimensions", result["manual_passages"]["rows"] == 20
          ...
          and result["manual_passages"]["v47_complete_rows"] == 20
    ...
    with tempfile.TemporaryDirectory(prefix="gdt670_validator_replay_") as directory:
    ...
    validation = {
        "schema": "GDT670_VALIDATION_V1", "experiment_id": "GDT670",
    ...
    print(f"GDT670 validation: {validation['status']} {validation['checks_passed']}/{validation['checks_total']}")
    if failures:
