...
m.update(title='Exploratory do label/prose contrast',question='Does the ofaldo/ofal label/prose surface contrast recur for other bases?'  ,claim_ceiling='Candidate capacity only; no held prediction, calibrated null, segmentation mechanism or semantics.',status=status,dependencies=['GDT631','GDT829'],inputs=[{'path':n,'sha256':sha(ROOT/n),'role':'frozen_source_or_extractor'} for n in up],outputs=[{'path':p.relative_to(ROOT).as_posix(),'sha256':sha(p),'role':'primary_report' if p.name=='REPORT.md' else 'source_or_artifact'} for p in outputs],validation={'status':'PASS' if result.exists() else 'NOT_RUN','artifact':rel+'/artifacts/VALIDATION.json' if result.exists() else None})
