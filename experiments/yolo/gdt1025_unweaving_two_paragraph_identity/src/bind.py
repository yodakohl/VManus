...
s=source();paths=[b['path'] for b in s['source_bindings']]+['research_registry/proposals/raw_f83r_unweaving_second_paragraph_frozen24_20260921.json']
paths += [str((OLD/f).relative_to(R)) for f in ['src/common.py','src/model.py','src/independent.py','src/SOURCE.json','src/SPEC.json','artifacts/ROWS.json','REPORT.md','PREREG_LOCK.json']]
...
m=read(E/'experiment.json');m.update(question='Can the complete fixed unweaving summary and original paragraph share all24 old meanings, state scopes and activity identities, and does their explicit shared-frame assumption distinguish the old two schedules?',claim_ceiling='Conditional96group projected account;40 new guesses,11 clauses,7 binding conventions; no independent meaning, source identity or confirmed word.',dependencies=['GDT928','GDT1020'],status=args.status,inputs=[dict(path=p,sha256=sha(R/p),role='fixed_input') for p in paths],outputs=[])
