...
def main():
    rows=[]
    for j in fixtures():
        a=primary.evaluate(j['parse'],j['variant']);b=independent.evaluate(j['parse'],j['variant'])
        assert a['status']==b['status'],(j['id'],a,b)
        if j['expected']:assert a['status']==j['expected'],(j['id'],a)
        if a['status']!='INVALID':assert (a['successful_paths'],a['failed_prefixes'])==(b['successful_paths'],b['failed_prefixes']),(j['id'],a,b)
        rows.append(dict(id=j['id'],status=a['status']))
    result=dict(status='PASS',cases=len(rows),counts=dict(collections.Counter(r['status'] for r in rows)),cases_detail=rows,scope='Complete synthetic narratives only; no new target predicate run.')
    put('PREFLIGHT.json',result);print(json.dumps({k:v for k,v in result.items() if k!='cases_detail'}))
