"""Render two frozen conditional-clause hypotheses, keeping all source groups and gaps."""
...
def build():
    ...
    inherited={k:dict(base) for k in m['models']}
    for prior_path in m['prior_extensions']:
        prior=json.loads((ROOT/prior_path).read_text())['models']
        for k in m['models']:inherited[k].update(prior[k]['lexicon'])
    ...
    for e in EDS:
        ...
        d=[f'# GDT936 — vollständige Quellausrichtung {e}', '', 'R=wenn/dann gerichtete Bewegung, T=wenn/dann späterer Ruhezustand; dann ist nur deutsche Erläuterung, kein zusätzlicher Wortwert. Alle deutschen Werte sind Hypothesen.', 'Rohgruppen bleiben erhalten: / = unsicherer kleiner Abstand; // = Zeichnungsunterbrechung. Kein deutscher Fließtext für unbekannte Gruppen.', 'Die Erweiterungen bewahren GDT932-M und GDT936-R/T. Q und V bleiben offene Gegenlesungen.', '']
        lastblock=None
    ...
    result={'experiment':'GDT936','status':'TWO_FULL_CONDITIONAL_CLAUSE_DRAFTS_UNSELECTED','source_groups':len(src),'all_alignment_rows':len(alignment),'target_occurrences':len(cases)//2,'candidate_occurrence_rows':len(cases),'counts':counts,'target_pages':sorted(set(r['locus'].split('.')[0] for r in cases)),'target_loci':sorted(set(r['locus'] for r in cases),key=lambda x:(PAGES.index(x.split('.')[0]),int(x.split('.')[1]))),'selected_translation':None,'confirmed_words':0,'independent_meaning_tests':0,'unexposed_confirmation_folios':0,'new_admissions':0,'significance_claimed':False,'semantics_validated':False,'meaning_selection_by_counts':False,'base_V_changed':False,'base_Q_changed':False,
        'new_form_occurrences':sum(r['ivtff_group_raw'] in m['new_forms'] for r in src),
        'line37_known_groups':{e:{'total':len(lines[e,'f77r.37']),'known':sum(r['ivtff_group_raw'] in lex['R'] for r in lines[e,'f77r.37'])} for e in EDS},
        'line37_interpretation':'Complete seven-group conditional hypothesis ZL/IT, five of seven groups RF; no factual warmth, motion or rest inferred.'}
    local=[]
    for e in EDS:
        lookup={r['ivtff_group_raw']:r['source_group_id'] for r in lines[e,'f77r.37']}
        for mid in m['models']:
            complete=all(w in lookup for w in ['teeolain','chey','qoteedy','qokain','cheedy','cheey','lchedy'])
            local.append({'model':mid,'edition':e,'topic_id':lookup['teeolain'],'condition_marker_id':lookup['chey'],'warm_id':lookup.get('qoteedy',''),'copula_id':lookup['qokain'],'consequent_id':lookup['cheedy'],'anaphor_id':lookup['cheey'],'anaphor_referent_id':lookup['teeolain'],'adjunct_id':lookup.get('lchedy',''),'consequent_role':'MOTION' if mid=='R' else 'REST_STATE','conditional_claim':('IF_WARM_THEN_MOTION_TOWARD_UNBOUND_GOAL' if mid=='R' else 'IF_WARM_THEN_REST_AT_UNBOUND_LATER_TIME') if complete else 'CONDITION_PROPERTY_AND_ADJUNCT_UNRESOLVED','status':'LEXICALLY_COMPLETE_HYPOTHESIS' if complete else 'RAW_READING_UNRESOLVED','factual_warmth_inferred':False,'factual_consequent_inferred':False,'semantics_confirmed':False})
