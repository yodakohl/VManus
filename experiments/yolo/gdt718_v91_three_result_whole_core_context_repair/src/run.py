#!/usr/bin/env python3
"""Build V91 by separating three learned workflow-whole cores from local heads."""
...
EXP = ROOT / "experiments/yolo/gdt718_v91_three_result_whole_core_context_repair"
SRC = EXP / "src"
ART = EXP / "artifacts"
G717 = ROOT / "experiments/yolo/gdt717_v90_quality_stage_core_context_repair/artifacts"

SOURCE_LEXICAL = G717 / "V90_324_ACTIVE_LEXICAL_READINGS.tsv"
SOURCE_CONTEXT = G717 / "V90_479_CONTEXT_REALIZATIONS.tsv"
SOURCE_COMPLETE = G717 / "V90_COMPLETE_WORD_CONFIDENCE.tsv"
SOURCE_CENSUS = G717 / "V90_71_HELD_READING_AUDIT.tsv"
SOURCE_SPANS = G717 / "V90_2_BOUND_SPAN_RENDERER.tsv"
SPECS = SRC / "V91_3_AUDIT_SPECS.tsv"
BINDINGS = SRC / "V91_35_PRIMARY_EVIDENCE_BINDINGS.tsv"

HISTORICAL = "H0_NONE"
STATUS = (
    "PASS_V91_3_WORKFLOW_WHOLES_REVISED__SHARED_WITHDRAWAL_FAMILY_REJECTED__"
    "3_POSITIONS_3_PAGES__61_WEAK_READINGS_REMAIN__NO_SCORE_CREDIT__ALL_H0_NONE"
)
...
def v91_key(value: str) -> str:
    return value.replace("v90", "v91").replace("V90", "V91")


def rename_v90_fields(row: dict[str, str]) -> dict[str, str]:
    return {v91_key(key): value for key, value in row.items()}
...
def resolve_evidence(
    ...
    for spec in specs:
        ...
        assert families == []
        assert split_pipe(spec["score_credit_family_ids"]) == []
        deltas[source_id] = 0
...
def build_lexical(
    ...
    for source in source_rows:
        ...
        if spec:
            ...
            row.update({
                "v91_lexical_core_de": spec["v91_lexical_core_de"],
                "v91_context_realizations_de": spec["v91_context_realization_de"],
                ...
                "source_gdts": append_pipe(source["source_gdts"], "GDT718"),
                "positive_evidence_de": (
                    "GDT718 trennt den portablen Zustandskern vom lokalen Workflow-Produktkopf: "
                    + spec["evidence_de"] + " || " + source["positive_evidence_de"]
                ),
                "counterevidence_de": (
                    "GDT718 Gegenbeleg: " + spec["counterevidence_de"]
                    + " || Historisch unbestaetigte Arbeitstheorie; keine Klartextidentifikation."
                ),
                "v91_audit_decision": "REVISE",
                "v91_evidence_class": spec["evidence_class"],
                "v91_open_semantic_slots": spec["open_semantic_slots"],
                "v91_component_global_export_allowed": "0",
                "v91_prior_lexical_core_de": source["v90_lexical_core_de"],
            })
        else:
            row.update({
                "v91_audit_decision": "NOT_IN_GDT718_TRANCHE",
                "v91_evidence_class": "INHERITED_V90",
                "v91_open_semantic_slots": "NOT_EVALUATED",
                "v91_component_global_export_allowed": "NOT_EVALUATED",
                "v91_prior_lexical_core_de": source["v90_lexical_core_de"],
            })
        output.append(row)
...
def build_contexts(
    ...
    for source in source_rows:
        ...
        row.update({
            "v91_reading_id": lexical["v91_reading_id"],
            "v91_lexical_core_de": lexical["v91_lexical_core_de"],
            "v91_context_realization_de": spec["v91_context_realization_de"] if spec else source["v90_context_realization_de"],
            "v91_repair_mode": spec["repair_mode"] if spec else source["v90_repair_mode"],
            "v91_resolved_debt_atom": spec["resolved_debt_atom"] if spec else source["v90_resolved_debt_atom"],
            "v91_lexical_score": lexical["working_model_score_0_100_not_probability"],
            "v91_lexical_level": lexical["working_model_level"],
            "v91_context_score": lexical["context_realization_score_0_100_not_probability"],
            "v91_context_level": lexical["context_realization_level"],
            "v91_semantic_scope": lexical["semantic_scope"],
            "v91_semantic_applicability": lexical["semantic_applicability"],
            "v91_global_export_scope": lexical["global_export_scope"],
            "v91_lexical_bound_span_ids": lexical["bound_span_ids"],
            "v91_unconditional_global_export_allowed": lexical["unconditional_global_export_allowed"],
            "v91_historical_confirmation": HISTORICAL,
            "v91_audit_decision": "REVISE" if spec else "NOT_IN_GDT718_TRANCHE",
            "v91_evidence_class": spec["evidence_class"] if spec else "INHERITED_V90",
            "v91_open_semantic_slots": spec["open_semantic_slots"] if spec else "NOT_EVALUATED",
            "v91_component_global_export_allowed": "0" if spec else "NOT_EVALUATED",
            "v91_local_context_hypothesis": spec["local_context_hypothesis"] if spec else "NONE",
            "v91_expected_left_surface": spec["expected_left_surface"] if spec else "NONE",
        })
        output.append(row)
...
def build_census(
    ...
    for source in source_rows:
        ...
        if spec:
            row.update({
                "disposition": "REVISED_IN_V91",
                "repair_mode": spec["repair_mode"],
                "resolved_debt_atom": spec["resolved_debt_atom"],
                "v91_reading_id": lexical["v91_reading_id"],
                "v91_lexical_core_de": lexical["v91_lexical_core_de"],
                "v91_context_realization_de": spec["v91_context_realization_de"],
            ...
            row.update({
                "v91_reading_id": lexical["v91_reading_id"],
                "v91_lexical_core_de": lexical["v91_lexical_core_de"],
                "v91_context_realization_de": lexical["v91_context_realizations_de"],
                "new_lexical_score": lexical["working_model_score_0_100_not_probability"],
                "new_lexical_level": lexical["working_model_level"],
                "new_context_score": lexical["context_realization_score_0_100_not_probability"],
                "new_context_level": lexical["context_realization_level"],
                "v91_audit_decision": "HELD_FOR_LATER_REPAIR",
                "v91_evidence_class": "INHERITED_V90",
                "v91_open_semantic_slots": "NOT_EVALUATED",
            })
        output.append(row)
...
def build_delta(
    ...
    for spec in specs:
        ...
        output.append({
            ...
            "left_surface": spec["expected_left_surface"], "old_lexical_core_de": source["v90_lexical_core_de"],
            "v91_lexical_core_de": target["v91_lexical_core_de"],
            "v91_context_realization_de": spec["v91_context_realization_de"],
            "old_score": source["working_model_score_0_100_not_probability"],
            "v91_score": target["working_model_score_0_100_not_probability"],
            "old_level": source["working_model_level"], "v91_level": target["working_model_level"],
...
def build_rejected_family(specs: list[dict[str, str]]) -> list[dict[str, Any]]:
    return [{
        "candidate_family_id": "REJECTED_SHARED_WITHDRAWAL_WORKFLOW",
        "selected_source_reading_ids": ordered_compact(row["source_reading_id"] for row in specs),
        "selected_surfaces": ordered_compact(row["source_reading_id"].split("#", 1)[0] for row in specs),
        "candidate_value_de": "Auszug oder abgezogenes Workflowprodukt",
        "decision": "REJECT_NO_COMMON_LICENSED_WRITTEN_UNIT",
        "reason_de": "GDT691/GDT692 stuetzen eine lokale Ganzwortinsel, aber keine gemeinsame schriftliche Dekomposition; kchod teilt eed nicht und d/e/eed bleiben unexportiert.",
...
def build_complete(source_rows: list[dict[str, str]], lexical: list[dict[str, Any]]) -> list[dict[str, Any]]:
    output: list[dict[str, Any]] = []
    for source in source_rows:
        if source["current_layer"] == "ACTIVE_V90_LEXICAL_CORE":
            continue
        row: dict[str, Any] = rename_v90_fields(source)
        row.update({
            "v91_audit_decision": "OUTSIDE_ACTIVE_V91_TRANCHE",
            "v91_evidence_class": "INHERITED_GLOBAL_V48",
            "v91_open_semantic_slots": "NOT_EVALUATED",
            "v91_component_global_export_allowed": "NOT_EVALUATED",
        })
        output.append(row)
    for row in lexical:
        output.append({
            "surface": row["surface"], "reading_id": row["v91_reading_id"],
            "working_meaning_de": row["v91_lexical_core_de"], "current_layer": "ACTIVE_V91_LEXICAL_CORE",
            ...
            "v91_context_realizations_de": row["v91_context_realizations_de"],
            "source_reading_ids": row["source_reading_ids"], "v91_audit_decision": row["v91_audit_decision"],
            "v91_evidence_class": row["v91_evidence_class"], "v91_open_semantic_slots": row["v91_open_semantic_slots"],
            "v91_component_global_export_allowed": row["v91_component_global_export_allowed"],
        })
...
def render_f7r2(contexts: list[dict[str, Any]], directives: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for source in source_rows:
        ...
        if directive:
            ...
            rendered = source["v91_context_realization_de"]
...
def report_text(result: dict[str, Any], delta: list[dict[str, Any]]) -> str:
    table = "\n".join(
        f"| `{row['surface']}` | {row['v91_lexical_core_de']} | {row['v91_context_realization_de']} | {row['old_score']}→{row['v91_score']} |"
        for row in delta
    )
    return f"""# GDT718 — V91 learned workflow-whole core/context repair

Status: `{STATUS}`

## Ergebnis

Drei konkrete Auszug-Renderer werden auf ihre engeren portablen Zustandskerne
zurueckgefuehrt. Der konkrete GDT691-Produktkopf bleibt nur in der jeweiligen
Fundstellenausgabe. Eine gemeinsame Auszug-/Abziehfamilie wird ausdruecklich
verworfen: Die Karten sind gelernte Ganzwoerter ohne gemeinsamen lizenzierten
Schriftstamm.
...
`oteed` und `keeod` behalten den belegten abgekuehlten beziehungsweise erhitzten
Zustand samt lokalem Abgezogen-Resultat. `kchod` behaelt nur heiß-trocken und
abgeschlossen; Auszug, Trockengut und Abgezogen-Identitaet bleiben dort lokal.
Kein Ziel erhaelt einen Familien- oder Fluessigkeitsbonus; alle drei bleiben 30/W1.

## Bestand

- 3 Lesungen / 3 Positionen / 3 Seiten
- {result['primary_evidence_bindings']} exakt aufgeloeste Primaerevidenzbindungen
- 321 nicht betroffene Kerne und 476 nicht betroffene Kontexte unveraendert
- beide Spans erhalten; f7r.2 erneut als acht Einheiten ausgegeben
- V91 aktiv: `{json.dumps(result['active_level_counts'], ensure_ascii=False, sort_keys=True)}`
...
Auszug bleibt eine ersetzbare lokale Rendererwahl. Es wird kein freies d, e,
eed, O, K, T oder CH und keine Entnahmemorpheme exportiert. Alle historischen
Felder bleiben `H0_NONE`; keine neue Seite oder Transkription wurde benutzt.
"""
...
def main() -> int:
    ...
    assert (
        ...
        len(source_spans), len(specs), len(bindings)
    ) == (324, 479, 1586, 71, 2, 3, 35)
    ...
    evidence, deltas = resolve_evidence(bindings, specs)
    ...
    rejected_family = build_rejected_family(specs)
    ...
    assert len(census) == 64 and Counter(row["disposition"] for row in census) == Counter({"HELD_FOR_LATER_REPAIR": 61, "REVISED_IN_V91": 3})
    assert len(rejected_family) == 1 and rejected_family[0]["score_delta"] == 0
    ...
    lexical_fields = unique_fields([v91_key(field) for field in source_lexical[0]] + ["v91_audit_decision", "v91_evidence_class", "v91_open_semantic_slots", "v91_component_global_export_allowed", "v91_prior_lexical_core_de"])
    context_fields = unique_fields([v91_key(field) for field in source_context[0]] + ["v91_audit_decision", "v91_evidence_class", "v91_open_semantic_slots", "v91_component_global_export_allowed", "v91_local_context_hypothesis", "v91_expected_left_surface"])
    census_fields = unique_fields([v91_key(field) for field in source_census[0]] + ["v91_audit_decision", "v91_evidence_class", "v91_open_semantic_slots"])
    complete_fields = unique_fields([v91_key(field) for field in source_complete[0]] + ["v91_audit_decision", "v91_evidence_class", "v91_open_semantic_slots", "v91_component_global_export_allowed"])

    write_tsv(ART / "V91_324_ACTIVE_LEXICAL_READINGS.tsv", lexical_fields, lexical)
    write_tsv(ART / "V91_479_CONTEXT_REALIZATIONS.tsv", context_fields, contexts)
    write_tsv(ART / "V91_64_HELD_READING_AUDIT.tsv", census_fields, census)
    write_tsv(ART / "V91_3_WORKFLOW_WHOLE_CORE_CONTEXT_DELTA.tsv", list(delta[0]), delta)
    write_tsv(ART / "V91_35_PRIMARY_EVIDENCE_BINDINGS.tsv", list(evidence[0]), evidence)
    write_tsv(ART / "V91_1_REJECTED_SHARED_WORKFLOW_FAMILY.tsv", list(rejected_family[0]), rejected_family)
    write_tsv(ART / "V91_2_BOUND_SPAN_RENDERER.tsv", list(spans[0]), spans)
    write_tsv(ART / "V91_2_ONE_SHOT_RENDER_DIRECTIVES.tsv", list(directives[0]), directives)
    write_tsv(ART / "V91_8_F7R2_RENDERED_UNITS.tsv", list(rendered[0]), rendered)
    write_tsv(ART / "V91_COMPLETE_WORD_CONFIDENCE.tsv", complete_fields, complete)

    result = {
        "experiment_id": "GDT718", "status": STATUS,
        "claim_ceiling": "EXPLORATORY_WORKING_DICTIONARY_ONLY_NOT_PLAINTEXT",
        "audited_readings": 3, "audited_positions": 3, "audited_pages": 3,
        "learned_workflow_wholes": 3, "shared_withdrawal_family_accepted": 0,
        ...
        "remaining_unreviewed_weak_readings": sum(row["v91_audit_decision"] == "HELD_FOR_LATER_REPAIR" for row in census),
        ...
        "relation_word_credit_gdt718": 0, "historical_confirmation": HISTORICAL,
