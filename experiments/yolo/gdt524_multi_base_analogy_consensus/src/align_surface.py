#!/usr/bin/env python3
"""Compile a surface through GDT524's two-base analogy consensus."""
...
def main() -> int:
    ...
    g522_missing, g522_weight, path_mode, path_weight = (
        core.selected_upstream_parameters()
    )
    ...
    for base_index, candidate in enumerate(candidates):
        ...
        consensus, consensus_trace, bases = core.consensus_features(
            surface, candidate.recipe, analogy, g522_missing
        )
        feature = consensus[consensus_mode]
        scored.append(
            {
                "compiler_rank": base_index + 1,
                "recipe": core.recipe_text(candidate.recipe),
                "gdt523_score": gdt523_score,
                "consensus_feature": feature,
                "gdt524_score": gdt523_score - consensus_weight * feature,
    ...
    scored.sort(key=lambda row: (float(row["gdt524_score"]), int(row["compiler_rank"])))
    ...
    result = {
        ...
        "selected_consensus_stage": core.SELECTED_STAGE,
        ...
        "selection_precedence": "EXACT_EVENT>KNOWN_SURFACE_ROLE_OPTION>GDT524_TWO_BASE_CONSENSUS_TOP1",
        "default_selection": default,
        "guard": "TWO_DISTINCT_BASE_AND_EDIT_CHANNEL_CONSENSUS__NO_CONFIRMED_LEXEME_OR_PLAINTEXT",
    }
