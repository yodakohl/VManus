...
def replay(atoms,words,types,seconds=10,max_nodes=1000000,max_solutions=2,pinned=None):
    ...
    for a,k in freq.items():
        ...
        ds=[p for p in pieces if len(p)<=mx and sum(w.count(p) for w in words)>=k]
    ...
    def visit(i,end):
        ...
        if a in code:
            ...
            return begin>=starts[end] and text[begin:end]==v and visit(i-1,begin)
        for v in domains[a]:
            begin=end-len(v)
