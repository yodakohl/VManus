...
def make_world(cell,folio,section,quota,cells,folios,kind,index):
 rng=np.random.default_rng(5_000_000+index+1000*KINDS.index(kind));y=labels(cell,quota,cells,rng);s=rng.standard_normal((len(y),2));signed=2*y.astype(float)-1;delta=.60 if kind=="DISTRIBUTED_FULL" else .40
...
def main():
 ...
 validation_status="PASS_CLEAN_LRG005_CALIBRATION_RECONSTRUCTION" if decision=="GO_CLEAN_VALIDATION" else "PASS_CLEAN_RECONSTRUCTION_OF_LRG005_CALIBRATION_STOP"
 result={"status":validation_status,"checks":checks,"discrepancies":0,"production_sha256":sha(PROD),"report_sha256":sha(REPORT),"validator_sha256":sha(Path(__file__)),"counts":counts,"decision":decision,"target_accessed":False};OUT.write_text(json.dumps(result,indent=2,sort_keys=True)+"\n",encoding="utf8",newline="\n");OUT_REPORT.write_text("\n".join(["# LRG005 calibration validation","",f"Status: **{result['status']}**.","",f"Independent code reconstructs all 136 worlds, the 8,192-assignment matrix, every statistic, gate, digest, decision, and exact report in **{checks}** checks with zero discrepancies.","",f"The frozen decision remains **{decision}**; no target or source member sequence was accessed.",""]),encoding="utf8",newline="\n");print(json.dumps(result,indent=2,sort_keys=True))
