...
A=8192;SEED=510052026;KINDS=("NULL","DISTRIBUTED_FULL","DISTRIBUTED_HALF","ONE_FOLIO","ONE_SECTION","ONE_PARITY","FOLIO_RANDOM","ONE_CHANNEL","OPPOSITE_CHANNEL","CELL_CONSTANT");CHANNELS=("D1_BARE","D1_OTHER");checks=0
def need(x,m):
...
def make_world(cell,folio,section,quota,cells,folios,kind,index):
 rng=np.random.default_rng(5_000_000+index+1000*KINDS.index(kind));y=labels(cell,quota,cells,rng);s=rng.standard_normal((len(y),2));signed=2*y.astype(float)-1;delta=.42 if kind=="DISTRIBUTED_FULL" else .22
 if kind in {"DISTRIBUTED_FULL","DISTRIBUTED_HALF"}:s+=delta*signed[:,None]
 elif kind=="ONE_FOLIO":
...
def eval_world(s,y,cell,folio,section,folios,coef,null):
 ...
 for j,name in enumerate(CHANNELS):
  vals=fe[:,j];t=float(obs[j]);mu=float(null[:,j].mean());sd=float(null[:,j].std(ddof=0));p=(1+int(np.count_nonzero(null[:,j]>=t)))/(A+1);z=(t-mu)/sd if sd>0 else float('-inf');sec={x:float(vals[sf==x].mean()) for x in ('B','P')};par={'ODD':float(vals[nums%2==1].mean()),'EVEN':float(vals[nums%2==0].mean())};d=[float(np.delete(vals,k).mean()) for k in range(len(vals))];den=float(np.abs(vals).sum());con=float(np.max(np.abs(vals))/den) if den else 1.;g={"p_at_most_001":p<=.01,"z_at_least_3":z>=3,"effect_at_least_010":t>=.10,"support_at_least_10":int(np.count_nonzero(vals>0))>=10,"both_sections_at_least_005":min(sec.values())>=.05,"both_parities_at_least_005":min(par.values())>=.05,"all_deletions_at_least_005":min(d)>=.05,"concentration_at_most_025":con<=.25};metrics.append({"channel":name,"effect":t,"null_mean":mu,"null_sd":sd,"z":z,"p":p,"positive_folios":int(np.count_nonzero(vals>0)),"folio_effects":{f:float(v) for f,v in zip(folios,vals,strict=True)},"section_effects":sec,"parity_effects":par,"minimum_deletion":min(d),"maximum_absolute_folio_concentration":con,"null_sha256":ah(null[:,j]),"gates":g,"passes":all(g.values())})
