            ...
            output.append(
                {
                    ...
                    "occupied": "YES" if row["normalized_code"] else "NO",
                    "physical_folio": physical_folio(page),
                    "page": page,
                    "unit": unit,
                    "locus": row["locus"],
                    "source_locus": row["source_locus"],
                    "normalized_code": row["normalized_code"] or "MISSING",
...
def build() -> tuple[list[dict[str, str]], dict[str, object], str]:
    ...
    missing = [row for row in rows if row["occupied"] == "NO"]
    if (len(array_ids), len(rows), len(rows) - len(missing), len(missing)) != (45, 504, 502, 2):
    ...
    result: dict[str, object] = {
        ...
        "schema": "SPECIAL_CIRCLE_TEXT_BLIND_ARRAY_INVENTORY_V1",
        "status": "PASS_NEW_VERSIONED_TEXT_BLIND_SPECIAL_CIRCLE_INVENTORY",
        "decision": "AUTHORIZE_SEPARATE_SCORE_BLIND_DESIGN_ONLY",
        "counts": {
            "arrays": len(array_ids),
            "slots": len(rows),
            "occupied_slots": len(rows) - len(missing),
            "source_explicit_missing_slots": len(missing),
        ...
        "missing_slots": [
            {"array_id": row["array_id"], "page": row["page"], "unit": row["unit"], "locus": row["locus"], "slot_index": int(row["slot_index"])}
            for row in missing
        ],
        "inputs": {
        ...
        "claim_ceiling": (
            "This is a new filler-blind source inventory of 45 human-defined special-circle arrays and 504 slots. It does "
