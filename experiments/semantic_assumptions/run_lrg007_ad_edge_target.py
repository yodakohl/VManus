...
HERE=Path(__file__).resolve().parent;ROOT=HERE.parents[1];R=HERE/'results';FREEZE=HERE/'LRG007_TARGET_FREEZE_V2.json';GROUPS=R/'source_sta_family_consensus_groups.tsv';PANEL=R/'lrg007_ad_edge_capacity.tsv';MARGINS=R/'lrg007_ad_edge_margins.tsv';CAP=R/'lrg007_ad_edge_capacity.json';CAL=R/'lrg007_target_blind_calibration.json';CALV=R/'lrg007_target_blind_calibration_validation.json';OUT=R/'lrg007_ad_edge_target.json';REPORT=R/'lrg007_ad_edge_target_report.md';VAL=R/'lrg007_ad_edge_target_validation.json';VALR=R/'lrg007_ad_edge_target_validation_report.md'
...
def main():
 ...
 features=np.asarray([(int(lookup[p['unit_id']]['family_surface'][0]=='A'),int(lookup[p['unit_id']]['family_surface'][0]=='D')) for p in panel],dtype=np.int8);capacity=json.loads(CAP.read_text())
 if ah(features)!=capacity['feature_matrix_sha256'] or tuple(features.sum(0))!=(1118,1235) or np.any(features.sum(1)>1):raise RuntimeError('feature drift')
