...
def evaluate(scores:np.ndarray,y:np.ndarray,g:Geometry,coef:np.ndarray,null_values:np.ndarray|None=None)->dict[str,object]:
    ...
    for j,name in enumerate(CHANNELS):
        ...
        sections={s:float(vals[section_by_folio==s].mean()) for s in ("B","P")};parity={"ODD":float(vals[nums%2==1].mean()),"EVEN":float(vals[nums%2==0].mean())};section_balance=min(sections.values())/max(sections.values()) if max(sections.values())>0 else float("-inf");parity_balance=min(parity.values())/max(parity.values()) if max(parity.values())>0 else float("-inf");deletions=[float(np.delete(vals,k).mean()) for k in range(len(vals))];den=float(np.abs(vals).sum());concentration=float(np.max(np.abs(vals))/den) if den else 1.0
        gates={"p_at_most_001":p<=.01,"z_at_least_3":z>=3,"effect_at_least_010":t>=.10,"support_at_least_10":int(np.count_nonzero(vals>0))>=10,"both_sections_at_least_005":min(sections.values())>=.05,"section_balance_at_least_035":section_balance>=.35,"both_parities_at_least_005":min(parity.values())>=.05,"parity_balance_at_least_035":parity_balance>=.35,"all_deletions_at_least_005":min(deletions)>=.05,"concentration_at_most_030":concentration<=.30}
        metrics.append({"channel":name,"effect":t,"null_mean":mu,"null_sd":sd,"z":z,"p":p,"positive_folios":int(np.count_nonzero(vals>0)),"folio_effects":{f:float(v) for f,v in zip(g.folio_names,vals,strict=True)},"section_effects":sections,"section_balance_ratio":section_balance,"parity_effects":parity,"parity_balance_ratio":parity_balance,"minimum_deletion":min(deletions),"maximum_absolute_folio_concentration":concentration,"null_sha256":array_hash(null[:,j]),"gates":gates,"passes":all(gates.values())})
