...
KINDS=("NULL","DISTRIBUTED_FULL","DISTRIBUTED_HALF","ONE_FOLIO","ONE_SECTION","ONE_PARITY","FOLIO_RANDOM","ONE_CHANNEL","OPPOSITE_CHANNEL","CELL_CONSTANT")
...
def world(g,kind,index):
    ...
    delta=.42 if kind=="DISTRIBUTED_FULL" else .22
    if kind in {"DISTRIBUTED_FULL","DISTRIBUTED_HALF"}:scores+=delta*signed[:,None]
...
def main():
    ...
    gates={"zero_of_64_null":counts["NULL"]==0,"all_full_plants":counts["DISTRIBUTED_FULL"]==8,"all_half_plants":counts["DISTRIBUTED_HALF"]==8,"zero_all_adversaries":all(counts[k]==0 for k in KINDS[3:])}
