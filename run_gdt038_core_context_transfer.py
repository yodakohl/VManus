...
def contexts(rows):
 ...
 for locus,line in lines.items():
  ...
  physical_count=int(line[0]["group_count"]);indices={int(r["group_index"])for r in line};line_complete=len(line)==physical_count and indices==set(range(1,physical_count+1))
  fields=[];current=[]
...
def main():
 ...
 coverage={core:{"complete_line_occurrences":sum(r["retained_line_complete"]=="1"for r in occ if r["core"]==core),"physical_line_end_occurrences":sum(r["physical_line_end"]=="1"for r in occ if r["core"]==core)}for core in CORES}
 result={"schema":"GDT038_CORE_CONTEXT_TRANSFER_RESULT_V1","status":"DAM_FIELD_ROLE_PROVISIONAL_LOW_CAPACITY_DAIIN_STATE_ONLY_OKAM_ODAIN_SECTION_CONDITIONED","scope":"Full available formal local contexts for exact residual cores DAIIN, DAM, OKAM, ODAIN in Herbal-B versus Currier-B Stars/Recipe S.","cores":summaries,"coverage":coverage,"features":list(FEATURES),"occurrences":len(occ),"controls":{"folio_unit":"Exact section-label permutation across target-positive physical folios; all occurrences on a folio move together.","maxT":"Per-core maximum standardized JS divergence across eleven declared context families.","lofo":"Worst overlap and divergence after deleting each target-positive physical folio.","hand":"Hand-3-only overlap reported separately; capacity remains only two Herbal-B hand-3 folios."},"important_dependency":"GDT016 CARRIER_STATE is induced from the ch/che/sh renderer family. DAIIN/DAM state invariance is therefore not independent evidence beyond wrapper stability; field position and neighbors are tested separately. GDT016 retains only strict consensus prose groups, so field templates are inventory-relative on incomplete lines.","claim_ceiling":"Tests preservation of anonymous state/field roles only. No concrete function, word, morpheme, POS, referent, sound, language, plaintext, meaning, or translation.","f84r":{"opened":False,"retained":False,"queried":False,"joined":False,"scored":False},"inputs":{"gdt016_group_state_inventory.tsv":sha(SOURCE),"gdt016_result.json":sha(ROOT/"gdt016_result.json"),"gdt037_result.json":sha(ROOT/"gdt037_result.json")},"implementation":{Path(__file__).name:sha(Path(__file__))},"outputs":{OCC.name:sha(OCC),CLUSTERS.name:sha(CLUSTERS),COMPARE.name:sha(COMPARE)}}
