...
d=json.loads((R/'gdt301_design.json').read_text());q=dict(d);q.pop('content_sha256');ck('content',d['content_sha256']==can(q));ck('status',d['status']=='CORRECTED_AFTER_FIRST_SCORE_BEFORE_PUBLICATION');ck('axes',d['axes']==['physical_folio','register','section','currier','hand']);ck('folio_override',d['folio_layout_override']=='EXACT_GDT299_SECTION_CURRIER_HAND_GROUP_COUNT');ck('folio_population',d['folio_population_override']=='EXACT_GDT299_GLOBAL_CROSSFOLIO_ELIGIBLE_POPULATION');ck('flags',d['source_strings_inspected']==d['page_host_substrings_mined']==d['semantic_assignments']==0 and not any(d['f84'].values()));ck('method',sha(R/'GDT301_WHOLE_FORM_DOMAIN_TRANSFER_METHOD.md')==d['method_sha256']);ck('capacity',sha(R/'gdt301_capacity.tsv')==d['capacity_sha256']);ck('manifest',sha(R/'gdt301_freeze_manifest.tsv')==d['freeze_manifest_sha256']);
