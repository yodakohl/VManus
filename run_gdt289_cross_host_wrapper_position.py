...
def main():
 ...
 for p in d['panels']:
  q=rr[p][1];variable=p in d['maxT_panels'];local=(1+sum(x>=obs[p]-1e-15 for x in nulls[p]))/65 if variable else None;maxp=(1+sum(x>=zs[p]-1e-15 for x in worldmax))/65 if variable else None;summary.append({'control_id':p,'scored_events':q['n'],'folios':len({x['physical_folio'] for x in panels[p]}),'transfer_gain_bits_per_event':f'{obs[p]:.12f}','null_mean':f'{means[p]:.12f}','null_sd':f'{sds[p]:.12f}','observed_z':f'{zs[p]:.12f}' if variable else 'NA_ZERO_NULL_VARIANCE','local_p':f'{local:.12f}' if variable else 'NA_ZERO_NULL_VARIANCE','max4_p':f'{maxp:.12f}' if variable else 'NA_ZERO_NULL_VARIANCE','null_mobile_events_world0':mobile[p],'positive_host_buckets':sum(z[1]>0 for k,z in q['detail'].items() if k[0]=='BUCKET'),'positive_positions':sum(z[1]>0 for k,z in q['detail'].items() if k[0]=='POSITION')})
 ...
 report=['# GDT289 — cross-host wrapper-position transfer','',f'Status: **{status}**.','','## Result','', '| panel | scored | transfer gain (bits/event) | positive buckets | positive positions | local p | corrected max4 p |','|---|---:|---:|---:|---:|---:|---:|']
 for x in summary:
  mp=x['max4_p'] if x['max4_p'].startswith('NA') else f"{float(x['max4_p']):.4f}"
  lp=x['local_p'] if x['local_p'].startswith('NA') else f"{float(x['local_p']):.4f}"
  report.append(f"| {x['control_id']} | {x['scored_events']} | {float(x['transfer_gain_bits_per_event']):+.4f} | {x['positive_host_buckets']}/8 | {x['positive_positions']}/4 | {lp} | {mp} |")
