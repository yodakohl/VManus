...
def main():
 ...
 ck('models',len(d['models'])==5);ck('classes',d['wrapper_classes']==['NONE','q','ch','d','sh','che','t','s']);ck('regimes',len(d['transfer_regimes'])==3);ck('sections',len(d['powered_sections'])==6);ck('hands',len(d['powered_hands'])==4);ck('null',d['null_worlds']==64);ck('no_semantics',d['semantic_assignments']==d['hpr1_semantics_used']==d['page_host_substrings_mined']==0);ck('f84',all(v in (0,False) for v in d['f84'].values()))
