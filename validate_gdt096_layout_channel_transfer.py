...
def main():
 ...
 checks={"capacity":r["train_unhedged_loci"]==83 and r["test_hedged_loci"]==35 and r["positive_test_loci"]==4,"orbit":r["exact_worlds"]==1872,"grid":len(s)==10 and len(p)==35,"frozen_miss":f["representation"]=="HOST_WRAPPER_JOINT" and f["gain_bits"]<0 and f["positive_gain_folios"]==0,"roles":all(x["semantic_role"]=="UNASSIGNED" for x in p),"f84":not any(r["f84r"].values()),"content_hash":csha(b)==ch,"hashes":all(sha(R/n)==v for fam in ("inputs","outputs","documents","implementation") for n,v in r[fam].items())};z=[x for x in read(R/"GDT002_YOLO_LEDGER.tsv") if x["checkpoint_id"]=="GDT096_CKPT001"];checks["ledger"]=len(z)==1 and z[0]["status"]==r["status"]
