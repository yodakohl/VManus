...
## GDT606 historical-capacity mixed nomenclator and carrier correction
...
Retain a structural category lead, not a translation: `o`,`y`,`ol` are whole-word/formula-carrier category in every one of 12 real starts per language across all capacity grids, and `C`,`d` in at least 11/12. The MHG-only `gesch`/`o` prefix family at 597 held loci is generated by alternating `geschach`/`geschehen` outputs and does not license either word. Next infer roles for these five units from manuscript-internal context and fit carrier agreement inside the objective. Do not transfer GDT606 reference words or the noncanonical workshop values.
