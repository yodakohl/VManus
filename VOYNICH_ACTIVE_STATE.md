...
## GDT606–GDT607 mixed nomenclator and output-bucket correction
...
A 90-key explicit-boundary grid then exchanges0/3/6/8/11 `W` slots for outputless `B` slots. Across72 eligible runs `o`,`y`,`ol` enter `B` zero times and `C`,`d` only four times each; at `W=0` all five predominantly become output-bearing multi-character macros. Retain only these manuscript-wide formal defaults: `C` strict local opener; `d` chunk/physical-line head carrier; `y` chunk/line and weak paragraph closure carrier; `ol` boundary/occasional standalone carrier; `o` flexible bidirectional connector. Next decompose frozen BPE compounds through shared base components (`ol/or/ok/ot/dy/aN`) and fit opener/closer/internal/whole/null roles with output-length/codebook cost and synthetic carrier recovery. Do not transfer any generated reference word, sound or meaning.
