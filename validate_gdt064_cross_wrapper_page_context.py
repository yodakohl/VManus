...
def main():
 ...
 checks["pair_and_cell_counts"]=len(pairs)==r["cross_folio_pairs"]==45942 and len(cells)==r["host_register_cells"]==619
 counts=Counter((x["host"],x["register"],x["pair_type"])for x in pairs);checks["pair_cap_and_controls"]=r["pair_cap_per_host_register_type"]==200 and max(counts.values())<=200 and all(int(x["control_units"])>0 for x in pairs)
 ...
 checks["positive_cells"]=sum(float(x["different_minus_control"])>0 for x in cells)==r["positive_cells"]==373 and r["same_wrapper_cells"]==len(both)==460
 checks["small_invariance_gain"]=close(r["different_minus_control"],.004255744919660076)and close(r["relative_gain_vs_control"],.020439679317278515)and close(r["different_minus_same"],-.0009847689640196389)
 ...
 checks["variant_log"]={x["variant_id"]:x["status"]for x in variants}=={"V00":"PRIMARY","V01":"RUN_CONTROL","V02":"RUN_SENSITIVITY","V03":"POSTSELECTED_DISPLAY","V04":"NOT_RUN","V05":"CORRECTION"}
 ...
 checks["status_and_ceiling"]=r["schema"]=="GDT064_CROSS_WRAPPER_PAGE_CONTEXT_RESULT_V2"and"140 unsupported"in r["correction"]and r["status"]=="CROSS_WRAPPER_PAGE_CONTEXT_PRESERVATION_SUPPORTED"and"external semantic preservation remains unconfirmed"in r["interpretation"]and"No role"in r["claim_ceiling"]
