...
def main():
 ...
 for panel in d['panels']:
  ...
  local='' if panel not in profile_active else f"{(1+sum(x>=sum(v.values())-1e-15 for x in nullgain[panel]))/65:.12f}"
  maxp='' if panel not in profile_active else f"{(1+sum(x>=obsz[panel]-1e-15 for x in worldmax))/65:.12f}"
