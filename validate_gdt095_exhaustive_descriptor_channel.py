...
def main():
 ...
 checks={"capacity":r["loci"]==83 and r["physical_folios"]==5 and len(manifest)==19,"representation_grid":len(scores)==10 and set(by)==set(r["representations"]),"best":scores[0]["representation"]=="HOST_WRAPPER_JOINT" and abs(float(scores[0]["gain_bits"])-11.252103541840825)<1e-9,"marginals":float(by["PAGE_HOST_CHAR3"]["gain_bits"])<0 and float(by["WRAPPER_ONLY"]["gain_bits"])<0,"channel_ablation":r["posthoc_channel_ablation"]["layout_position"]["gain_bits"]>15 and r["posthoc_channel_ablation"]["appearance_remainder"]["gain_bits"]<0,"max_null":float(scores[0]["max_representation_p"])<.01,"classes":len(classes)>0 and all(x["semantic_role"]=="UNASSIGNED" for x in classes),"f84":not any(r["f84r"].values()),"content_hash":csha(body)==claimed,"hashes":all(sha(ROOT/n)==v for fam in ("inputs","outputs","documents","implementation") for n,v in r[fam].items())}
