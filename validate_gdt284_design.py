...
def main():
 ...
 ck('status',d['status']=='FROZEN_BEFORE_GDT284_SCORING');ck('content',d['content_sha256']==csha(d));ck('manifest_hash',d['freeze_manifest_sha256']==sha(M));ck('manifest_files',len(m)==11 and all(sha(R/x['artifact'])==x['frozen_sha256'] for x in m));ck('panels',len(d['panels'])==12 and len(set(d['panels']))==12);ck('events',all(sum(x['control_id']==p for x in native)==8448 for p in d['panels']));ck('no_f84',not any(x['page'].startswith('f84') or x['locus'].startswith('f84') for x in native));ck('components',d['components']==['INITIAL','INTERNAL','FINAL','EOS']);ck('modes',d['modes']==['STANDARD_HELD_FOLIO','NESTED_UNSEEN_HOST_BUCKET']);ck('null',d['null_worlds']==64 and d['host_bucket_count']==8);ck('capacity_rule',d['capacity_rule']['known_zero_capacity_panels']==['LEARNED_ABBREVIATION_MAP','LEARNED_ABBREVIATION_SAMPLED'] and d['capacity_rule']['exclude_from_sign_match'] and d['capacity_rule']['exclude_from_standardized_maxT']);ck('no_tuning',d['semantic_assignments']==d['page_host_substrings_mined']==d['new_synthetic_worlds']==d['oracle_fields_scored']==0 and not d['threshold_tuned']);ck('f84',d['f84']['input_files']==0 and not any(v for k,v in d['f84'].items() if k!='input_files'));ck('method',d['method_sha256']==sha(R/'GDT284_WRAPPER_POSITIONAL_PROFILE_CALIBRATION_METHOD.md'));ck('implementation',all(sha(R/k)==v for k,v in d['implementation'].items()))
