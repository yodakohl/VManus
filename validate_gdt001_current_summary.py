...
def main():
    ...
    context=json.load(open(ROOT/'gdt001_context_axis_source_results.json'));ok(context['best']['decoder_hash']==valid[0]['decoder_hash'],'context_decoder');ok(abs(context['best']['total_bits']-float(valid[0]['total_bits']))<1e-5,'context_total');ok(context['best']['selected_contexts']==40,'context_count');ok(context['best']['axis_counts']=='{"CURRIER":21,"GRAMMAR_SCOPE":5,"HAND":4,"KIND":6,"SECTION":4}','context_axes')
