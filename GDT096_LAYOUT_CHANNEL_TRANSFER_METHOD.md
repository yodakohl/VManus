...
Freeze GDT095's exact `base|edge|ground|level` mixed spatial/context vocabulary and its ten
...
prevalence outside the target folio. The endpoint is not a pure label-placement
class: the same words may describe plant geometry or panel relations. Enumerate every target-label assignment
