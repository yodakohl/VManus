...
Forty-seven scoped question dossiers distinguish empirical failure, invalid test,
...
requirements. The current47 dossiers directly target60 semantic cards. The nine
priorities directly target27, with23 shared targets;3,790 semantic cards have no
direct target binding in these two lists. This is a coverage limit, not an
automatic scientific verdict on those cards (see decisions/hd_assessment_coverage.json; HA and HC remain earlier snapshots). An editorial wording preference is not an empirical falsifier.
