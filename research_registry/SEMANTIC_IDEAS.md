...
| Unit | Count |
|---|---:|
| Active semantic hypothesis variants | 3,878 |
| Separately typed formal-role cards | 266 |
| Reviewed source cases, including the correction archive | 4,673 |
...
| Public semantic display entries | 3,776 |
...
the ordinary local default therefore displays3,871 semantic entries. A fresh
...
127 scoped question dossiers distinguish empirical failure, invalid test,
...
priorities directly target27, with23 shared targets;3,722 semantic cards have no
direct target binding in these two lists. This is a coverage limit, not an
automatic scientific verdict on those cards (see decisions/ls_assessment_coverage.json; earlier snapshots remain preserved). An editorial wording preference is not an empirical falsifier.
