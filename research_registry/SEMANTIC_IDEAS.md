...
| Unit | Count |
|---|---:|
| Active semantic hypothesis variants | 3,866 |
| Separately typed formal-role cards | 266 |
| Reviewed source cases, including the correction archive | 4,661 |
...
| Public semantic display entries | 3,764 |
...
the ordinary local default therefore displays3,859 semantic entries. A fresh
...
priorities directly target27, with23 shared targets;3,710 semantic cards have no
direct target binding in these two lists. This is a coverage limit, not an
automatic scientific verdict on those cards (see decisions/lg_assessment_coverage.json; earlier snapshots remain preserved). An editorial wording preference is not an empirical falsifier.
