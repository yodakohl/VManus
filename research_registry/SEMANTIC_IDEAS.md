...
Fifty-two scoped question dossiers distinguish empirical failure, invalid test,
...
requirements. The current52 dossiers directly target66 semantic cards. The nine
priorities directly target27, with23 shared targets;3,784 semantic cards have no
direct target binding in these two lists. This is a coverage limit, not an
automatic scientific verdict on those cards (see decisions/he_assessment_coverage.json; earlier snapshots remain preserved). An editorial wording preference is not an empirical falsifier.
