...
for sm in ['Q','A']:
 for kind,keys in [('ARGUMENTS',['paragraph','grammar','operation']),('QUALITY',['paragraph','grammar','mention']),('EVENTS',['paragraph','grammar','model','timing','kind','location','assertion']),('TAKE_ARGUMENTS',['paragraph','grammar','target'])]:
  ...
  for key in sorted(a.keys()|b.keys()):
   ...
   fields=[k for k in old.keys()|new.keys() if k not in ['source_event','execution_index','row_status','polarity'] and old.get(k)!=new.get(k)]
...
result=dict(idea='IDEA000228',changes=counts,new_hard_conflicts=newconf,decision='Retain C editorially; N has missing negative patient and additional missing quality binding; no full negative reading selected',independent_confirmation_capacity=0,confirmed_meanings=0,held_access=False)
