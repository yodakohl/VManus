...
def table(n,rr,cols=None):
 ...
 with (E/(SHOL_MODEL+'_'+n)).open('w') as f:
...
exec(compile((E/'polarity.py').read_text(),'W31_polarity','exec'))
negative={r['target'] for r in rows(W/'W30/SCOPES.tsv') if r['shol']==SHOL_MODEL and r['contract']=='TRANSFER' and r['parity']=='1'}
replay=lambda base,shifts:replay_negative(base,shifts,negative)
...
reader=['# W31 vollständige hypothetische Negationslesung /'+SHOL_MODEL+'','W28 plus chey nicht, neue Bindungen durch unveränderte Grammatik; keine Bedeutungsbestätigung. Übernommen: shol Q=feucht / A=befeuchte, hier '+SHOL_MODEL+'. chol D=trockne/H=erhitze. Keine Übersetzungsbestätigung. GDT809s beschreibende Rivalen bleiben offen.']
for p in source:
 reader.append('\n## '+p['id'])
 for l in p['lines']:
  rr=[r for r in full if r['locus']==l['locus']];reader.append('\n'+l['locus']+'\n\n'+' · '.join(r['raw']+' ['+('D: trockne / H: erhitze' if r['raw']=='chol' else r['H'])+']' for r in rr))
(E/(SHOL_MODEL+'_READING.md')).write_text('\n'.join(reader)+'\n')
...
(E/(SHOL_MODEL+'_RESULT.json')).write_text(json.dumps(result,indent=2)+'\n');print(SHOL_MODEL,json.dumps(result))
