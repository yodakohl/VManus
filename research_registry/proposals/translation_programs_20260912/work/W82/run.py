...
D=Path(__file__).resolve().parent.relative_to(Path.cwd());spec=Path('experiments/yolo/gdt929_fixed_four_form_context_square/src/SPEC.json');s=json.loads(spec.read_text());allow=set(json.loads(Path(s['allow_source']).read_text())['allowed_selectors']);hits=[]
for path in s['sources']:
 ...
 for l in d['lines']:
  ...
  for i,w in enumerate(ws):
   if w in {'cthar','ochey'}:hits.append(dict(edition=m['edition'],page=m['page'],locus=m['locus'],at=l['groups'][i][si],word=w,left=ws[i-1] if i else '',right=ws[i+1] if i+1<len(ws) else '',words=ws))
source=Path('experiments/yolo/gdt928_multi_anchor_complete_paragraphs/artifacts/PARAGRAPHS.json');loci={h['locus'] for h in hits};ps={(p['edition'],p['id']):p for p in json.loads((D.parent/'W81/PARAGRAPHS.json').read_text())}
...
models={'N':{},'C':{'cthar':dict(kind='MATERIAL',meaning='Krautportion')},'O':{'ochey':dict(kind='MATERIAL',meaning='Zubereitung V')},'CO':{'cthar':dict(kind='MATERIAL',meaning='Krautportion'),'ochey':dict(kind='MATERIAL',meaning='Zubereitung V')}}
...
for name,rr in [('EVENTS.tsv',rows),('CHANGES.tsv',changes),('ALIGNMENT.tsv',al)]:
...
r=dict(hits=len(hits),counts=dict(collections.Counter(h['word']+'|'+h['edition'] for h in hits)),loci=len(loci),paragraphs=len(ps),groups=len(al)//4,models=summary,changed_events=len(changes),meaning_confirmed=False)
(D/'RESULT.json').write_text(json.dumps(r,indent=2)+'\n')
paths=[spec,Path(s['allow_source']),source,D.parent/'W81/PARAGRAPHS.json',D.parent/'P09/MODELS.json']+[Path(p) for p in s['sources']]
