...
for path in s['sources']:
 ...
 for l in d['lines']:
  ...
  for i,g in enumerate(l['groups']):
   if g[wc] in {'tshor','tchaly','chtols'}:hits.append(dict(edition=m['edition'],page=m['page'],locus=m['locus'],at=g[sc],word=g[wc],left=ws[i-1] if i else '',right=ws[i+1] if i+1<len(ws) else '',words=ws))
assert hits==json.loads((D/'OCCURRENCES.json').read_text())
loci={h['locus'] for h in hits};ps={(p['edition'],p['id']):p for p in json.loads((D.parent/'W82/PARAGRAPHS.json').read_text())}
...
assert base==json.loads((D/'LEXICON.json').read_text())
...
assert len(hits)==14 and len(ps)==116 and len(al)==58376 and len(events)==3696 and len(changes)==118
