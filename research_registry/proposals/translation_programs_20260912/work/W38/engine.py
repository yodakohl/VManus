...
D['chkaiin']={'role':'STATE','hypothesis':'heiß-trocken [Wertklasse offen]'}
D['cfhy']={'role':'LINK','hypothesis':'Untereintrag'}
...
glosses['chkaiin']='heiß-trocken [Hypothese; '+QUALITY_SCOPE+'; Wertklasse offen]'
glosses['cfhy']='Untereintrag [Strukturhypothese]'
...
# The independent-entry rival scopes all three existing binding functions.
frozen_solve,frozen_requal,frozen_take=solve,requal,take
def chunks(flat):
 out=[];part=[]
 for x in flat:
  if x['form']=='cfhy' and part:out.append(part);part=[]
  part.append(x)
 if part:out.append(part)
 return out
def local(flat,location):return next(c for c in chunks(flat) if any(x['id']==location for x in c))
def solve(flat):
 if SCOPE=='S':return frozen_solve(flat)
 out={g:[] for g in ['B','J','M']}
 for c in chunks(flat):
  for g,aa in frozen_solve(c).items():out[g].extend(aa)
 return out
def requal(q,flat,args):
 if SCOPE=='S':return frozen_requal(q,flat,args)
 c=local(flat,q['mention']);ids={x['id'] for x in c};return frozen_requal(q,c,[a for a in args if a['operation'] in ids])
def take(flat,x,args,g):
 if SCOPE=='S':return frozen_take(flat,x,args,g)
 c=local(flat,x['id']);ids={r['id'] for r in c};return frozen_take(c,x,[a for a in args if a['operation'] in ids],g)
...
for p in source:
 ...
 for g,aa in solve(flat).items():
  ...
  for x in flat:
   ...
   if x['role']!='STATE' or (x['form'] not in standalone and x['form']!='chkaiin'):continue
   fields=[dict(axis='thermal',value='hot',scope=QUALITY_SCOPE),dict(axis='moisture',value='dry',scope=QUALITY_SCOPE)] if x['form']=='chkaiin' else [standalone[x['form']]]
   for f in fields:
    q=requal(dict(mention=x['id'],kind='STANDALONE',form=x['form'],axis=f['axis'],value=f['value'],scope=f.get('scope','PHYSICAL')),flat,aa);qq.append(q);quality.append(dict(paragraph=p['id'],grammar=g,**q))
...
reader=['# W38 vollständige hypothetische Lesung '+RUN_ID+'','W37 S plus chkaiin quality '+QUALITY_SCOPE+'; cfhy-scope '+SCOPE+'; keine Bedeutungsbestätigung. Übernommen: shol Q=feucht / A=befeuchte, hier '+SHOL_MODEL+'. chol D=trockne/H=erhitze. Keine Übersetzungsbestätigung. GDT809s beschreibende Rivalen bleiben offen.']
for p in source:
 reader.append('\n## '+p['id'])
 for l in p['lines']:
