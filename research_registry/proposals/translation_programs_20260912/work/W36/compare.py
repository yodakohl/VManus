...
result=dict(idea='IDEA000233',changes=summaries,new_failures=bad,prediction_groups=groups,source_paragraphs=17,source_groups=1045,new_worlds=1224,new_events=sum(json.loads((E/(c+'_'+sm+'_RESULT.json')).read_text())['events'] for c in ['P','K','PK'] for sm in ['Q','A']),confirmed_meanings=0,independent_confirmation_capacity=0,held_access=False)
(E/'RESULT.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result))
for r in changes:
 if r['candidate']=='PK' and r['shol']=='A' and r['table'] in ['ARGUMENTS','QUALITY','TAKE_ARGUMENTS']:
