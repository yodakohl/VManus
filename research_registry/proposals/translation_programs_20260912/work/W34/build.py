"""W34 fixed C/O inputs and frozen W09 alias/W14 timing."""
...
source=read(B/'SOURCE.json')['paragraphs'];targets=[];ev=[];changes=[];later=[];summary=[];baseline_count=0;eligibility=[];takecases=[]
for lexicon,sm in [(l,s) for l in S['lexicons'] for s in S['shol_models']]:
 B=W/('W28' if lexicon=='C' else 'W33');prefix=sm if lexicon=='C' else 'O_'+sm
 flats=rows(B/(prefix+'_SOURCE_FLAT.tsv'));args=rows(B/(prefix+'_ARGUMENTS.tsv'));quals=rows(B/(prefix+'_QUALITY.tsv'));inputs=rows(B/(prefix+'_INPUT_ELIGIBILITY.tsv'));old=rows(B/(prefix+'_EVENTS.tsv'))
 for p in source:
  ...
  tt=[x for x in flat if x['form'] in S['targets']]
  for g in ['B','J','M']:
   ...
   for model in S['models']:
    ...
    for timing in S['timings']:
     ...
     baseline_count+=1
     for target in tt:
      prior=[x for x in flat if x['offset']<target['offset'] and x['role'] in ns['material_roles']]
      status='REPEATED_SAME' if any(x['form']==target['form'] for x in prior) else 'NO_PRIOR_MATERIAL' if not prior else 'ELIGIBLE'
      eligibility.append(dict(lexicon=lexicon,shol=sm,paragraph=p['id'],grammar=g,chol=model,timing=timing,target=target['id'],form=target['form'],status=status,parent=prior[-1]['id'] if prior else ''))
      ...
      meta=dict(lexicon=lexicon,shol=sm,paragraph=p['id'],grammar=g,chol=model,timing=timing,target=target['id']);targets.append(dict(**meta,antecedent=parent['id'],antecedent_form=parent['form'],antecedent_object=oid,earlier_applied_actions=';'.join(r['location'] for r in earlier),E_state=te['before'],R_state=next(r['before'] for _,r in trR if r['kind']=='MATERIAL' and r['location']==target['id'])))
      for reference,trace in [('E',trE),('R',trR)]:
       mentions={r['location']:r['object'] for _,r in trace if r['kind']=='MATERIAL'}
       for t in rows(B/(prefix+'_TAKE_ARGUMENTS.tsv')):
        if t['paragraph']==p['id'] and t['grammar']==g:
         takecases.append(dict(**meta,reference=reference,take=t['target'],patient=t['patient'],patient_form=t['patient_form'],object=mentions.get(t['patient'],''),rule=t['rule'],debts=t['debts']))
...
meta=['lexicon','shol','paragraph','grammar','chol','timing','target']
...
table('ELIGIBILITY.tsv',eligibility,list(eligibility[0]));table('TAKE_REFERENCES.tsv',takecases,list(takecases[0]))
res=dict(idea='IDEA000231',eligibility_cases=len(eligibility),unavailable=sum(r['status']!='ELIGIBLE' for r in eligibility),source_paragraphs=len(source),baseline_worlds_replayed=baseline_count,target_cases=len(targets),reference_worlds=len(summary),events=len(ev),changes=len(changes),later_requirements=len(later),constraint_discriminators=sum(r['constraint_discriminator'] for r in later),new_R_conflicts=sum(json.loads(r['R'])['status']=='CONFLICT' and json.loads(r['E'])['status']!='CONFLICT' for r in changes),independent_confirmation_capacity=0,confirmed_meanings=0,held_access=False)
