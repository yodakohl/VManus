...
for run in ['S_Q','S_A','I_Q','I_A']:
 candidate,sm=run.split('_');nominals=['chocthy','cthaiin']
 ...
 for p in source:
  ...
  boundaries=[-1]+[x['offset'] for x in ff if x['form']=='cfhy']+[len(ff)]
  def segment(location):
   off=next(x['offset'] for x in ff if x['id']==location)
   lower=max(b for b in boundaries if b<=off);upper=min(b for b in boundaries if b>off)
   return [x for x in ff if lower<=x['offset']<upper] if candidate=='I' else ff
  parsed={g:[] for g in ['B','J','M']}
  parts=[ff] if candidate=='S' else [[x for x in ff if lo<=x['offset']<hi] for lo,hi in zip(boundaries,boundaries[1:])]
  for part in parts:
   if part:
    for g,ars in solve(part).items():parsed[g].extend(ars)
  for g,ar in parsed.items():
   for x in ff:
    if x['form']=='ychor':
     key=(p['id'],g,x['id']);take_seen.add(key);assert all(take_index[key][k]==v for k,v in take(segment(x['id']),x,[a for a in ar if a['operation'] in {z['id'] for z in segment(x['id'])}],g).items())
   for a in ar:
   ...
   for x in ff:
    if x['role']=='STATE' and x['form'] in stand:
     f=stand[x['form']];q=requal(dict(mention=x['id'],kind='STANDALONE',form=x['form'],axis=f['axis'],value=f['value'],scope='PHYSICAL'),segment(x['id']),[a for a in ar if a['operation'] in {z['id'] for z in segment(x['id'])}]);key=(p['id'],g,x['id']);qseen.add(key);assert all(qx[key][k]==v for k,v in q.items());countqualities+=1
...
changes=rows(E/'ALL_CHANGES.tsv');fails=rows(E/'NEW_FAILURES.tsv');cases=rows(E/'CANDIDATES.tsv')
assert len(cases)==48 and len(fails)==24
for sm in ['Q','A']:
 for name in ['ARGUMENTS','QUALITY','TAKE_ARGUMENTS','EVENTS']:
  baseline=rows(W/'W36'/('PK_'+sm+'_'+name+'.tsv'));same=rows(E/('S_'+sm+'_'+name+'.tsv'));assert baseline==same
 for r in rows(E/('I_'+sm+'_ARGUMENTS.tsv')):
  if r['operation']=='f32v.9:1':assert not r['patient'] and not r['coingredient'] and r['debts']=='MISSING_RELATION_PARTNER'
 for name in ['QUALITY','TAKE_ARGUMENTS']:
  assert rows(E/('S_'+sm+'_'+name+'.tsv'))==rows(E/('I_'+sm+'_'+name+'.tsv'))
for c in cases:
 links=json.loads(c['cross_boundary_links'])
 if c['scope']=='S':
  assert (c['patient'],c['partner'],c['observed_status'])==('f32v.9:4','f32v.9:6','NO_STATE_TRANSITION_MODELLED')
  assert {(r['origin'],r['slot'],r['target']) for r in links}=={('f32v.9:1','patient','f32v.9:4'),('f32v.9:1','coingredient','f32v.9:6')}
 else:assert not links and not c['patient'] and not c['partner'] and c['observed_status']=='ARGUMENT_INCOMPLETE'
assert all(r['scope']=='I' and r['new_status']=='ARGUMENT_INCOMPLETE' and 'f32v.9:1' in r['key'] for r in fails)
assert collections.Counter(r['table'] for r in changes)=={'ARGUMENTS':6,'EVENTS':24}
result=dict(status='PASS',bound_files=len(S['inputs']),source_groups=1045,new_runs=4,replayed_arguments=countargs,replayed_qualities=countqualities,independent_state_events=sum(len(rr) for ww in worlds.values() for rr in ww.values()),S_exact_operational_baseline_parity=True,candidate_rows=48,new_missing_relation_events=24,limits='Frozen binding solvers on independently determined segments, separate state replay; no independent meaning validation')
