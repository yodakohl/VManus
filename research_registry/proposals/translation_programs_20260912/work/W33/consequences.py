...
for sm in ['Q','A']:
 cflat=rows(B/(sm+'_SOURCE_FLAT.tsv'));targets=[r for r in cflat if r['form'] in ['ol','lor']];Cevents=rows(B/(sm+'_EVENTS.tsv'));Ctakes=rows(B/(sm+'_TAKE_ARGUMENTS.tsv'))
   ...
   for grammar in ['B','J','M']:
    for model in ['D','H']:
     for timing in ['O','I']:
      ...
      direct=[r for r in nn if r['kind'] in ['ACTION','QUALITY'] and r['patient']==t['id']];objects={mat['object']} if mat else set();displaced={ci[eventkey(r)]['object'] for r in direct if eventkey(r) in ci and ci[eventkey(r)]['object']!=r['object']}-{''};objects|=displaced
...
table('CANDIDATES.tsv',out,list(out[0]));table('ALL_LATER_REQUIREMENTS.tsv',future,list(future[0]));table('ALL_LATER_TAKE.tsv',takefuture,list(takefuture[0]))
