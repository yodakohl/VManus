...
def main():
 ...
 for (host,reg),z in sorted(byhr.items()):
  ...
  for typ in("diff","same"):
   for _,a,b in sorted(cand[typ])[:200]:
    ...
    cs,nc=control_cache[ck]
    if not nc:continue
    pairrows.append({"host":host,"register":reg,"pair_type":typ.upper()+"_WRAPPER","left_unit":a["unit_id"],"right_unit":b["unit_id"],"context_similarity":sim,"matched_control_similarity":cs,"gain_vs_control":sim-cs,"control_units":nc});cell[host,reg][typ].append(sim);cell[host,reg]["control"].append(cs)
 cells=[]
 ...
 variants=[{"variant_id":"V00","status":"PRIMARY","description":"Host×register-balanced exact-host cross-wrapper page-context similarity; 200 SHA256-smallest pairs per cell/type."},{"variant_id":"V01","status":"RUN_CONTROL","description":"Different-host match on register, host length, page size, and wrapper contrast; unsupported pairs excluded."},{"variant_id":"V02","status":"RUN_SENSITIVITY","description":"Same-host same-wrapper comparison where capacity exists."},{"variant_id":"V03","status":"POSTSELECTED_DISPLAY","description":"GDT063 d and ok cells displayed but not selected for the manuscript-wide score."},{"variant_id":"V04","status":"NOT_RUN","description":"No external annotation score, semantic role, alternative parser, or f84r."},{"variant_id":"V05","status":"CORRECTION","description":"GDT065 audit removed 140 formerly retained zero-control pairs before corrected publication."}];write(VARIANTS,variants,list(variants[0]));p=binom_two(pos,n);status="CROSS_WRAPPER_PAGE_CONTEXT_PRESERVATION_SUPPORTED"if mean_diff>mean_ctrl and p<.05 else"CROSS_WRAPPER_PAGE_CONTEXT_PRESERVATION_WEAK_OR_UNSTABLE"
...
{len(pairrows):,} cross-folio exact-host pairs with a real matched control.
The corrected implementation excludes 140 formerly retained zero-control
pairs.  Across {n} host×register cells,
