...
## GDT754–GDT759 corrected whole-form formula and content bridge
...
GDT759 licenses seventeen observed exact construction types at 122 spans: 96 `s/or/ar+value`, 23 part/state and three `ols+value`. Seven complete three-reader lines normalize exactly across fused/separated boundaries, including four `s aiin/saiin`, so the inherited seed readings of `san/sain/saiin/saiiin` are quarantined. Exact `s+value` now leads concretely as one–four Drachmen with equal-parts and ounce rivals; `or+value` gives portions and `ar+value` shares. The 23 state pairs yield `chor↔chol=getrockneter Blüten-/Samenstand` (15), `cthy↔chol=getrocknetes Blattgut` (6) and `chor↔qokchol=erhitzt und getrockneter Blüten-/Samenstand` (2); no `sheol` part pair occurs. `odol=abgemessene Zubereitung` stays C1 and `ols` widens from end product to value-bearing `abgeseihte Zubereitung` C0. Report: `experiments/yolo/gdt759_quantity_part_state_construction_atlas/REPORT.md`. Next: follow recurrent wholes immediately left of these exact amount constructions to identify ingredient/preparation candidates such as water, wine, oil, salt, powder and plant parts without EVA-letter export.
