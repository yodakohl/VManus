...
Its creative Pass-1029/GDT401 release binds 4,581 groups (3,888 running, 693 local) in one physically ordered 627-statement edition with 4,374 rebuilt focus attachments and a revised fourteen-rule future-page error deck.
...
redefining roots.
Pass 1026 audits every one of the 271 events that Pass 1008 had flattened onto a one-edit neighbour. Only 23 events/21 surfaces retain the same recipe under a named package rule; 239 events/203 surfaces require a visibly different recipe, while `cheo` and `okeor` were already repaired in Pass 1025. Pass 1027/GDT399 then rebuilds scope from those recipes rather than patching old attachments: all 3,888 events, 627 statements and 4,374 focus atoms resolve, net +32 attachments; 127 bounded forward links look exactly one card ahead and no owner boundary is crossed. Pass 1028/GDT400 replays every attachment through exact-head, typed-micro, untyped-micro, rule-combination and component levels. All twenty running pages pass; 4,370 attachments transfer across registers without decomposing their rule combination, while four Biological forward-to-R rows initially remained amber. Pass 1029/GDT401 shows that these are only three visible card transitions: the f82r warning is two foci in one `OT+EE+Y` packet. All four scope rows reduce to an existing one-card forward rule followed by the existing R-head license, so no tenth family is needed. One semantic caution remains: `EE` stays inside the OT packet and may not be expanded as the duration or strength of `R`. Future pages may add rooted forms and local names, but an edit is never an allograph unless an explicit wrapper rule licenses it; absent atoms, alternate recipes, retuned cores, owner jumps and new coarse scope remain forbidden.
## Scientific ceiling
