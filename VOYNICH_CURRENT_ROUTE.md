...
GDT412's decisive repair is that 207/301 `CHD` events are open, so closure belongs to the licensed hull, not the core. GDT413 applies the dictionary to every current group, event and statement. GDT414–GDT418 add the error deck, reversible readings, imperative edition, cross-register recipes and P/L closure. GDT419 expands seven all-register anchors into controlled one-atom cells. GDT420 blocks free grade attachment with nine head-specific cards. GDT421 adds 81 ordered action-pair cards: order is substantive (`CH+K` 119 vs `K+CH` 21; `CH+T` 83 vs `T+CH` 17; `OK+CH` 23 vs `CH+OK` 0). GDT422 reduces all 110 recipes/168 events with 3–5 actions: 99 use only old adjacent pairs; eleven singleton gaps reduce through five visible slot boundaries, two R topologies, two repeated-action scopes and two peer-package breaks. No learned long-action card remains. GDT423 then hides every page key in turn: 2,553/2,662 clean action events replay an exact slot from another page, 50 combine old margins in a new cell and 59 remain locally red. GDT424 restores the older individual-head attachment map: 90/92 red-event focus edges transfer, 9/14 private pair events split at old package boundaries, and 8/9 closes transfer at the terminal head. Fifty of 57 red cells dissolve; only seven local combination cards on six pages remain. Role changes, second portable meanings, label lexicalization, unseen atoms and scope-crossing repairs remain red failures. These are working meanings, not confirmed lexemes or plaintext.
