...
GDT433 permits a bounded second ring only through two-arm squares; only `AIR+AIN` and `P+L` become Amber-II. GDT434 folds the 49-card main deck into a 1,563-key reader with 246 narrow exact-key-only entries. GDT435 corrects its unsafe shortcut: recipe+register has multiple clauses in 276/1,766 keys, while adding inherited action+argument yields 2,465 conflict-free state keys. GDT436 generates those fields without an event oracle: 57 page-owner banks reproduce 4,576/4,576 event clauses and 715/715 statements. GDT437 exhausts the 49-card deck over 49 reachable states and five registers; preserving written order separates the formerly universal `AIR+Y`/`Y+AIR` collision and gives 49 unique signatures. GDT438 integrates that repair into the prospective stream command: all 4,576 states and repaired clauses, all 715 statements and 245 future-card probes replay exactly; unseen or unlicensed input stops without state mutation. The 68 clause/59 statement changes are order-only. Exact ordered keys remain mandatory.
