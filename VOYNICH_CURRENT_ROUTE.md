...
GDT433 permits a bounded second ring only through two-arm squares; only `AIR+AIN` and `P+L` become Amber-II. GDT434 folds the 49-card main deck into a 1,563-key reader with 246 narrow exact-key-only entries. GDT435 corrects its unsafe shortcut: recipe+register has multiple clauses in 276/1,766 keys, while adding inherited action+argument yields 2,465 conflict-free state keys. GDT436 generates those fields without an event oracle: 57 page-owner banks reproduce 4,576/4,576 event clauses and 715/715 statements. GDT437 then exhausts the 49-card deck over 49 reachable states and five registers. The old renderer collapsed `AIR+Y` and `Y+AIR` in all 245 cells; preserving written relation/argument order removes every collision and gives 49 unique transition signatures. The repair reorders 68 current clauses/59 statements but changes no meaning, state, surface or page. Exact ordered keys remain mandatory.
