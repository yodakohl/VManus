...
GDT433 permits a bounded second ring only through two-arm squares: an observed base must reach two different strong GDT431 intermediates at different positions, and both intermediates must reach the same opposite corner. The complete inventory has only 21 squares/14 targets. `AIR+AIN` (BAHN · ANTEIL) and `P+L` (EINSETZEN · VERBINDUNG) are the only new targets with four observed bases and four strong arms; they become weaker second-ring amber cards. `AIR+OR` only reinforces its existing narrow rank, and eight one-square targets are not promoted. No surface is predicted.
