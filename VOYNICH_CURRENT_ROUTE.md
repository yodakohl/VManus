...
GDT433 permits a bounded second ring only through two-arm squares. Of 21 squares/14 targets, only `AIR+AIN` and `P+L` have four observed bases and four strong arms. GDT434 folds the resulting 49-card main deck into a 1,563-key reader with 246 narrow exact-key-only entries. GDT435 corrects its only unsafe shortcut: recipe+register has multiple full clauses in 276/1,766 keys (2,162 events), and taking the first would misread 1,401 events. Recipe+register+inherited action+inherited argument yields 2,465 conflict-free state keys, so the replacement reader prints only a safe core unless that state is present. The complete catalog also has 121 natural-phrase collision groups/261 recipes; ordered component keys remain mandatory. The 49 cards and all meanings remain unchanged; no surface or page is added.
