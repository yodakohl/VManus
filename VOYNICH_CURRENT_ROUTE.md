...
GDT412's decisive repair is that 207/301 `CHD` events are open, so closure belongs to the licensed hull, not the core. GDT413 applies the dictionary to every current group, event and statement. GDT414 turns it into a nineteen-core next-page error deck backed by all 8,505 current root mentions: new pictured objects may broaden owner-local expansions, but role changes, second portable meanings, label lexicalization, unseen atoms and scope-crossing repairs are red failures. These are working meanings, not confirmed lexemes or plaintext.
