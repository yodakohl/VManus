...
GDT433 permits a bounded second ring only through two-arm squares; only `AIR+AIN` and `P+L` become Amber-II. GDT434 folds the 49-card main deck into a 1,563-key reader with 246 narrow exact-key-only entries. GDT435 corrects its unsafe shortcut: recipe+register has multiple clauses in 276/1,766 keys, while adding inherited action+argument yields 2,465 conflict-free state keys. GDT436 now generates those fields without an event oracle. Fifty-seven page-owner banks, each storing only the last action and argument, reproduce 4,576/4,576 event states/clauses and 715/715 statements. Of the 49 future cards, 21 carry action+argument, fourteen need the active action, thirteen need the active argument and `AIR+OL` needs both. Exact ordered keys remain mandatory; the meanings, surfaces and pages are unchanged.
