...
Its creative Pass-1019 release binds 4,581 groups (3,888 running, 693 local) in one physically ordered 627-statement edition.
...
All 627 statements now have an owner–quantity–action–relation–grade–endpoint reading; the 19 local signs reduce to HIER, VARIANTE, KLASSE and VORBEZUG. Every portable core occurs in all four registers; cross-register and four-image rereading keep `AIIN=WERT`, `AIN=ANTEIL`, and `OR=EINHEIT`, while the other 16 values remain. Pictures supply plant-part, station-share, sector, ingredient, or entry-group expansions but show no universal measure, portion, or preparation. Fixed future readings are `chain=Anteil nehmen`, `pain=Anteil einsetzen`, `paiin=Wert einsetzen`, and `lair=Verbindung im Lauf`.
## Scientific ceiling
