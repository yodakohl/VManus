...
GDT412's decisive repair is that 207/301 `CHD` events are open, so closure belongs to the licensed hull, not the core. GDT413 applies the dictionary to every current group, event and statement. GDT414–GDT416 add the error deck, reversible register readings and complete imperative edition. GDT417 isolates seven fully self-contained all-register recipes; GDT418 retains `P=EINSETZEN` and broad `L=VERBINDUNG`. GDT419 expands the seven anchors into 120 controlled one-atom cells: 55 are attested. GDT420 maps 547 single-head recipes/2,184 events onto all 360 action×grade×argument×endpoint cells: 108 are attested, 145 open and 107 blocked. `CHD` and `R` license no grade, `R` no close, and `P` only a narrow grade/argument set; six GDT419 forms are now explicitly blocked. Role changes, second portable meanings, label lexicalization, unseen atoms and scope-crossing repairs remain red failures. These are working meanings, not confirmed lexemes or plaintext.
