...
GDT412's decisive repair is that 207/301 `CHD` events are open, so closure belongs to the licensed hull, not the core. GDT413 applies the dictionary to every current group, event and statement. GDT414–GDT416 add the nineteen-core error deck, 95 reversible register readings and a complete 4,576-clause/715-statement imperative edition. GDT417 finds 298 exact recipes shared across registers covering 3,317 events; twenty occur in all five registers and seven are fully self-contained (`CHD+Y`, `OK+EE+Y`, `OK+AIIN`, `SH+E+Y`, `CH+K+E+Y`, `K+EE+Y`, `CH+T+E+Y`). GDT418 retains `P=EINSETZEN` and broad `L=VERBINDUNG`. GDT419 expands the seven anchors into 120 controlled one-atom cells: 55 are attested, 45 under the same owner and 38 across registers. Grad deletion is 5/5 and grade substitution 6/10, but free grade insertion is 0/6, so E/EE/EEE is a licensed slot rather than a freely attachable suffix. Role changes, second portable meanings, label lexicalization, unseen atoms and scope-crossing repairs remain red failures. These are working meanings, not confirmed lexemes or plaintext.
