...
## GDT754–GDT757 corrected whole-form formula and content bridge

GDT754 removes literal prose from 172 source-composed wholes; GDT755 gives the fixed top-24 concrete whole candidates; GDT756 refines `ychor` to **Item/ferner/ebenso** and fills its 71 body positions. GDT757 now separates eleven high-triad/high-initial-purity wholes over 79 initial lines: `pchor=nimm` opens 6/7 paragraphs versus `ychor=ferner/ebenso` 0/13; `polaiin/pol` lead as entry or preparation heads; `tshol=für/gegen` is the Herbal-purpose lead; `qokchor=mische hinein`; and `ycheol/ychol/dcheol/ycheor` form an internal/closure neighborhood with only 1/26 paragraph starts but 9/26 ends, led by `ycheor=zum Schluss`. Every reading has two rivals and evidence; `paiin=drei Teile/dritte Menge` remains forced C0. Four attractive low-purity forms (`ykar/yteedy/qotor/dchey`) are not made global formula words. Reports: `experiments/yolo/gdt756_ychor_line_frame_content_slots/REPORT.md` and `experiments/yolo/gdt757_initial_formula_role_atlas/REPORT.md`.
The next route follows every admitted occurrence of the direct `ychor` content followers, using the new opening/continuation/addition/indication slots to choose among concrete whole-form rivals such as leaf/root/seed, water/wine/oil and soak/dry/cool. Preserve alternatives, but do not export a letter, substring or EVA spelling resemblance.
