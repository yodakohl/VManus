...
GDT433 permits a bounded second ring only through two-arm squares; only `AIR+AIN` and `P+L` become Amber-II. GDT434 folds the 49-card main deck into a 1,563-key reader with 246 narrow exact-key-only entries. GDT435–GDT438 supply left context, preserve `AIR+Y` versus `Y+AIR` order and publish an oracle-free stream command: all 4,576 states/clauses, 715 statements and 245 main-card probes replay exactly, with safe stops. GDT439 expands the audit to 382,935 transitions for all 1,563 keys. The 49 main cards remain mutually unique, but 218 catalog recipes occupy 104 semantic collision groups: 76 erase same-atom order and 28 contain intentionally co-valued local controls. Five main cards touch only external catalog cards—four order repairs plus one HIER-channel collision. Exact keys remain authoritative; no meaning, surface or page changed.
