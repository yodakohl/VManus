...
GDT412's decisive repair is that 207/301 `CHD` events are open, so closure belongs to the licensed hull, not the core. GDT413 applies the dictionary to every current group, event and statement. GDT414 turns it into a nineteen-core next-page error deck backed by all 8,505 current root mentions. GDT415 then supplies 95 reversible register readings, 906 attested root×owner combinations and concrete readings for all 4,576 events/715 statements: local nouns change with the visible owner, while every reading maps back to the same core. Role changes, second portable meanings, label lexicalization, unseen atoms and scope-crossing repairs remain red failures. These are working meanings, not confirmed lexemes or plaintext.
